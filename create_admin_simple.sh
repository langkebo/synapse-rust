...
echo "=== 创建管理员账户并测试管理员API ==="

# 步骤1: 创建管理员账户（使用admin字段）
echo "步骤1: 创建管理员账户（使用admin字段）..."
RESULT=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/_matrix/client/r0/register" \
    -H "Content-Type: application/json" \
    -d "{\"username\":\"$ADMIN_USERNAME\",\"password\":\"$ADMIN_PASSWORD\",\"admin\":true,\"auth\":{\"type\":\"m.login.dummy\"}}" 2>&1)
...
# 步骤3: 测试管理员API - 获取服务器版本
echo ""
echo "步骤3: 测试管理员API - 获取服务器版本..."
