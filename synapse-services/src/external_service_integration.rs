...
impl ExternalServiceIntegration {
    ...
    fn verify_webhook_auth(
        ...
        if auth
            ...
            .any(|candidate| secrets.iter().any(|s| synapse_common::crypto::secure_compare(candidate, s)))
        {
            return Ok(());
        }

        let payload_bytes = serde_json::to_vec(payload)
            .map_err(|e| ApiError::internal_with_cause("Failed to serialize webhook payload", e))?;
        ...
        let signature_matches = auth.signature.as_deref().is_some_and(|signature| {
            ...
            secrets.iter().any(|secret| {
                let expected = URL_SAFE_NO_PAD.encode(synapse_common::crypto::hmac_sha256(secret, &payload_bytes));
                synapse_common::crypto::secure_compare(normalized, &expected)
                    || synapse_common::crypto::secure_compare(signature, &expected)
            })
        });
    ...
    pub async fn register_external_service(
        ...
        let service = self
            ...
            .register(request)
            .await
            .map_err(|e| ApiError::internal_with_cause("Failed to register external service", e))?;
    ...
    pub async fn unregister_external_service(&self, request_id: &str, service_id: &str) -> Result<(), ApiError> {
        ...
        self.storage
            ...
            .await
            .map_err(|e| ApiError::internal_with_cause("Failed to unregister service", e))?;
    ...
    pub async fn update_external_service(
        ...
        self.storage
            ...
            .await
            .map_err(|e| ApiError::internal_with_cause("Failed to update service", e))?
            .ok_or_else(|| ApiError::not_found("Service not found"))
    }
    ...
    pub async fn list_external_services(
        ...
        let services = self
            ...
            .get_all_active()
            .await
            .map_err(|e| ApiError::internal_with_cause("Failed to get services", e))?;
...
mod tests {
    ...
    async fn test_verify_webhook_auth_accepts_direct_token_and_hmac_signature() {
        let integration = ExternalServiceIntegration::new(
            Arc::new(synapse_storage::application_service::ApplicationServiceStorage::new(&Arc::new(
        ...
        let signature = format!(
            "sha256={}",
            URL_SAFE_NO_PAD
                .encode(synapse_common::crypto::hmac_sha256("config-secret", serde_json::to_vec(&payload).unwrap()))
        );
