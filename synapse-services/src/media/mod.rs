...
impl MediaDomainService {
    ...
    pub fn new(
        ...
        Self {
            ...
            quarantine_change_storage: None,
            cache_invalidation: None,
        }
    }

    /// Attach quarantine stream storage and cache invalidation support.
    /// This enables the quarantine/unquarantine admin APIs and stream writer
    ...
    /// Remove quarantine from a media item by server_name/media_id.
    /// Records the change in the quarantine stream and updates media_metadata.
    /// Invalidates relevant caches in multi-worker deployments.
    ...
    /// Get quarantine changes since a given stream position.
    /// Used by workers to catch up on quarantine state changes.
    ...
    pub async fn get_user_alerts(
        ...
        user_id: &str,
        unread_only: bool,
    ) -> Result<Vec<synapse_storage::media_quota::MediaQuotaAlert>, ApiError> {
...
mod tests {
    ...
    async fn prepare_media_test_pool() -> Result<Arc<sqlx::PgPool>, String> {
        // Use the shared isolated test pool which clones from the v11 baseline.
        // This ensures all required tables (users, upload_progress, media_metadata,
        // quarantined_media_changes, server_media_quota, etc.) are present in the
        // test schema, avoiding FK violations when queries silently fall back to
        // the shared public schema.
        test_utils::prepare_isolated_test_pool().await
    }
    ...
    ) -> (MediaDomainService, MediaService, Vec<synapse_storage::user::User>, tempfile::TempDir) {
    ...
    async fn setup_test_media_domain(
        username: &str,
    ) -> (MediaDomainService, MediaService, synapse_storage::user::User, tempfile::TempDir) {
