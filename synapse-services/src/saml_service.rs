...
impl SamlService {
    ...
    pub async fn apply_runtime_overrides(&self, patch: serde_json::Value) -> Result<serde_json::Value, ApiError> {
...

        {
            let mut guard = self
                .runtime_overrides
                .lock()
                .map_err(|e| ApiError::internal_with_context("SAML runtime overrides lock poisoned", &e))?;
            for (k, v) in patch_map {
                guard.insert(k, v);
            }
        }
    ...
    async fn fetch_idp_metadata(&self) -> Result<SamlMetadata, ApiError> {
        let metadata_xml = if let Some(ref url) = self.config.metadata_url {
            let response = self
                ...
                .send()
                .await
                .map_err(|e| ApiError::internal_with_cause("Failed to fetch IdP metadata", e))?;

            if !response.status().is_success() {
                return Err(ApiError::internal_with_context("IdP metadata request failed", &response.status()));
            }

            response.text().await.map_err(|e| ApiError::internal_with_cause("Failed to read IdP metadata", e))?
   ...
   <samlp:SessionIndex>{}</samlp:SessionIndex>
    ...
    fn build_redirect_url(
        ...
        let mut url =
            url::Url::parse(base_url).map_err(|e| ApiError::internal_with_cause("Invalid SAML base URL", e))?;
        {
    ...
    fn parse_metadata_xml(xml: &str) -> Result<SamlMetadata, ApiError> {
        let parsed =
            parse_saml_metadata(xml).map_err(|e| ApiError::internal_with_cause("Failed to parse SAML metadata", e))?;
...
mod tests {
    use super::*;
    use synapse_storage::saml::SamlStorage;
    ...
    fn create_test_config() -> SamlConfig {
        SamlConfig {
            ...
            attribute_mapping: synapse_common::config::SamlAttributeMapping {
    ...
    fn create_test_service() -> SamlService {
        let pool = Arc::new(
            sqlx::PgPool::connect_lazy(&synapse_common::test_isolation::test_database_url())
