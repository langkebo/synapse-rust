...
impl FederationBlacklistService {
    ...
    pub async fn add_to_blacklist(
        ...
        let storage_request = synapse_storage::federation_blacklist::AddBlacklistRequest {
        ...
        self.storage
            .create_log(synapse_storage::federation_blacklist::CreateLogRequest {
    ...
    fn matches_rule(cache: &RegexCache, server_name: &str, rule: &FederationBlacklistRule) -> Result<bool, ApiError> {
        match rule.rule_type.as_str() {
            ...
            "regex" => cache
                ...
                .map_err(|e| ApiError::internal_with_cause("Invalid regex pattern", e)),
            "wildcard" => {
                let pattern = format!("^{}$", rule.pattern.replace('*', ".*"));
                cache
                    .is_match(&pattern, server_name)
                    .map_err(|e| ApiError::internal_with_cause("Invalid wildcard pattern", e))
            }
            "cidr" => Ok(false),
            _ => Ok(false),
        }
    ...
    pub async fn record_access(
        ...
        let stats = self
            .storage
            .update_access_stats(synapse_storage::federation_blacklist::UpdateStatsRequest {
