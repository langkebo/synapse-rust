...
impl WorkerStorage {
    ...
    pub async fn get_worker(&self, worker_id: &str) -> Result<Option<WorkerInfo>, sqlx::Error> {
        let row = sqlx::query_as::<_, WorkerRow>(r"SELECT id, worker_id, worker_name, worker_type, host, port, status, last_heartbeat_ts, started_ts, stopped_ts, config, metadata, version FROM workers WHERE worker_id = $1")
            .bind(worker_id)
    ...
    pub async fn get_workers_by_type(&self, worker_type: &str) -> Result<Vec<WorkerInfo>, sqlx::Error> {
        let rows =
            sqlx::query_as::<_, WorkerRow>(r"SELECT id, worker_id, worker_name, worker_type, host, port, status, last_heartbeat_ts, started_ts, stopped_ts, config, metadata, version FROM workers WHERE worker_type = $1 ORDER BY started_ts DESC")
                .bind(worker_type)
                .fetch_all(&*self.pool)
                .await?;
    ...
    pub async fn get_active_workers(&self) -> Result<Vec<WorkerInfo>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkerRow>(r"SELECT id, worker_id, worker_name, worker_type, host, port, status, last_heartbeat_ts, started_ts, stopped_ts, config, metadata, version FROM active_workers").fetch_all(&*self.pool).await?;
    ...
    pub async fn get_pending_commands(&self, worker_id: &str, limit: i64) -> Result<Vec<WorkerCommand>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkerCommandRow>(
            r"
            SELECT id, command_id, target_worker_id, source_worker_id, command_type, command_data, priority, status, created_ts, sent_ts, completed_ts, error_message, retry_count, max_retries FROM worker_commands
    ...
    pub async fn add_event(
        ...
        let stream_id: i64 =
            sqlx::query("SELECT nextval('worker_event_stream_id_seq')").fetch_one(&*self.pool).await?.get(0);
    ...
    pub async fn get_events_since(&self, stream_id: i64, limit: i64) -> Result<Vec<WorkerEvent>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkerEventRow>(
            r"SELECT id, event_id, stream_id, event_type, room_id, sender, event_data, created_ts, processed_by FROM worker_events WHERE stream_id > $1 ORDER BY stream_id ASC LIMIT $2",
        )
        .bind(stream_id)
        .bind(limit)
        .fetch_all(&*self.pool)
    ...
    pub async fn get_replication_position(
        ...
        let result =
            sqlx::query(r"SELECT stream_position FROM replication_positions WHERE worker_id = $1 AND stream_name = $2")
                .bind(worker_id)
                .bind(stream_name)
                .fetch_optional(&*self.pool)
                .await?;
    ...
    pub async fn get_pending_tasks(&self, limit: i64) -> Result<Vec<WorkerTaskAssignment>, sqlx::Error> {
        let rows = sqlx::query_as::<_, WorkerTaskAssignment>(
            r"
            SELECT id, task_id, task_type, task_data, assigned_worker_id, status, priority, created_ts, assigned_ts, completed_ts, result, error_message FROM worker_task_assignments
    ...
    pub async fn get_statistics(&self) -> Result<Vec<serde_json::Value>, sqlx::Error> {
        let rows = sqlx::query(r"SELECT id, worker_id, worker_name, worker_type, status, host, port, last_heartbeat_ts, started_ts, cpu_usage, memory_usage, active_connections, requests_per_second, average_latency_ms, queue_depth, pending_commands, active_tasks FROM worker_statistics").fetch_all(&*self.pool).await?;
    ...
    pub async fn get_type_statistics(&self) -> Result<Vec<serde_json::Value>, sqlx::Error> {
        let rows = sqlx::query(r"SELECT worker_type, total_count, running_count, starting_count, stopping_count, stopped_count, avg_cpu_usage, avg_memory_usage, total_connections FROM worker_type_statistics").fetch_all(&*self.pool).await?;
