...
impl RoomSummaryService {
    ...
    pub async fn get_stats(&self, room_id: &str) -> Result<Option<RoomSummaryStats>, ApiError> {
        ...
        match stats_res {
            Ok(s) => Ok(s),
            Err(e) => Err(ApiError::internal_with_cause("Failed to get stats", e)),
        }
    }
    ...
    pub async fn recalculate_stats(&self, room_id: &str) -> Result<RoomSummaryStats, ApiError> {
        ...
        match stats_res {
            Ok(s) => Ok(s),
            Err(e) => Err(ApiError::internal_with_cause("Failed to update stats", e)),
        }
    }
    ...
    pub async fn queue_update(
        ...
        match result {
            Ok(_) => Ok(()),
            Err(e) => Err(ApiError::internal_with_cause("Failed to queue update", e)),
        }
    }
    ...
    pub async fn process_pending_updates(&self, limit: i64) -> ApiResult<usize> {
        ...
        let updates = match updates_res {
            Ok(u) => u,
            Err(e) => return Err(ApiError::internal_with_cause("Failed to get pending updates", e)),
        };
    ...
    async fn process_update(&self, update: &RoomSummaryUpdateQueueItem) -> Result<(), ApiError> {
        ...
        let event = match event_res {
            ...
            Err(e) => return Err(ApiError::internal_with_cause("Failed to get event", e)),
        };
        ...
        if event.state_key.is_some() {
            ...
            if let Err(e) = update_res {
                return Err(ApiError::internal_with_cause("Failed to update summary", e));
            }
        }
    ...
    pub async fn increment_unread(&self, room_id: &str, highlight: bool) -> Result<(), ApiError> {
        ...
        match result {
            Ok(_) => Ok(()),
            Err(e) => Err(ApiError::internal_with_cause("Failed to increment unread", e)),
        }
    }
    ...
    pub async fn clear_unread(&self, room_id: &str) -> Result<(), ApiError> {
        ...
        match result {
            Ok(_) => Ok(()),
            Err(e) => Err(ApiError::internal_with_cause("Failed to clear unread", e)),
        }
    }
    ...
    pub async fn recalculate_heroes(&self, room_id: &str) -> Result<Vec<String>, ApiError> {
        ...
        if let Err(e) = set_hero_res {
            return Err(ApiError::internal_with_cause("Failed to update hero flags", e));
        }

        let hero_users = match serde_json::to_value(&hero_ids) {
            Ok(v) => v,
            Err(e) => return Err(ApiError::internal_with_cause("Failed to serialize heroes", e)),
        };
        ...
        if let Err(e) = update_res {
            return Err(ApiError::internal_with_cause("Failed to update heroes", e));
        }
