...
// ============================================================================
// Per-test isolated pool
// ============================================================================
//
// The template machinery above is deliberately primitive-only: it takes a
// database URL and a baseline string. This section is the convenience wrapper
// every DB test actually wants, and it lives here (not in a consumer crate)
// because `synapse-common` is compiled unconditionally — sibling crates'
// `#[cfg(test)]` fixtures cannot see a `#[cfg(test)]` module of a dependency
// (Rust does not propagate `cfg(test)` across crates), so a wrapper parked in
// `synapse-storage` is unreachable from `synapse-e2ee`, `synapse-federation`,
// etc. Those crates then hand-rolled pools pointed at the shared `public`
// schema and raced the unit target that wipes it.
//
// The baseline SQL stays a *parameter*: `synapse-common` must not embed the
// workspace migrations (see the module docs), so each caller supplies the bytes
// and the fingerprint-named template is shared across callers that pass the
// same bytes.
...
/// template (the name is a content fingerprint). `tests/unit/test_isolation_unification_tests.rs`
/// pins each caller's input to the single v12 baseline.
...
impl IsolatedTestPool {
    /// Create a new isolated test pool whose schema is a clone of the shared
    /// baseline template built from `baseline_sql`.
    pub async fn new(baseline_sql: &str) -> Result<Self, sqlx::Error> {
        let db_url = test_database_url();

        let template = ensure_template_schema(&db_url, baseline_sql).await.map_err(sqlx::Error::Protocol)?;
        ...
        clone_schema_from_template(&clone_pool, &schema, &template, SeedSource::Everything)
            .await
