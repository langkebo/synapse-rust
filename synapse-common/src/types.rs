...
matrix_id!(MediaId, "media_id", doc = "Media upload ID generated by the server, e.g. `AQDaVFlbkQoErdOgqWRgiGSV`.");
matrix_id!(SessionId, "session_id", doc = "MSC4108 rendezvous session ID, opaque token.");
matrix_id!(BackupId, "backup_id", doc = "E2EE key backup version ID, e.g. `backup-1234567890`.");
...
mod tests {
    ...
    fn test_matrix_id_constructors() {
        ...
        // MediaId
        let mid = MediaId::new_unchecked("AQDaVFlbkQoErdOgqWRgiGSV");
        assert_eq!(mid.as_str(), "AQDaVFlbkQoErdOgqWRgiGSV");
    }
