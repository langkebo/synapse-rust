...
# Re-test script for warning and failed APIs
...
OUTPUT_FILE="/tmp/api_retest_results.txt"
echo "=== API Re-test Results ===" > "$OUTPUT_FILE"
...
test_api() {
    ...
    if [ -z "$body" ]; then
        ...
        response=$(curl -s -w "\n%{http_code}" -X "$method" "$url" $headers -d "$body" 2>&1)
    fi
    ...
    echo "Response Time: ${duration}s" >> "$OUTPUT_FILE"
...
test_api "Refresh token" \
    ...
    '-H "Content-Type: application/json"' \
    '{"refresh_token":"6v4CEA6tTZqlEZV7ijUqrV38EX88S8a5Xq9lmJs88qk="}'
...
# 2.1 Update device (PUT) - test if method is actually supported
test_api "Update device (PUT)" \
    "PUT" \
    "$BASE_URL/_matrix/client/r0/devices/lcS8Laaw01fG%2FU%2BEoR8wHQ%3D%3D" \
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -H \"Content-Type: application/json\"" \
    '{"display_name":"Test Device Updated"}'

# 2.2 Delete single device (DELETE) - test if method is actually supported
...
# 3.2 Get room members (first join the room)
...
# 4.1 Send message - test if POST is supported or if it needs PUT
test_api "Send message (POST)" \
    "POST" \
    "$BASE_URL/_matrix/client/r0/rooms/$ROOM_ID/send/m.room.message/$(date +%s%N)" \
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -H \"Content-Type: application/json\"" \
    '{"msgtype":"m.text","body":"Test message from re-test"}'
...
test_api "Send friend request" \
    ...
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -H \"Content-Type: application/json\"" \
    "{\"user_id\":\"$ADMIN_ID\",\"message\":\"Test friendship\"}"
...
test_api "Block user (correct field)" \
    ...
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -H \"Content-Type: application/json\"" \
    "{\"user_id\":\"$ADMIN_ID\",\"reason\":\"Test block\"}"
...
test_api "Upload voice message (valid base64)" \
    ...
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -H \"Content-Type: application/json\"" \
    '{"content":"VGVzdCBhdWRpbyBjb250ZW50","content_type":"audio/ogg","duration_ms":5000,"room_id":"'$ROOM_ID'"}'
...
test_api "Upload keys (POST)" \
    ...
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -H \"Content-Type: application/json\"" \
    '{"device_keys":{"user_id":"'$USER_ID'","device_id":"lcS8Laaw01fG/U+EoR8wHQ==","algorithms":["m.olm.v1.curve25519-aes-sha2"],"keys":{},"signatures":{}},"one_time_keys":{}}'
...
test_api "Upload media v3 (multipart)" \
    ...
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -F \"file=@/tmp/test_media.jpg;type=image/jpeg\""
...
test_api "Delete message (valid ID)" \
    "DELETE" \
    "$BASE_URL/_synapse/enhanced/private/messages/$(uuidgen)" \
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\""
...
test_api "Get backup version (latest)" \
    "GET" \
    "$BASE_URL/_matrix/client/r0/room_keys/version/$(date +%s)" \
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\""
...
test_api "Get room keys" \
    "GET" \
    "$BASE_URL/_matrix/client/r0/room_keys/$(date +%s)" \
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\""
...
test_api "Get room keys by room ID" \
    "GET" \
    "$BASE_URL/_matrix/client/r0/room_keys/$(date +%s)/keys/$ROOM_ID" \
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\""

# 10.7 Get session key
test_api "Get session key" \
    "GET" \
    "$BASE_URL/_matrix/client/r0/room_keys/$(date +%s)/keys/$ROOM_ID/session123" \
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\""
...
test_api "Create session" \
    ...
    "-H \"Authorization: Bearer $TEST_USER_TOKEN\" -H \"Content-Type: application/json\"" \
    "{\"user_id\":\"$ADMIN_ID\",\"title\":\"Private Chat\"}"
