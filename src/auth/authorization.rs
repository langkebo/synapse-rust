...
impl AuthorizationService {
    ...
    pub async fn check_resource_access(
        ...
        match resource_type {
            ResourceType::User => {
                self.check_user_access(ctx, resource_owner_id, &action)
                    .await?;
            }
            ResourceType::Room => {
                self.check_room_access(ctx, resource_owner_id, &action)
                    .await?;
            }
            ResourceType::Device => {
                self.check_device_access(ctx, resource_owner_id, &action)
                    .await?;
            }
            ResourceType::Media => {
                self.check_media_access(ctx, resource_owner_id, &action)
                    .await?;
            }
            _ => {}
        }
    ...
    async fn check_room_access(
        ...
        match action {
            ...
            Action::ModifyPowerLevels => {
                ...
                let user_power = self.get_user_power_level(room_id, &ctx.user_id).await?;
