...
impl BackgroundUpdateService {
    ...
    pub async fn create_update(&self, request: CreateBackgroundUpdateRequest) -> Result<BackgroundUpdate, ApiError> {
        ...
        let update = self.storage.create_update(request).await
    ...
    pub async fn get_update(&self, job_name: &str) -> Result<Option<BackgroundUpdate>, ApiError> {
        let update = self.storage.get_update(job_name).await
    ...
    #[instrument(skip(self))]
    pub async fn get_all_updates(&self, limit: i64, offset: i64) -> Result<Vec<BackgroundUpdate>, ApiError> {
        let updates = self.storage.get_all_updates(limit, offset).await
    ...
    pub async fn get_pending_updates(&self) -> Result<Vec<BackgroundUpdate>, ApiError> {
        let updates = self.storage.get_pending_updates().await
    ...
    pub async fn get_running_updates(&self) -> Result<Vec<BackgroundUpdate>, ApiError> {
        let updates = self.storage.get_running_updates().await
    ...
    pub async fn start_update(&self, job_name: &str) -> Result<BackgroundUpdate, ApiError> {
        ...
        let locked = self.storage.acquire_lock(job_name, &Uuid::new_v4().to_string(), 300000).await
        ...
        let update = self.storage.update_status(job_name, "running").await
    ...
    #[instrument(skip(self))]
    pub async fn update_progress(&self, job_name: &str, items_processed: i32, total_items: Option<i32>) -> Result<BackgroundUpdate, ApiError> {
        let update = self.storage.update_progress(job_name, items_processed, total_items).await
            .map_err(|e| ApiError::internal(format!("Failed to update progress: {}", e)))?;

        if update.progress >= 100 || (update.total_items > 0 && update.processed_items >= update.total_items) {
    ...
    pub async fn complete_update(&self, job_name: &str) -> Result<BackgroundUpdate, ApiError> {
        ...
        let update = self.storage.update_status(job_name, "completed").await
    ...
    pub async fn fail_update(&self, job_name: &str, error_message: &str) -> Result<BackgroundUpdate, ApiError> {
        ...
        let update = self.storage.set_error(job_name, error_message).await
    ...
    pub async fn cancel_update(&self, job_name: &str) -> Result<BackgroundUpdate, ApiError> {
        ...
        let update = self.storage.update_status(job_name, "cancelled").await
    ...
    pub async fn delete_update(&self, job_name: &str) -> Result<(), ApiError> {
        self.storage.delete_update(job_name).await
    ...
    pub async fn retry_failed(&self) -> Result<i64, ApiError> {
        ...
        let count = self.storage.retry_failed().await
    ...
    pub async fn cleanup_expired_locks(&self) -> Result<i64, ApiError> {
        ...
        let count = self.storage.cleanup_expired_locks().await
    ...
    pub async fn get_history(&self, job_name: &str, limit: i64) -> Result<Vec<BackgroundUpdateHistory>, ApiError> {
        let history = self.storage.get_history(job_name, limit).await
    ...
    pub async fn count_by_status(&self, status: &str) -> Result<i64, ApiError> {
        let count = self.storage.count_by_status(status).await
    ...
    pub async fn count_all(&self) -> Result<i64, ApiError> {
        let count = self.storage.count_all().await
            .map_err(|e| ApiError::internal(format!("Failed to count updates: {}", e)))?;
    ...
    pub async fn is_locked(&self, job_name: &str) -> Result<bool, ApiError> {
        let locked = self.storage.is_locked(job_name).await
        ...
        for update in pending {
            if let Some(ref depends_on) = update.depends_on {
                ...
                for dep in depends_on {
                    if let Some(dep_update) = self.storage.get_update(dep).await
                        .map_err(|e| ApiError::internal(format!("Failed to check dependency: {}", e)))? {
            ...
            let locked = self.storage.is_locked(&update.job_name).await
                .map_err(|e| ApiError::internal(format!("Failed to check lock: {}", e)))?;
