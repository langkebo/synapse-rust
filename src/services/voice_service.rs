...
impl VoiceService {
    ...
    pub async fn save_voice_message(
        ...
        {
            return Err(ApiError::internal("Generated invalid event ID".to_string()));
        }
    ...
    pub async fn get_voice_message(&self, event_id: &str) -> ApiResult<Option<(Vec<u8>, String)>> {
        let voice_storage = VoiceStorage::new(&self.pool, self.cache.clone());
...
mod tests {
    ...
    async fn test_voice_service_extension_mpeg() {
        ...
        assert_eq!(
            service.get_extension_from_content_type("audio/mpeg3"),
            "mp3"
        );
    }
    ...
    async fn test_voice_service_extension_webm() {
        ...
        assert_eq!(
            service.get_extension_from_content_type("audio/webm"),
            "webm"
        );
        assert_eq!(
    ...
    async fn test_voice_service_extension_unknown() {
        ...
        assert_eq!(
            service.get_extension_from_content_type("audio/flac"),
            "audio"
        );
        assert_eq!(
            service.get_extension_from_content_type("audio/aac"),
            "audio"
        );
        assert_eq!(
            service.get_extension_from_content_type("audio/unknown"),
            "audio"
        );
        assert_eq!(
            service.get_extension_from_content_type("video/mp4"),
            "audio"
        );
    }
