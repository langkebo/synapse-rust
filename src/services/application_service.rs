...
impl ApplicationServiceManager {
    ...
    pub async fn register(
        ...
        if let Some(existing) = self.storage.get_by_id(&request.as_id).await
        ...
        let service = self.storage.register(request).await
            .map_err(|e| ApiError::internal(format!("Failed to register application service: {}", e)))?;
    ...
    pub async fn get(&self, as_id: &str) -> Result<Option<ApplicationService>, ApiError> {
        self.storage.get_by_id(as_id).await
    ...
    pub async fn get_by_token(&self, as_token: &str) -> Result<Option<ApplicationService>, ApiError> {
        let service = self.storage.get_by_token(as_token).await
            .map_err(|e| ApiError::internal(format!("Failed to get application service by token: {}", e)))?;

        if let Some(ref svc) = service {
            let _ = self.storage.update_last_seen(&svc.as_id).await
    ...
    pub async fn unregister(&self, as_id: &str) -> Result<(), ApiError> {
        ...
        self.storage.unregister(as_id).await
            .map_err(|e| ApiError::internal(format!("Failed to unregister application service: {}", e)))?;
    ...
    pub async fn set_state(
        ...
        self.storage.set_state(as_id, state_key, state_value).await
    ...
    pub async fn get_state(&self, as_id: &str, state_key: &str) -> Result<Option<ApplicationServiceState>, ApiError> {
        self.storage.get_state(as_id, state_key).await
    ...
    pub async fn get_all_states(&self, as_id: &str) -> Result<Vec<ApplicationServiceState>, ApiError> {
        self.storage.get_all_states(as_id).await
    ...
    pub async fn push_event(
        ...
        let event = self.storage.add_event(
            &event_id,
            as_id,
            room_id,
            event_type,
            ...
            content,
            state_key,
        ).await.map_err(|e| ApiError::internal(format!("Failed to add event: {}", e)))?;
    ...
    pub async fn get_pending_events(&self, as_id: &str, limit: i64) -> Result<Vec<ApplicationServiceEvent>, ApiError> {
        self.storage.get_pending_events(as_id, limit).await
    ...
    pub async fn send_transaction(
        ...
        let service = self.storage.get_by_id(as_id).await
        ...
        let _transaction = self.storage.create_transaction(as_id, &transaction_id, &events).await
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                let _ = self.storage.complete_transaction(as_id, &transaction_id).await
            ...
            Ok(resp) => {
                ...
                let _ = self.storage.fail_transaction(as_id, &transaction_id, &format!("HTTP {}: {}", status, error_body)).await
            ...
            Err(e) => {
                let _ = self.storage.fail_transaction(as_id, &transaction_id, &e.to_string()).await
                    .map_err(|e| error!("Failed to fail transaction: {}", e));

                Err(ApiError::internal(format!("Failed to send transaction: {}", e)))
            }
        }
    }
    ...
    pub async fn query_user(&self, user_id: &str) -> Result<Option<String>, ApiError> {
        self.storage.is_user_in_namespace(user_id).await
    ...
    pub async fn query_room_alias(&self, alias: &str) -> Result<Option<String>, ApiError> {
        self.storage.is_room_alias_in_namespace(alias).await
    ...
    pub async fn query_room_id(&self, room_id: &str) -> Result<Option<String>, ApiError> {
        self.storage.is_room_id_in_namespace(room_id).await
    ...
    pub async fn register_virtual_user(
        ...
        let user = self.storage.register_virtual_user(as_id, user_id, displayname, avatar_url).await
    ...
    pub async fn get_virtual_users(&self, as_id: &str) -> Result<Vec<ApplicationServiceUser>, ApiError> {
        self.storage.get_virtual_users(as_id).await
    ...
    pub async fn get_namespaces(&self, as_id: &str) -> Result<NamespacesInfo, ApiError> {
        let users = self.storage.get_user_namespaces(as_id).await
            .map_err(|e| ApiError::internal(format!("Failed to get user namespaces: {}", e)))?;
        let aliases = self.storage.get_room_alias_namespaces(as_id).await
            .map_err(|e| ApiError::internal(format!("Failed to get room alias namespaces: {}", e)))?;
        let rooms = self.storage.get_room_namespaces(as_id).await
    ...
    pub async fn ping(&self, as_id: &str) -> Result<bool, ApiError> {
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                let _ = self.storage.update_last_seen(as_id).await
                    .map_err(|e| warn!("Failed to update last seen: {}", e));
                Ok(true)
            }
            _ => Ok(false),
