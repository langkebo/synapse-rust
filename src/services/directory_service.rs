...
pub trait DirectoryService: Send + Sync {
    ...
    /// 
    /// 返回公共房间列表。
    async fn get_public_rooms(&self, limit: i32, since: Option<&str>) -> ApiResult<Vec<DirectoryRoom>>;
    ...
    /// 
    /// 返回匹配的公共房间列表。
    async fn search_public_rooms(&self, filter: Option<&str>, limit: i32) -> ApiResult<Vec<DirectoryRoom>>;
}
...
impl DirectoryService for DirectoryServiceImpl {
    ...
    async fn get_public_rooms(&self, limit: i32, _since: Option<&str>) -> ApiResult<Vec<DirectoryRoom>> {
        let rooms = self.public_rooms.read().await;
        let result: Vec<DirectoryRoom> = rooms
            .values()
            .take(limit as usize)
            .cloned()
            .collect();
...
mod tests {
    ...
    async fn test_get_public_rooms() {
        ...
        service.add_public_room(DirectoryRoom {
