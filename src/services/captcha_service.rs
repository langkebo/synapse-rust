...
impl CaptchaService {
    ...
    pub async fn send_captcha(
        ...
        let captcha = self.storage.create_captcha(CreateCaptchaRequest {
        ...
        let send_result = self.send_captcha_via_provider(
            &captcha,
            &code,
            expiry_minutes,
        ...
        self.storage.create_send_log(CreateSendLogRequest {
    ...
    async fn send_captcha_via_provider(
        ...
        let template = if let Some(name) = template_name {
            ...
            self.storage.get_default_template(&captcha.captcha_type).await?
        ...
        match captcha.captcha_type.as_str() {
            "email" => {
                self.send_email(&captcha.target, template.subject.as_deref(), &content).await
            }
            "sms" => {
                self.send_sms(&captcha.target, &content).await
            }
            "image" => {
                Ok(())
            }
    ...
    fn render_template(&self, template: &CaptchaTemplate, code: &str, expiry_minutes: i32) -> String {
    ...
    async fn send_email(&self, to: &str, _subject: Option<&str>, content: &str) -> Result<(), ApiError> {
