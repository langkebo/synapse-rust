...
pub trait DMService: Send + Sync {
    ...
    /// ```
    async fn get_existing_dm(&self, user_id: &str, recipient_id: &str) -> ApiResult<Option<String>>;
    ...
    /// 
    /// 如果存储操作失败，返回错误。
    async fn mark_room_as_dm(&self, room_id: &str, creator_id: &str, recipients: &[String]) -> ApiResult<()>;
    ...
    /// * `user_id` - 执行更新的用户 ID
    /// * `users` - 新的用户 ID 列表
    async fn update_dm_users(&self, room_id: &str, user_id: &str, users: &[String]) -> ApiResult<()>;
}
...
impl DMService for DMServiceImpl {
    ...
    async fn update_dm_users(&self, _room_id: &str, _user_id: &str, _users: &[String]) -> ApiResult<()> {
        Ok(())
    }
}
