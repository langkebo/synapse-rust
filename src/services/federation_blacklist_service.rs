...
impl FederationBlacklistService {
    ...
    pub async fn add_to_blacklist(
        ...
        self.storage.create_log(crate::storage::federation_blacklist::CreateLogRequest {
    ...
    pub async fn check_server(&self, server_name: &str) -> Result<CheckResult, ApiError> {
        ...
        let default_action = self.storage.get_config("default_action").await?
    ...
    fn matches_rule(&self, server_name: &str, rule: &FederationBlacklistRule) -> Result<bool, ApiError> {
    ...
    pub async fn record_access(
        ...
        let stats = self.storage.update_access_stats(crate::storage::federation_blacklist::UpdateStatsRequest {
    ...
    async fn check_auto_blacklist(&self, server_name: &str) -> Result<(), ApiError> {
        let auto_blacklist_enabled = self.storage.get_config_as_bool("enable_auto_blacklist", true).await?;
        
        if !auto_blacklist_enabled {
            return Ok(());
        }

        let threshold = self.storage.get_config_as_int("auto_blacklist_threshold", 10).await?;
        let window_minutes = self.storage.get_config_as_int("auto_blacklist_window_minutes", 60).await?;
                ...
                if let Some(last_failure) = stats.last_failure_at {
                    ...
                    if last_failure > window_start {
                        ...
                        self.add_to_blacklist(AddBlacklistRequest {
                            ...
                            reason: Some(format!("Auto-blacklisted: {} failures in {} minutes", stats.failed_requests, window_minutes)),
    ...
    pub async fn get_blacklist(&self, limit: i32, offset: i32) -> Result<Vec<FederationBlacklist>, ApiError> {
