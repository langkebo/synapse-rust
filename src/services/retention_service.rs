...
impl RetentionService {
    ...
    pub async fn get_room_policy(&self, room_id: &str) -> Result<Option<RoomRetentionPolicy>, ApiError> {
        let policy = self.storage.get_room_policy(room_id).await
    ...
    #[instrument(skip(self))]
    pub async fn set_room_policy(&self, request: CreateRoomRetentionPolicyRequest) -> Result<RoomRetentionPolicy, ApiError> {
        ...
        let policy = self.storage.create_room_policy(request).await
    ...
    #[instrument(skip(self))]
    pub async fn update_room_policy(&self, room_id: &str, request: UpdateRoomRetentionPolicyRequest) -> Result<RoomRetentionPolicy, ApiError> {
        let policy = self.storage.update_room_policy(room_id, request).await
    ...
    pub async fn delete_room_policy(&self, room_id: &str) -> Result<(), ApiError> {
        ...
        self.storage.delete_room_policy(room_id).await
    ...
    pub async fn get_server_policy(&self) -> Result<ServerRetentionPolicy, ApiError> {
        let policy = self.storage.get_server_policy().await
    ...
    #[instrument(skip(self))]
    pub async fn update_server_policy(&self, request: UpdateServerRetentionPolicyRequest) -> Result<ServerRetentionPolicy, ApiError> {
        info!("Updating server retention policy");

        let policy = self.storage.update_server_policy(request).await
    ...
    pub async fn run_cleanup(&self, room_id: &str) -> Result<RetentionCleanupLog, ApiError> {
        ...
        if policy.max_lifetime.is_none() {
            return Err(ApiError::bad_request("No retention policy configured for this room"));
        }
        ...
        match self.storage.delete_events_before(room_id, cutoff_ts).await {
            Ok(deleted_count) => {
                if let Err(e) = self.storage.complete_cleanup_log(log.id, deleted_count, 0, 0, 0).await {
                    warn!("Failed to complete cleanup log: {}", e);
                }

                if let Err(e) = self.storage.update_stats(room_id, 0, 0, deleted_count, None).await {
                    warn!("Failed to update stats: {}", e);
                }

                info!("Deleted {} expired events from room {}", deleted_count, room_id);
    ...
    pub async fn process_pending_cleanups(&self, limit: i64) -> Result<usize, ApiError> {
        let items = self.storage.get_pending_cleanups(limit).await
        ...
        for item in items {
            match self.process_cleanup_item(&item).await {
                ...
                Err(e) => {
                    if let Err(err) = self.storage.mark_cleanup_failed(item.id, &e.to_string()).await {
    ...
    async fn process_cleanup_item(&self, item: &RetentionCleanupQueueItem) -> Result<(), ApiError> {
        if let (Some(event_id), Some(event_type)) = (&item.event_id, &item.event_type) {
            ...
            self.storage.record_deleted_event(&item.room_id, event_id, "retention").await
                .map_err(|e| ApiError::internal(format!("Failed to record deleted event: {}", e)))?;
        }
    ...
    pub async fn schedule_room_cleanup(&self, room_id: &str) -> Result<i64, ApiError> {
        ...
        let count = self.storage.schedule_room_cleanup(room_id).await
    ...
    #[instrument(skip(self))]
    pub async fn get_cleanup_logs(&self, room_id: &str, limit: i64) -> Result<Vec<RetentionCleanupLog>, ApiError> {
        let logs = self.storage.get_cleanup_logs(room_id, limit).await
    ...
    #[instrument(skip(self))]
    pub async fn get_deleted_events(&self, room_id: &str, since_ts: i64) -> Result<Vec<DeletedEventIndex>, ApiError> {
        let events = self.storage.get_deleted_events(room_id, since_ts).await
    ...
    pub async fn get_rooms_with_policies(&self) -> Result<Vec<RoomRetentionPolicy>, ApiError> {
        let policies = self.storage.get_rooms_with_policies().await
            .map_err(|e| ApiError::internal(format!("Failed to get rooms with policies: {}", e)))?;
    ...
    pub async fn get_pending_cleanup_count(&self, room_id: &str) -> Result<i64, ApiError> {
        let count = self.storage.get_pending_cleanup_count(room_id).await
            .map_err(|e| ApiError::internal(format!("Failed to get pending cleanup count: {}", e)))?;
