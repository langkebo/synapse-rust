...
impl AlertManager {
    ...
    pub fn check_health_status(
        &mut self,
        health: &crate::storage::DatabaseHealthStatus,
    ) -> Vec<AlertState> {
    ...
    fn evaluate_rule(
        ...
        match &rule.condition {
                ...
                if health.connection_pool_status.connection_utilization > *threshold {
                    Some(AlertState {
                        ...
                        message: rule.message.replace(
                            ...
                            &format!(
                                "{:.1}",
                                health.connection_pool_status.connection_utilization
                            ),
                        ),
                        acknowledged: false,
                    })
            ...
            AlertCondition::IntegrityScoreLow { threshold: _ } => None,
...
mod tests {
    ...
    fn test_alert_condition_variants() {
        ...
        assert!(matches!(
            pool_high,
            AlertCondition::ConnectionPoolHigh { .. }
        ));
        assert!(matches!(
            slow_queries,
            AlertCondition::SlowQueriesHigh { .. }
        ));
        assert!(matches!(
            integrity_low,
            AlertCondition::IntegrityScoreLow { .. }
        ));
    ...
    fn test_alert_config_clone() {
        ...
        assert_eq!(
            config.connection_pool_threshold,
            cloned.connection_pool_threshold
        );
    }
