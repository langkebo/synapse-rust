...
pub(crate) fn media_legacy_result_response(
    result: Result<MediaResponsePayload, ApiError>,
) -> MediaHttpResponse {
    match result {
...
mod tests {
    ...
    fn test_api_error_response_returns_json_body_and_headers() {
        let (status, headers, body) =
            api_error_response(&ApiError::forbidden("forbidden".to_string()));
    ...
    fn test_binary_success_response_sets_content_headers() {
        let (status, headers, body) =
            binary_success_response("application/octet-stream", b"abc".to_vec());
