...
pub(crate) async fn send_receipt(
    ...
    if let Some(event) = ctx.room_service.get_event_record(&event_id).await? {
    ...
    ctx.room_service.send_receipt(&room_id, &auth_user.user_id, &event_id, &receipt_type, &body).await?;
...
pub(crate) async fn get_receipts(
    ...
    let receipts = ctx.room_service.get_receipts(&room_id, &receipt_type, &event_id).await?;
...
pub(crate) async fn set_read_markers(
    ...
    ctx.room_service.set_read_markers(&room_id, &auth_user.user_id, &body).await?;
