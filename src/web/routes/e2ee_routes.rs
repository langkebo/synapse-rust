...
pub fn create_e2ee_router(state: AppState) -> Router {
    Router::new()
        .route(
...
#[axum::debug_handler]
async fn query_keys(State(_state): State<AppState>, Json(_body): Json<Value>) -> Json<Value> {
    Json(serde_json::json!({
        "device_keys": {}
    }))
}
...
#[axum::debug_handler]
async fn claim_keys(State(_state): State<AppState>, Json(_body): Json<Value>) -> Json<Value> {
    Json(serde_json::json!({
        "one_time_keys": {}
    }))
}
...
#[axum::debug_handler]
async fn key_changes(State(_state): State<AppState>, Query(params): Query<Value>) -> Json<Value> {
