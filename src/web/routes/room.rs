...
use crate::web::routes::{
    ...
    invite_blocklist, invite_user, invite_user_by_room, join_room, join_room_by_id_or_alias, kick_user, knock_room,
    leave_room, pinned, put_state_event, put_state_event_empty_key, put_state_event_no_key, redact_event,
    room_initial_sync, search_room_messages, send_message, send_receipt, send_state_event, set_read_markers,
    set_room_account_data, set_room_vault_data, sign_room_event, sticky_event, translate_room_event, translate_text,
...
fn create_room_r0_v3_compat_router() -> Router<AppState> {
    Router::new()
        ...
        .route("/rooms/{room_id}/redact/{event_id}/{txn_id}", put(redact_event).post(redact_event))
...
fn room_r0_v3_shared_relative_routes() -> Vec<(axum::http::Method, &'static str)> {
    ...
    let base: Vec<(Method, &'static str)> = vec![
        ...
        (Method::POST, "/rooms/{room_id}/receipt/{receipt_type}/{event_id}"),
        (Method::GET, "/rooms/{room_id}/receipts/{receipt_type}/{event_id}"),
        ...
        (Method::PUT, "/rooms/{room_id}/state/{event_type}/{state_key}"),
        (Method::GET, "/rooms/{room_id}/state/{event_type}/{state_key}"),
...
async fn get_anti_screenshot(
    ...
    let enabled = events
        .into_iter()
        .find(|e| e.get("state_key").and_then(|v| v.as_str()).is_none_or(|s| s.is_empty()))
        .and_then(|e| e.get("content").and_then(|c| c.get("action")).and_then(|v| v.as_str()).map(|s| s.to_string()))
