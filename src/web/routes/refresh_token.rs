...
use crate::storage::refresh_token::{RefreshToken, RefreshTokenStats, RefreshTokenUsage};
...
pub async fn refresh(
    ...
    let (new_refresh_token, _token_record) = state
        .services
...
pub async fn get_user_tokens(
    ...
    let response: Vec<RefreshTokenResponse> =
        tokens.into_iter().map(RefreshTokenResponse::from).collect();
...
pub async fn revoke_all_tokens(
    ...
    let count = state
        .services
        .refresh_token_service
        .revoke_all_user_tokens(&user_id, &body.reason)
        .await?;
...
pub async fn get_usage_history(
    ...
    let response: Vec<TokenUsageResponse> =
        history.into_iter().map(TokenUsageResponse::from).collect();
...
pub fn create_refresh_token_router(state: AppState) -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/_synapse/admin/v1/users/{user_id}/tokens/active",
            get(get_active_tokens),
        )
        .route(
            "/_synapse/admin/v1/users/{user_id}/tokens/revoke_all",
            post(revoke_all_tokens),
        )
        .route(
            "/_synapse/admin/v1/users/{user_id}/tokens/stats",
            get(get_token_stats),
        )
        .route(
            "/_synapse/admin/v1/users/{user_id}/tokens/usage",
            get(get_usage_history),
        )
        .route("/_synapse/admin/v1/tokens/{id}", delete(delete_token))
        .route("/_synapse/admin/v1/tokens/{id}/revoke", post(revoke_token))
        .route(
            "/_synapse/admin/v1/tokens/cleanup",
