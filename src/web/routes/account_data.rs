...
pub fn create_account_data_router(state: AppState) -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/_matrix/client/unstable/org.matrix.msc3391/user/{user_id}/account_data/{type}",
            delete(delete_account_data),
        )
        .route(
        ...
        .route(
            "/_matrix/client/unstable/org.matrix.msc3391/user/{user_id}/rooms/{room_id}/account_data/{type}",
            delete(delete_room_account_data),
        )
        .route(
...
async fn delete_account_data(
    ...
    let result = sqlx::query(
        "DELETE FROM account_data WHERE user_id = $1 AND data_type = $2"
    )
    .bind(&user_id)
    .bind(&data_type)
    .execute(&*state.services.user_storage.pool)
    ...
    if result.rows_affected() == 0 {
        tracing::debug!(
            user_id = %user_id,
            data_type = %data_type,
            "Account data not found for deletion, returning success per MSC3391"
        );
    }
...
async fn delete_room_account_data(
    ...
    if result.rows_affected() == 0 {
        tracing::debug!(
            ...
            room_id = %room_id,
            data_type = %data_type,
            "Room account data not found for deletion, returning success per MSC3391"
        );
    }
