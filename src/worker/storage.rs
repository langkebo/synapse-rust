...
impl WorkerStorage {
    ...
    pub async fn register_worker(&self, request: RegisterWorkerRequest) -> Result<WorkerInfo, sqlx::Error> {
        ...
        let row: WorkerRow = sqlx::query_as!(
            ...
            RETURNING id as "id!", worker_id as "worker_id!", worker_name as "worker_name!",
    ...
    pub async fn add_event(
        ...
        // SKIP: sequence may not exist at compile time, processed_by type mismatch — keep as runtime sqlx::query_as
    ...
    pub async fn get_events_since(&self, stream_id: i64, limit: i64) -> Result<Vec<WorkerEvent>, sqlx::Error> {
        // SKIP: processed_by type mismatch with sqlx::query_as! — keep as runtime sqlx::query_as
    ...
    pub async fn mark_event_processed(&self, event_id: &str, worker_id: &str) -> Result<(), sqlx::Error> {
        // SKIP: array_append with jsonb type not supported by sqlx::query! — keep as runtime sqlx::query
        sqlx::query(
            r"
    ...
    pub async fn get_replication_position(
        ...
        let result =
            sqlx::query_scalar!(
                r#"SELECT stream_position as "stream_position!" FROM replication_positions WHERE worker_id = $1 AND stream_name = $2"#,
                worker_id,
                stream_name
            )
                .fetch_optional(&*self.pool)
    ...
    pub async fn get_statistics(&self) -> Result<Vec<serde_json::Value>, sqlx::Error> {
        // SKIP: table schema doesn't match query columns — keep as runtime sqlx::query
        let rows = sqlx::query(
