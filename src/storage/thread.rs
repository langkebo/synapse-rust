...
impl ThreadStorage {
    ...
    pub async fn create_thread_root(&self, params: CreateThreadRootParams) -> Result<ThreadRoot, sqlx::Error> {
        ...
        sqlx::query_as!(
            ...
            RETURNING id as "id!", room_id as "room_id!", root_event_id as "root_event_id!", sender as "sender!", thread_id, reply_count as "reply_count!",
                      last_reply_event_id, last_reply_sender, last_reply_ts,
                      participants, is_fetched as "is_fetched!", created_ts as "created_ts!", updated_ts
            "#,
            &params.room_id,
    ...
    pub async fn get_thread_root(&self, room_id: &str, thread_id: &str) -> Result<Option<ThreadRoot>, sqlx::Error> {
        sqlx::query_as!(
            ThreadRoot,
            r#"SELECT id as "id!", room_id as "room_id!", root_event_id as "root_event_id!", sender as "sender!", thread_id, reply_count as "reply_count!",
                   last_reply_event_id, last_reply_sender, last_reply_ts,
                   participants, is_fetched as "is_fetched!", created_ts as "created_ts!", updated_ts
            FROM thread_roots
    ...
    pub async fn create_thread_reply(&self, params: CreateThreadReplyParams) -> Result<ThreadReply, sqlx::Error> {
        ...
        let reply = sqlx::query_as!(
            ...
            RETURNING id as "id!", room_id as "room_id!", thread_id as "thread_id!", event_id as "event_id!", root_event_id as "root_event_id!", sender as "sender!",
                      in_reply_to_event_id, content as "content!", origin_server_ts as "origin_server_ts!", is_edited as "is_edited!", is_redacted as "is_redacted!", created_ts as "created_ts!"
            "#,
            &params.room_id,
            &params.thread_id,
            &params.event_id,
    ...
    pub async fn get_thread_replies(
        ...
        if let Some(from) = from {
            sqlx::query_as!(
                ThreadReply,
                r#"SELECT id as "id!", room_id as "room_id!", thread_id as "thread_id!", event_id as "event_id!", root_event_id as "root_event_id!", sender as "sender!",
                       in_reply_to_event_id, content as "content!", origin_server_ts as "origin_server_ts!", is_edited as "is_edited!", is_redacted as "is_redacted!", created_ts as "created_ts!"
                FROM thread_replies
    ...
    pub async fn get_reply_count(&self, room_id: &str, thread_id: &str) -> Result<i32, sqlx::Error> {
        let result = sqlx::query!(
            r#"SELECT COUNT(*) as "count!" FROM thread_replies
            WHERE room_id = $1 AND thread_id = $2"#,
            room_id,
            thread_id
        )
    ...
    pub async fn get_thread_participants(&self, room_id: &str, thread_id: &str) -> Result<Vec<String>, sqlx::Error> {
        ...
        Ok(result.into_iter().map(|r| r.sender).collect())
    }
    ...
    pub async fn subscribe_to_thread(
        ...
        sqlx::query_as!(
            ...
            RETURNING id as "id!", room_id as "room_id!", thread_id as "thread_id!", user_id as "user_id!", notification_level as "notification_level!", is_muted as "is_muted!", is_pinned as "is_pinned!", subscribed_ts as "subscribed_ts!", updated_ts as "updated_ts!"
            "#,
            room_id,
            thread_id,
            user_id,
    ...
    pub async fn get_thread_subscription(
        ...
        sqlx::query_as!(
            ThreadSubscription,
            r#"SELECT id as "id!", room_id as "room_id!", thread_id as "thread_id!", user_id as "user_id!", notification_level as "notification_level!", is_muted as "is_muted!", is_pinned as "is_pinned!", subscribed_ts as "subscribed_ts!", updated_ts as "updated_ts!"
    ...
    pub async fn update_read_receipt(
        ...
        sqlx::query_as!(
            ...
            ON CONFLICT (room_id, thread_id, user_id) DO UPDATE SET
                ...
                unread_count = 0,
                updated_ts = EXCLUDED.updated_ts
            RETURNING id as "id!", room_id as "room_id!", thread_id as "thread_id!", user_id as "user_id!", last_read_event_id, last_read_ts as "last_read_ts!", unread_count as "unread_count!", updated_ts as "updated_ts!"
            "#,
            room_id,
            thread_id,
            user_id,
    ...
    pub async fn get_read_receipt(
        ...
        sqlx::query_as!(
            ThreadReadReceipt,
            r#"SELECT id as "id!", room_id as "room_id!", thread_id as "thread_id!", user_id as "user_id!", last_read_event_id, last_read_ts as "last_read_ts!", unread_count as "unread_count!", updated_ts as "updated_ts!"
    ...
    pub async fn create_thread_relation(
        ...
        sqlx::query_as!(
            ...
            RETURNING id as "id!", room_id as "room_id!", event_id as "event_id!", relates_to_event_id as "relates_to_event_id!", relation_type as "relation_type!", thread_id, is_falling_back as "is_falling_back!", created_ts as "created_ts!"
            "#,
            room_id,
            event_id,
            relates_to_event_id,
    ...
    pub async fn search_threads(
        ...
        sqlx::query_as!(
            ...
            SELECT
                ...
                tr.room_id as "room_id!",
                COALESCE(tr.thread_id, '') as "thread_id!",
                tr.root_event_id as "root_event_id!",
                tr.sender as "root_sender!",
                COALESCE(e.content, '{}'::jsonb) as "root_content!",
                COALESCE(e.origin_server_ts, tr.created_ts) as "root_origin_server_ts!",
                latest_reply.latest_event_id,
            ...
            ) DESC, COALESCE(latest_reply.latest_origin_server_ts, e.origin_server_ts, tr.created_ts) DESC NULLS LAST
            LIMIT $3
            "#,
            room_id,
            &escaped_query,
