...
impl FederationQueueStorage {
    ...
    pub async fn get_pending_by_destination(
        ...
        sqlx::query_as!(FederationQueueEntry,
            r#"
            SELECT id as "id!", destination as "destination!", event_id as "event_id!", event_type as "event_type!", room_id, content as "content!", created_ts as "created_ts!", sent_at, retry_count as "retry_count!", status as "status!"
    ...
    pub async fn count_pending(&self) -> Result<i64, sqlx::Error> {
        let row =
            sqlx::query_scalar!(r#"SELECT COUNT(*) as "count!" FROM federation_queue WHERE status = 'pending'"#)
                .fetch_one(&self.pool)
                .await?;
