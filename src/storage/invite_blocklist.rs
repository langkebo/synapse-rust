...
impl InviteBlocklistStorage {
    ...
    pub async fn set_invite_blocklist(&self, room_id: &str, user_ids: Vec<String>) -> Result<(), sqlx::Error> {
        ...
        sqlx::query!(
            r#"
            DELETE FROM room_invite_blocklist WHERE room_id = $1
            "#,
            room_id,
        )
        .execute(&*self.pool)
        ...
        // SKIP: unnest($2::text[]) array parameter - not supported by sqlx::query!
    ...
    pub async fn get_invite_blocklist(&self, room_id: &str) -> Result<Vec<String>, sqlx::Error> {
        let rows = sqlx::query_scalar!(
            r#"SELECT user_id AS "user_id!" FROM room_invite_blocklist WHERE room_id = $1"#,
            room_id,
        )
        .fetch_all(&*self.pool)
        .await?;
    ...
    pub async fn is_user_blocked(&self, room_id: &str, user_id: &str) -> Result<bool, sqlx::Error> {
        let result = sqlx::query_scalar!(
            r#"SELECT user_id AS "user_id!" FROM room_invite_blocklist
    ...
    pub async fn set_invite_allowlist(&self, room_id: &str, user_ids: Vec<String>) -> Result<(), sqlx::Error> {
        ...
        sqlx::query!(
            r#"
            DELETE FROM room_invite_allowlist WHERE room_id = $1
            "#,
            room_id,
        )
        .execute(&*self.pool)
    ...
    pub async fn get_invite_allowlist(&self, room_id: &str) -> Result<Vec<String>, sqlx::Error> {
        let rows = sqlx::query_scalar!(
            r#"SELECT user_id AS "user_id!" FROM room_invite_allowlist WHERE room_id = $1"#,
            room_id,
        )
        .fetch_all(&*self.pool)
        .await?;
    ...
    pub async fn is_user_allowed(&self, room_id: &str, user_id: &str) -> Result<bool, sqlx::Error> {
        let result = sqlx::query_scalar!(
            r#"SELECT user_id AS "user_id!" FROM room_invite_allowlist
            WHERE room_id = $1 AND user_id = $2"#,
            room_id,
            user_id,
        )
    ...
    pub async fn has_any_invite_restriction(&self, room_id: &str) -> Result<bool, sqlx::Error> {
        let blocklist = sqlx::query_scalar!(
            r#"SELECT 1 AS "exists!" FROM room_invite_blocklist WHERE room_id = $1 LIMIT 1"#,
            room_id,
        )
        .fetch_optional(&*self.pool)
        .await?;
        ...
        let allowlist = sqlx::query_scalar!(
            r#"SELECT 1 AS "exists!" FROM room_invite_allowlist WHERE room_id = $1 LIMIT 1"#,
            room_id,
        )
        .fetch_optional(&*self.pool)
        .await?;
    ...
    pub async fn get_global_invite_blocklist(&self) -> Result<Vec<serde_json::Value>, sqlx::Error> {
        let rows = sqlx::query!(
            r#"SELECT room_id AS "room_id!", user_id AS "user_id!", created_ts AS "created_ts!" FROM room_invite_blocklist
    ...
    pub async fn get_global_invite_allowlist(&self) -> Result<Vec<serde_json::Value>, sqlx::Error> {
        let rows = sqlx::query!(
            r#"SELECT room_id AS "room_id!", user_id AS "user_id!", created_ts AS "created_ts!" FROM room_invite_allowlist
            ORDER BY created_ts DESC"#,
        )
        .fetch_all(&*self.pool)
        .await?;
