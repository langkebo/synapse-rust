...
use crate::common::rate_limit_config::{RateLimitEndpointRule, RateLimitRule};
    ...
    fn from(rule: RateLimitEndpointRule) -> Self {
        Self {
            ...
            match_type: match rule.match_type {
                ...
                crate::common::rate_limit_config::RateLimitMatchType::Prefix => {
                    "prefix".to_string()
                }
            },
            rule: rule.rule.into(),
...
pub fn create_rate_limit_admin_router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/_admin/rate-limit/endpoints/{path}",
            delete(remove_endpoint_rule),
        )
        .route("/_admin/rate-limit/exempt-paths", get(get_exempt_paths))
        .route("/_admin/rate-limit/exempt-paths", post(add_exempt_path))
        .route(
            "/_admin/rate-limit/exempt-paths/{path}",
            delete(remove_exempt_path),
        )
...
pub async fn set_rate_limit_enabled(
    ...
    manager.set_enabled(body.enabled).await.map_err(|e| {
        ApiError::internal(format!("Failed to update rate limit enabled status: {}", e))
    })?;
...
pub async fn remove_endpoint_rule(
    ...
    manager
        .remove_endpoint_rule(&decoded_path)
        .await
...
pub async fn add_exempt_path(
    ...
    manager
        .add_exempt_path(body.path.clone())
        .await
...
pub async fn remove_exempt_path(
    ...
    manager
        .remove_exempt_path(&decoded_path)
        .await
