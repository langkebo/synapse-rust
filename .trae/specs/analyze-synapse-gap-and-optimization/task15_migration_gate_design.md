...
- `room_summaries` / `room_summary_members` 已形成第四条样板：
  ...
  - 行为闭环：创建 summary -> 添加成员 -> 更新 membership/is_hero -> 校验聚合计数和排序
...
- `db-migration-gate.yml` 当前已把 `room summary`、`space`、`account_data`、`search`、`e2ee` 五条 integration target 接到 `sqlx Migrate Run` 阻断链路中。
...
- unified schema 之后的关键补偿迁移现以 `scripts/ci/critical_migrations.txt` 为准；`20260406000001-00004` 已纳入该清单，确保 unified apply、expected baseline、sqlx forward-only source 与 migration gate 口径一致。
