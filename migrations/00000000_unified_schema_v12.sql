...
--   - migrations/ 下只允许存在一个 `00000000_unified_schema_v*.sql` 基线
--   - 同一张表/同一个索引名在本文件中只能出现一次；重复段落在
--     `IF NOT EXISTS` 语义下整体空转，会让"文件多长"与"实际 schema"脱钩
...
-- ============================================================================
-- 完整性约束与性能索引折入块（append-only，只定义一次，不可删除）
--
-- 内容：
--   - P0-5 完整性约束 / P0-6 热点索引
--   - P1 增量迁移 20260904010000_schema_p1_federation_and_integrity 的 FK/CHECK/索引
--   - P2 增量迁移 20260904020000_schema_p2_data_integrity 的 UQ/CHECK/索引
--   - P3 增量迁移 20260904030000_schema_p3_perf 的索引与 backup_keys 约束
--
-- 这些对象在 baseline 主体中**没有**等价定义，删除本块会让全新库缺 10 个索引与
-- 9 个约束。本块内的 CREATE INDEX CONCURRENTLY 不能在事务内执行，故保留
-- `--no-transaction` 标记。索引清单见 migrations/INDEXES.md。
-- ============================================================================
