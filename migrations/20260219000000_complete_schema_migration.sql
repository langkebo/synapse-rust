-- =============================================================================
-- Synapse-Rust 完整数据库迁移脚本
-- 版本: 20260219000000
-- 创建日期: 2026-02-19
-- 描述: 确保数据库架构完整性，添加所有缺失的表结构
-- 
-- 执行方式:
--   docker exec -i synapse-postgres psql -U synapse -d synapse_test < 20260219000000_complete_schema_migration.sql
--
-- 回滚方式:
...
VALUES ('20260219000000', 'Complete schema migration - ensure all tables exist')
...
CREATE TABLE IF NOT EXISTS saml_user_mapping (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT saml_user_mapping_user_unique UNIQUE(name_id)
);
...
CREATE INDEX IF NOT EXISTS idx_registration_tokens_enabled ON registration_tokens(enabled);
...
CREATE TABLE IF NOT EXISTS registration_token_batches (
    ...
    uses_per_token INTEGER DEFAULT 1,
    expiry_time TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    created_by VARCHAR(255),
    description TEXT
...
CREATE INDEX IF NOT EXISTS idx_registration_token_batches_batch ON registration_token_batches(batch_id);
...
CREATE TABLE IF NOT EXISTS registration_token_usage (
    ...
    ip_address VARCHAR(45),
    user_agent TEXT,
    FOREIGN KEY (token) REFERENCES registration_tokens(token) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_registration_token_usage_token ON registration_token_usage(token);
CREATE INDEX IF NOT EXISTS idx_registration_token_usage_user ON registration_token_usage(user_id);
...
CREATE TABLE IF NOT EXISTS background_updates (
    ...
    average_items_per_ms DOUBLE PRECISION DEFAULT 0,
    total_item_count BIGINT DEFAULT 0,
    inserted_ts BIGINT
);
...
CREATE TABLE IF NOT EXISTS refresh_token_usage (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT refresh_token_usage_unique UNIQUE(user_id, date)
);
...
CREATE TABLE IF NOT EXISTS space_children (
    ...
    child_room_id VARCHAR(255) NOT NULL,
    "order" INTEGER,
    suggested BOOLEAN DEFAULT FALSE,
    via_servers JSONB DEFAULT '[]',
    created_ts BIGINT,
    CONSTRAINT space_children_unique UNIQUE(space_id, child_room_id)
);

CREATE INDEX IF NOT EXISTS idx_space_children_space ON space_children(space_id);
CREATE INDEX IF NOT EXISTS idx_space_children_child ON space_children(child_room_id);
...
CREATE TABLE IF NOT EXISTS thread_replies (
    ...
    content JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    FOREIGN KEY (thread_id) REFERENCES thread_roots(thread_id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_server_notifications_status ON server_notifications(status);
CREATE INDEX IF NOT EXISTS idx_server_notifications_scheduled ON server_notifications(scheduled_at);
...
CREATE INDEX IF NOT EXISTS idx_application_services_enabled ON application_services(enabled);
...
CREATE INDEX IF NOT EXISTS idx_worker_connections_worker ON worker_connections(worker_id);
