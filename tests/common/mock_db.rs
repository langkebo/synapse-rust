...
pub async fn get_test_pool() -> Option<Pool<Postgres>> {
    ...
    PgPoolOptions::new().max_connections(5).acquire_timeout(Duration::from_secs(10)).connect(&database_url).await.ok()
}
...
pub async fn create_test_user(pool: &Pool<Postgres>, user_id: &str, username: &str) -> Result<(), sqlx::Error> {
