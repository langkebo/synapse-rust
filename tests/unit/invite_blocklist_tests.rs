...
mod invite_blocklist_suite {
    ...
    async fn seed_room(pool: &sqlx::PgPool, suffix: u128, other_users: &[String]) -> (String, String, Vec<String>) {
        ...
        for user_id in other_users {
            let username = user_id.trim_start_matches('@').split(':').next().unwrap_or("user").to_string();
    ...
    async fn test_invite_blocklist_operations() {
        ...
        let blocked_users = vec![format!("@blocked1{suffix}:localhost"), format!("@blocked2{suffix}:localhost")];
        let (_creator, room_id, users) = seed_room(&pool, suffix, &blocked_users).await;

        // Set blocklist
        storage.set_invite_blocklist(&room_id, blocked_users.clone()).await.expect("Failed to set blocklist");

        // Get blocklist
        let result = storage.get_invite_blocklist(&room_id).await.expect("Failed to get blocklist");
    ...
    async fn test_invite_allowlist_operations() {
        ...
        let storage = InviteBlocklistStorage::new(pool.clone());
        let suffix = unique_suffix();
        let allowed_users = vec![format!("@allowed1{suffix}:localhost"), format!("@allowed2{suffix}:localhost")];
        let (_creator, room_id, users) = seed_room(&pool, suffix, &allowed_users).await;

        // Set allowlist
        storage.set_invite_allowlist(&room_id, allowed_users.clone()).await.expect("Failed to set allowlist");

        // Get allowlist
        let result = storage.get_invite_allowlist(&room_id).await.expect("Failed to get allowlist");
    ...
    fn test_user_id_format_validation() {
        ...
        let valid_users = vec!["@user:localhost", "@user:example.com", "@user:matrix.org", "@alice:hs1.example.org"];
