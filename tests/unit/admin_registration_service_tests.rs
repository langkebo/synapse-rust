...
fn create_service(pool: &Arc<PgPool>, shared_secret: &str, enabled: bool) -> AdminRegistrationService {
...
#[test]
fn test_generate_nonce_returns_nonce() {
...
#[test]
fn test_generate_nonce_unique_values() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_disabled() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_invalid_nonce() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_empty_shared_secret() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_wrong_hmac() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_success() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_non_admin() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_with_user_type() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_with_displayname() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_hmac_mismatch_tampered_username() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_hmac_mismatch_tampered_password() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_hmac_mismatch_admin_flag() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_register_admin_user_duplicate_username() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
