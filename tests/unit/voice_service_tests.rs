...
async fn test_voice_service_new() {
    ...
    let pool = Arc::new(PgPoolOptions::new().max_connections(1).connect_lazy("postgres://localhost/test").unwrap());
    let media_service = synapse_rust::services::media_service::MediaService::new(media_path, None, "test.server");
    ...
    let media_domain_service =
        Arc::new(MediaDomainService::new(media_service, media_quota_service, chunked_upload_service));
