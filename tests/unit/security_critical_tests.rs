...
fn test_device_info_excludes_ip_address() {
    ...
    assert!(device_response.get("last_seen_ip").is_none(), "Device info response must not expose last_seen_ip");
}
...
fn test_device_list_excludes_ip_addresses() {
    ...
    for device in &devices {
        assert!(device.get("last_seen_ip").is_none(), "Each device in list must not expose last_seen_ip");
    }
}
...
fn test_shared_room_filter_allows_self() {
    ...
    let requested_users = vec!["@alice:example.com".to_string(), "@bob:example.com".to_string()];
...
fn test_revoked_token_always_rejected() {
    ...
    let is_revoked = revoked_token.get("is_revoked").and_then(|v| v.as_bool()).unwrap_or(false);
...
fn test_password_change_rejects_all_non_password_auth_types() {
    ...
    for auth_type in invalid_auth_types {
        ...
        assert!(result.is_err(), "Auth type {auth_type} should be rejected for password change");
    }
}
