...
async fn setup_test_database() -> Option<Arc<Pool<Postgres>>> {
    let pool = match synapse_rust::test_utils::prepare_empty_isolated_test_pool().await {
        ...
        Err(error) => {
            eprintln!(
                "Skipping friend room storage tests because test database is unavailable: {error}"
            );
            return None;
        }
    };
...
fn create_storage(pool: &Arc<Pool<Postgres>>) -> FriendRoomStorage {
...
#[test]
fn test_get_friend_list_room_id_found() {
...
#[test]
fn test_get_friend_list_room_id_not_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_friend_list_content() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_friend_list_content_empty() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_friend_info_not_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
fn test_create_friend_request() {
    ...
    rt.block_on(async {
        ...
        let id = storage
            .create_friend_request(&sender_id, &receiver_id, Some("Hello!"))
            .await
            .unwrap();
        assert!(id > 0);

        let req = storage
            .get_friend_request(&sender_id, &receiver_id)
            .await
            .unwrap()
            .unwrap();
...
#[test]
fn test_create_friend_request_upsert() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let id1 = storage
            .create_friend_request(&sender_id, &receiver_id, Some("First"))
            .await
            .unwrap();

        let id2 = storage
            .create_friend_request(&sender_id, &receiver_id, Some("Second"))
            .await
            .unwrap();
...
#[test]
fn test_get_pending_friend_request() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_request(&sender_id, &receiver_id, None)
            .await
            .unwrap();
        ...
        storage
            .update_friend_request_status(&sender_id, &receiver_id, "accepted")
            .await
            .unwrap();
...
#[test]
fn test_get_incoming_friend_requests() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_request(&sender1, &receiver_id, None)
            .await
            .unwrap();
        storage
            .create_friend_request(&sender2, &receiver_id, Some("Hi"))
            .await
            .unwrap();

        let incoming = storage
            .get_incoming_friend_requests(&receiver_id)
            .await
            .unwrap();
...
#[test]
fn test_get_outgoing_friend_requests() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_request(&sender_id, &recv1, None)
            .await
            .unwrap();
        storage
            .create_friend_request(&sender_id, &recv2, None)
            .await
            .unwrap();

        let outgoing = storage
            .get_outgoing_friend_requests(&sender_id)
            .await
            .unwrap();
...
#[test]
fn test_update_friend_request_status() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_update_friend_request_status_already_accepted() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let updated = storage
            .update_friend_request_status(&sender_id, &receiver_id, "rejected")
            .await
            .unwrap();
        assert!(!updated);
    });
...
#[test]
fn test_delete_friend_request_nonexistent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_has_any_pending_request_bidirectional() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_request(&user_a, &user_b, None)
            .await
            .unwrap();
...
#[test]
fn test_ensure_user_exists_present() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_ensure_user_exists_absent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_create_friend_request_with_user_ensure_success() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let id = storage
            .create_friend_request_with_user_ensure(&sender_id, &receiver_id, Some("Hi"))
            .await
            .unwrap();
        assert!(id > 0);
    });
...
#[test]
fn test_create_friend_request_with_user_ensure_sender_missing() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let result = storage
            .create_friend_request_with_user_ensure(&sender_id, &receiver_id, None)
            .await;
        assert!(result.is_err());
    });
}
...
#[test]
fn test_create_and_delete_friend_group() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_group(&room_id, &user_id, "Work")
            .await
            .unwrap();
        ...
        let deleted = storage
            .delete_friend_group(&room_id, &user_id, "Work")
            .await
            .unwrap();
        assert!(deleted);
...
#[test]
fn test_create_friend_group_duplicate() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_group(&room_id, &user_id, "Family")
            .await
            .unwrap();
...
#[test]
fn test_delete_friend_group_nonexistent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let deleted = storage
            .delete_friend_group(&room_id, &user_id, "Nonexistent")
            .await
            .unwrap();
        assert!(!deleted);
    });
...
fn test_rename_friend_group() {
    ...
    rt.block_on(async {
        ...
        storage
            .create_friend_group(&room_id, &user_id, "OldName")
            .await
            .unwrap();

        let renamed = storage
            .rename_friend_group(&room_id, &user_id, "OldName", "NewName")
            .await
            .unwrap();
        assert!(renamed);
...
#[test]
fn test_rename_friend_group_nonexistent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let renamed = storage
            .rename_friend_group(&room_id, &user_id, "Ghost", "NewGhost")
            .await
            .unwrap();
        assert!(!renamed);
    });
...
#[test]
fn test_add_and_remove_friend_from_group() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_group(&room_id, &user_id, "Close")
            .await
            .unwrap();

        let added = storage
            .add_friend_to_group(&room_id, &user_id, "Close", &friend_id)
            .await
            .unwrap();
        assert!(added);

        let groups_for = storage
            .get_friend_groups_for_user(&room_id, &friend_id)
            .await
            .unwrap();
        assert_eq!(groups_for, vec!["Close"]);

        let removed = storage
            .remove_friend_from_group(&room_id, &user_id, "Close", &friend_id)
            .await
            .unwrap();
        assert!(removed);
...
#[test]
fn test_add_friend_to_group_duplicate() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_group(&room_id, &user_id, "Test")
            .await
            .unwrap();

        storage
            .add_friend_to_group(&room_id, &user_id, "Test", &friend_id)
            .await
            .unwrap();
...
#[test]
fn test_add_friend_to_group_nonexistent_group() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let added = storage
            .add_friend_to_group(&room_id, &user_id, "Ghost", &friend_id)
            .await
            .unwrap();
        assert!(!added);
    });
...
#[test]
fn test_remove_friend_from_group_not_member() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        let removed = storage
            .remove_friend_from_group(&room_id, &user_id, "Test", &friend_id)
            .await
            .unwrap();
        assert!(!removed);
    });
...
#[test]
fn test_get_friend_groups_for_user_multiple_groups() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
        ...
        storage
            .create_friend_group(&room_id, &user_id, "GroupA")
            .await
            .unwrap();
        tokio::time::sleep(std::time::Duration::from_millis(2)).await;
        storage
            .create_friend_group(&room_id, &user_id, "GroupB")
            .await
            .unwrap();
        tokio::time::sleep(std::time::Duration::from_millis(2)).await;

        storage
            .add_friend_to_group(&room_id, &user_id, "GroupA", &friend_id)
            .await
            .unwrap();
        tokio::time::sleep(std::time::Duration::from_millis(2)).await;
        storage
            .add_friend_to_group(&room_id, &user_id, "GroupB", &friend_id)
            .await
            .unwrap();
...
#[test]
fn test_get_friend_requests_empty() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_user_friend_ids_no_room() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_find_friend_lists_by_dm_room_id() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
