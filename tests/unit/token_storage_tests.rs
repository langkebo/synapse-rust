...
#[test]
fn test_create_token_with_device() {
...
#[test]
fn test_create_token_without_device() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_token_after_create() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_token_excludes_revoked() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_user_tokens_returns_empty_for_unknown_user() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_delete_token_is_soft_delete() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_delete_user_tokens_skips_already_revoked() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_delete_user_device_tokens() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_delete_user_tokens_except_device() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_token_exists_false_for_missing() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_token_exists_false_for_revoked() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_is_token_revoked_false_for_active() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_is_token_revoked_false_for_missing() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_add_to_blacklist_and_is_in_blacklist() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_add_to_blacklist_without_reason() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_is_in_blacklist_false_for_missing() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_cleanup_expired_blacklist_entries() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_cleanup_expired_blacklist_keeps_valid_entries() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_cleanup_expired_tokens_keeps_no_expiry() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_delete_user_tokens_does_not_affect_other_users() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_delete_user_tokens_except_device_with_no_other_devices() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_multiple_tokens_same_device() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
