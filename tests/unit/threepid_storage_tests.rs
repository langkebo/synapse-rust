...
fn create_storage(pool: &Arc<Pool<Postgres>>) -> ThreepidStorage {
...
#[test]
fn test_user_threepid_struct_fields() {
...
#[test]
fn test_user_threepid_struct_unverified() {
    let threepid = UserThreepid {
        id: 2,
...
#[test]
fn test_create_threepid_request_fields() {
...
#[test]
fn test_create_threepid_request_minimal() {
    let request = CreateThreepidRequest {
...
#[test]
fn test_threepid_validation_session_struct() {
...
#[test]
fn test_add_threepid_without_verification() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_add_threepid_duplicate_medium_address() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_threepid_not_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_threepids_by_user_empty() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_threepid_by_address_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_threepid_by_address_not_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_verified_threepid_by_address_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_verified_threepid_by_address_unverified() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_verify_threepid_success() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_verify_threepid_nonexistent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_verify_threepid_by_token_success() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_verify_threepid_by_token_expired() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_verify_threepid_by_token_invalid() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_remove_threepid_success() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_remove_threepid_nonexistent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_remove_threepids_by_user() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_remove_threepids_by_user_no_threepids() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_cleanup_expired_verifications_none() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_create_validation_session() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_create_validation_session_without_next_link() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_create_validation_session_duplicate_session_id() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_validation_session() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_validation_session_not_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_validation_session_already_validated() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_validation_session_expired() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_validation_session_by_token() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_get_validation_session_by_token_not_found() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_mark_validation_validated() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_increment_validation_send_attempt() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_cleanup_expired_validation_sessions() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_full_threepid_lifecycle() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
...
#[test]
fn test_full_validation_session_lifecycle() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let pool = match setup_test_database().await {
            Some(pool) => pool,
