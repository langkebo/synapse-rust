...
async fn setup_test_database() -> Option<(Arc<Pool<Postgres>>, PresenceStorage)> {
...
async fn insert_test_user(pool: &Pool<Postgres>, user_id: &str) {
...
#[test]
fn test_get_presence_nonexistent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
...
#[test]
fn test_set_presence_without_status_msg() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_presence_upsert_updates_existing() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_get_presence_with_meta() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_get_presence_with_meta_nonexistent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (_pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_get_presences_empty_input() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (_pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_add_subscription_idempotent() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_set_typing_start_and_stop() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_get_presence_batch_empty_input() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (_pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_get_presence_snapshots() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
...
#[test]
fn test_get_presence_snapshots_empty_input() {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
        let (_pool, storage) = match setup_test_database().await {
            Some(tuple) => tuple,
