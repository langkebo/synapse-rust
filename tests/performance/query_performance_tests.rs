...
mod batch_query_correctness_tests {
    ...
    fn test_batch_query_handles_duplicates() {
        let ids = vec!["id_1".to_string(), "id_2".to_string(), "id_1".to_string()];
    ...
    #[test]
    fn test_n_plus_one_query_returns_all_requested_items() {
        let ids: Vec<String> = (0..5).map(|i| format!("id_{}", i)).collect();

        let result = simulate_n_plus_one_query(&ids, |id| format!("value_for_{}", id));

        assert_eq!(result.len(), ids.len());
        for id in &ids {
            assert_eq!(result.get(id), Some(&format!("value_for_{}", id)));
        }
    }
