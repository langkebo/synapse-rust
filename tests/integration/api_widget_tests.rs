...
async fn create_widget(app: &axum::Router, token: &str, room_id: &str) -> String {
    ...
    let response = ServiceExt::<Request<Body>>::oneshot(app.clone(), create_widget_request).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
...
async fn test_create_widget_succeeds_for_existing_room() {
    ...
    let create_widget_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), create_widget_request).await.unwrap();
...
async fn test_create_widget_forbidden_for_non_member() {
    ...
    let outsider_token = register_user(&app, &format!("widget_outsider_{}", rand::random::<u32>())).await;
    ...
    let response = ServiceExt::<Request<Body>>::oneshot(app, create_widget_request).await.unwrap();
...
async fn test_create_widget_rejects_joined_non_creator() {
    ...
    let (owner_token, _) = register_user_with_id(&app, &format!("widget_create_owner_{}", rand::random::<u32>())).await;
    let (member_token, member_user_id) =
        register_user_with_id(&app, &format!("widget_create_member_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &owner_token).await;
...
async fn test_create_widget_returns_not_found_for_missing_room() {
    ...
    let create_widget_response = ServiceExt::<Request<Body>>::oneshot(app, create_widget_request).await.unwrap();
...
async fn test_get_widget_forbidden_for_non_member() {
    ...
    let owner_token = register_user(&app, &format!("widget_owner_{}", rand::random::<u32>())).await;
    let viewer_token = register_user(&app, &format!("widget_viewer_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &owner_token).await;
...
async fn test_joined_room_member_can_read_and_create_session_but_cannot_modify_widget() {
    ...
    let (owner_token, _) = register_user_with_id(&app, &format!("widget_owner_manage_{}", rand::random::<u32>())).await;
    let (member_token, member_user_id) =
        register_user_with_id(&app, &format!("widget_member_manage_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &owner_token).await;
    let widget_id = create_widget(&app, &owner_token, &room_id).await;
    ...
    let session_request = Request::builder()
        ...
        .unwrap();
    let session_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), session_request).await.unwrap();
    assert_eq!(session_response.status(), StatusCode::OK);
    let session_json = read_json(session_response).await;
    let session_id = session_json["session"]["session_id"].as_str().unwrap().to_string();
    ...
    let get_session_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let get_session_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), get_session_request).await.unwrap();
    ...
    let permission_request = Request::builder()
        ...
        .unwrap();
    let permission_response = ServiceExt::<Request<Body>>::oneshot(app, permission_request).await.unwrap();
...
async fn test_get_room_widget_capabilities_forbidden_for_non_member() {
    ...
    let owner_token = register_user(&app, &format!("widget_caps_owner_{}", rand::random::<u32>())).await;
    let outsider_token = register_user(&app, &format!("widget_caps_outsider_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &owner_token).await;
    let widget_id = create_widget(&app, &owner_token, &room_id).await;
...
async fn test_get_jitsi_config_forbidden_for_non_member() {
    ...
    let owner_token = register_user(&app, &format!("widget_jitsi_owner_{}", rand::random::<u32>())).await;
    let outsider_token = register_user(&app, &format!("widget_jitsi_outsider_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &owner_token).await;
...
async fn test_room_widget_routes_forbidden_for_admin_without_room_access() {
    ...
    let owner_token = register_user(&app, &format!("widget_caps_owner_admin_{}", rand::random::<u32>())).await;
    let (admin_token, _) = super::get_admin_token(&app).await;
    let room_id = create_room(&app, &owner_token).await;
    let widget_id = create_widget(&app, &owner_token, &room_id).await;
...
async fn test_get_room_widget_capabilities_rejects_widget_room_mismatch() {
    ...
    let token = register_user(&app, &format!("widget_caps_mismatch_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &token).await;
...
async fn test_set_room_widget_capabilities_rejects_joined_non_creator() {
    ...
    let (owner_token, _) =
        register_user_with_id(&app, &format!("widget_caps_owner_write_{}", rand::random::<u32>())).await;
    let (member_token, member_user_id) =
        register_user_with_id(&app, &format!("widget_caps_member_write_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &owner_token).await;
    let widget_id = create_widget(&app, &owner_token, &room_id).await;
...
async fn test_get_widget_session_forbidden_for_unrelated_user() {
    ...
    let owner_token = register_user(&app, &format!("widget_session_owner_{}", rand::random::<u32>())).await;
    let outsider_token = register_user(&app, &format!("widget_session_outsider_{}", rand::random::<u32>())).await;
    let room_id = create_room(&app, &owner_token).await;
    let widget_id = create_widget(&app, &owner_token, &room_id).await;
...
async fn test_get_widget_session_forbidden_for_admin_without_room_access() {
    ...
    let owner_token = register_user(&app, &format!("widget_session_owner_admin_{}", rand::random::<u32>())).await;
    let (admin_token, _) = super::get_admin_token(&app).await;
    let room_id = create_room(&app, &owner_token).await;
    let widget_id = create_widget(&app, &owner_token, &room_id).await;
    let session_id = create_widget_session(&app, &owner_token, &widget_id).await;
