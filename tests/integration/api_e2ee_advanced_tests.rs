...
async fn test_e2ee_key_backup_lifecycle() {
    ...
    assert_eq!(response.status(), StatusCode::OK, "Key backup creation should return 200 OK");
    ...
    assert_eq!(response.status(), StatusCode::OK, "List backups should return 200 OK");

    let body = axum::body::to_bytes(response.into_body(), 1024 * 1024).await.unwrap();
    let list_json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(list_json[&backup_id]["backup_id"], backup_id);
    assert_eq!(list_json[&backup_id]["algorithm"], "m.megolm_backup.v1.secure");

    // 3. 查询备份信息
    let get_backup_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v3/keys/backup/secure/{}", backup_id))
        .header("Authorization", format!("Bearer {}", user_token))
        .body(Body::empty())
        .unwrap();

    let response = app.clone().oneshot(get_backup_request).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK, "Get backup info should return 200 OK");
    ...
    let store_keys_request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/client/v3/keys/backup/secure/{}/keys", backup_id))
        .header("Authorization", format!("Bearer {}", user_token))
        .header("Content-Type", "application/json")
        .body(Body::from(
            json!({
    ...
    assert_eq!(response.status(), StatusCode::OK, "Restore backup should return 200 OK");
    ...
    assert_eq!(response.status(), StatusCode::OK, "Delete backup should return 200 OK");
    ...
    assert_eq!(response.status(), StatusCode::NOT_FOUND, "Deleted backup should return 404");
}
...
async fn test_e2ee_cross_signing_flow() {
    ...
    let response = app.clone().oneshot(upload_device_keys_request).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    ...
    let response = app.clone().oneshot(upload_signatures_request).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK, "Upload signatures should return 200 OK");
    ...
    assert_eq!(response.status(), StatusCode::OK, "Query keys should return 200 OK");

    let body = axum::body::to_bytes(response.into_body(), 1024 * 1024).await.unwrap();
    let query_json: serde_json::Value = serde_json::from_slice(&body).unwrap();

    // 验证返回的密钥包含交叉签名信息
    assert!(query_json["device_keys"].is_object(), "Should return device keys");
...
async fn test_e2ee_key_backup_error_handling() {
    ...
    let response = app.clone().oneshot(nonexistent_backup_request).await.unwrap();
    assert_eq!(response.status(), StatusCode::NOT_FOUND, "Nonexistent backup should return 404");
}
