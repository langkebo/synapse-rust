...
async fn test_space_children_hierarchy_suite_keeps_nested_chain_verified() {
    ...
    let children_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let children_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), children_request).await.unwrap();
    assert_eq!(children_response.status(), StatusCode::OK);

    let children_body = axum::body::to_bytes(children_response.into_body(), 8192).await.unwrap();
    ...
    let hierarchy_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v1/spaces/{}/hierarchy/v1?max_depth=3&limit=1", root_space_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let hierarchy_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), hierarchy_request).await.unwrap();
    assert_eq!(hierarchy_response.status(), StatusCode::OK);

    let hierarchy_body = axum::body::to_bytes(hierarchy_response.into_body(), 16384).await.unwrap();
    ...
    assert_eq!(hierarchy_json["rooms"][0]["children_state"][0]["state_key"], leaf_room_id);
    ...
    let parents_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let parents_response = ServiceExt::<Request<Body>>::oneshot(app, parents_request).await.unwrap();
    assert_eq!(parents_response.status(), StatusCode::OK);

    let parents_body = axum::body::to_bytes(parents_response.into_body(), 8192).await.unwrap();
...
async fn test_space_membership_state_suite_keeps_invite_join_leave_closure_verified() {
    ...
    assert!(state_entries
        .iter()
        .any(|entry| { entry["type"] == "m.room.create" && entry["content"]["room_type"] == "m.space" }));
    assert!(state_entries
        .iter()
    ...
    let rooms_body = axum::body::to_bytes(rooms_response.into_body(), 8192).await.unwrap();
    ...
    let forbidden_join_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), forbidden_join_request).await.unwrap();
    ...
    let joined_members_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), joined_members_request).await.unwrap();
    ...
    let joined_members = joined_members_json["members"].as_array().unwrap_or_else(|| joined_members_json.as_array().unwrap());
    ...
    let members_after_leave_response =
        ServiceExt::<Request<Body>>::oneshot(app, members_after_leave_request).await.unwrap();
    ...
    let members_after_leave = members_after_leave_json["members"].as_array().unwrap_or_else(|| members_after_leave_json.as_array().unwrap());
...
async fn test_space_lifecycle_query_suite_keeps_create_update_lookup_and_delete_verified() {
    ...
    let get_body = axum::body::to_bytes(get_response.into_body(), 8192).await.unwrap();
    ...
    let by_room_body = axum::body::to_bytes(by_room_response.into_body(), 8192).await.unwrap();
    ...
    let update_body = axum::body::to_bytes(update_response.into_body(), 8192).await.unwrap();
    ...
    let user_spaces_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let user_spaces_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), user_spaces_request).await.unwrap();
    assert_eq!(user_spaces_response.status(), StatusCode::OK);

    let user_spaces_body = axum::body::to_bytes(user_spaces_response.into_body(), 8192).await.unwrap();
    ...
    let public_spaces = public_spaces_json["spaces"].as_array().unwrap_or_else(|| public_spaces_json.as_array().unwrap());
    ...
    let search_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let search_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), search_request).await.unwrap();
    assert_eq!(search_response.status(), StatusCode::OK);

    let search_body = axum::body::to_bytes(search_response.into_body(), 8192).await.unwrap();
...
async fn test_space_statistics_only_returns_visible_spaces() {
    ...
    let public_space_id: String = sqlx::query_scalar("SELECT space_id FROM spaces WHERE room_id = $1")
        ...
        .await
        .unwrap();
    let private_space_id: String = sqlx::query_scalar("SELECT space_id FROM spaces WHERE room_id = $1")
    ...
    let anonymous_body = axum::body::to_bytes(anonymous_response.into_body(), 8192).await.unwrap();
    let anonymous_json: Value = serde_json::from_slice(&anonymous_body).unwrap();
    let anonymous_stats = anonymous_json.as_array().unwrap();
    assert!(anonymous_stats.iter().any(|entry| entry["name"] == "Stats Public Space"));
    assert!(!anonymous_stats.iter().any(|entry| entry["name"] == "Stats Private Space"));
    ...
    let outsider_body = axum::body::to_bytes(outsider_response.into_body(), 8192).await.unwrap();
    let outsider_json: Value = serde_json::from_slice(&outsider_body).unwrap();
    let outsider_stats = outsider_json.as_array().unwrap();
    assert!(outsider_stats.iter().any(|entry| entry["name"] == "Stats Public Space"));
    assert!(!outsider_stats.iter().any(|entry| entry["name"] == "Stats Private Space"));
    ...
    let owner_body = axum::body::to_bytes(owner_response.into_body(), 8192).await.unwrap();
    let owner_json: Value = serde_json::from_slice(&owner_body).unwrap();
    let owner_stats = owner_json.as_array().unwrap();
    assert!(owner_stats.iter().any(|entry| entry["name"] == "Stats Public Space"));
    assert!(owner_stats.iter().any(|entry| entry["name"] == "Stats Private Space"));
}
...
async fn test_space_shared_write_routes_reject_joined_non_creator() {
    ...
    let remove_child_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let remove_child_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), remove_child_request).await.unwrap();
    ...
    let owner_update_request = Request::builder()
        ...
        .unwrap();
    let owner_update_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), owner_update_request).await.unwrap();
