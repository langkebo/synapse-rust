...
#[tokio::test]
async fn test_encode_cursor_round_trip() {
    let cursor =
...
#[tokio::test]
async fn test_encode_cursor_different_values() {
...
async fn test_get_blacklist_entry_found() {
    ...
    insert_blacklist_entry(&pool, &server_name, "blacklist", Some("Bad actor"), "@admin:example.com", true, None).await;
...
async fn test_is_server_whitelisted_false_blacklist_type() {
    ...
    insert_blacklist_entry(&pool, &server_name, "blacklist", Some("Blocked"), "@admin:example.com", true, None).await;
...
async fn test_update_access_stats_first_failure() {
    ...
    let request = UpdateStatsRequest { server_name: server_name.clone(), is_success: false, response_time_ms: None };
