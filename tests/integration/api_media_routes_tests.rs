...
fn tiny_png() -> Vec<u8> {
    ...
    dynamic.write_to(&mut std::io::Cursor::new(&mut output), ImageFormat::Png).expect("encode test png");
    output
}
...
async fn test_media_routes_share_content_across_versions() {
    ...
    let token = register_user(&app, &format!("media_routes_shared_{}", rand::random::<u32>())).await;
    ...
    let v3_download_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), v3_download_request).await.unwrap();
    assert_eq!(v3_download_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(v3_download_response.into_body(), 2048).await.unwrap();
    ...
    let r1_download_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), r1_download_request).await.unwrap();
...
async fn test_media_preview_and_delete_boundaries() {
    ...
    let delete_request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/media/v3/delete/{}/{}", server_name, media_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    ...
    let missing_thumbnail_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/media/r0/thumbnail/{}/{}?width=64&height=64", server_name, media_id))
        .body(Body::empty())
        .unwrap();
    let missing_thumbnail_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), missing_thumbnail_request).await.unwrap();
    ...
    let download_after_delete_response =
        ServiceExt::<Request<Body>>::oneshot(app, download_after_delete_request).await.unwrap();
    assert_eq!(download_after_delete_response.status(), StatusCode::NOT_FOUND);
}
...
async fn test_client_v1_authenticated_media_download_requires_access_token() {
    ...
    let token = register_user(&app, &format!("media_auth_download_{}", rand::random::<u32>())).await;
    ...
    let anonymous_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v1/media/download/{}/{}", server_name, media_id))
        .body(Body::empty())
        .unwrap();
    ...
    let authed_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let authed_response = ServiceExt::<Request<Body>>::oneshot(app, authed_request).await.unwrap();
    assert_eq!(authed_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(authed_response.into_body(), 2048).await.unwrap();
...
async fn test_client_v1_authenticated_media_thumbnail_and_preview_routes_work() {
    ...
    let thumbnail_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v1/media/thumbnail/{}/{}?width=32&height=32&method=scale", server_name, media_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let thumbnail_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), thumbnail_request).await.unwrap();
    assert_eq!(thumbnail_response.status(), StatusCode::OK);
    assert_eq!(thumbnail_response.headers().get("content-type").unwrap(), "image/jpeg");
    ...
    let anonymous_thumbnail_response =
        ServiceExt::<Request<Body>>::oneshot(app, anonymous_thumbnail_request).await.unwrap();
...
async fn test_media_delete_forbids_admin_override() {
    ...
    let admin_delete_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), admin_delete_request).await.unwrap();
    ...
    let owner_download_response = ServiceExt::<Request<Body>>::oneshot(app, owner_download_request).await.unwrap();
...
async fn test_media_download_is_public_but_delete_requires_authentication() {
    ...
    let token = register_user(&app, &format!("media_routes_public_read_{}", rand::random::<u32>())).await;
    ...
    let anonymous_download_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), anonymous_download_request).await.unwrap();
    assert_eq!(anonymous_download_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(anonymous_download_response.into_body(), 2048).await.unwrap();
    ...
    let anonymous_delete_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let anonymous_delete_response = ServiceExt::<Request<Body>>::oneshot(app, anonymous_delete_request).await.unwrap();
...
async fn test_media_upload_and_route_boundaries_are_preserved() {
    ...
    let r0_upload_request = Request::builder()
        ...
        .unwrap();
    let r0_upload_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_upload_request).await.unwrap();
    assert_eq!(r0_upload_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(r0_upload_response.into_body(), 2048).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    let (server_name, media_id) = parse_mxc_uri(json["content_uri"].as_str().unwrap());
    ...
    let v3_put_upload_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/media/v3/upload/{}/{}?filename=modern.txt", server_name, named_media_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "text/plain")
        .body(Body::from("modern-upload"))
        .unwrap();
    let v3_put_upload_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), v3_put_upload_request).await.unwrap();
    assert_eq!(v3_put_upload_response.status(), StatusCode::OK);

    let v1_delete_request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/media/v1/delete/{}/{}", server_name, media_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let v1_delete_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), v1_delete_request).await.unwrap();
    ...
    let r1_preview_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let r1_preview_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), r1_preview_request).await.unwrap();
    assert_eq!(r1_preview_response.status(), StatusCode::NOT_FOUND);

    let r0_delete_request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/media/r0/delete/{}/{}", server_name, media_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let r0_delete_response = ServiceExt::<Request<Body>>::oneshot(app, r0_delete_request).await.unwrap();
...
async fn test_named_media_upload_uses_provided_media_id() {
    ...
    let token = register_user(&app, &format!("media_routes_named_{}", rand::random::<u32>())).await;
    ...
    let bootstrap_upload_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), bootstrap_upload_request).await.unwrap();
    assert_eq!(bootstrap_upload_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(bootstrap_upload_response.into_body(), 2048).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    ...
    let named_upload_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/media/v3/upload/{}/{}?filename=named.txt", server_name, named_media_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "text/plain")
        .body(Body::from("named-upload"))
        .unwrap();
    let named_upload_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), named_upload_request).await.unwrap();
    assert_eq!(named_upload_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(named_upload_response.into_body(), 2048).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["content_uri"].as_str().unwrap(), format!("mxc://{}/{}", server_name, named_media_id));
    assert_eq!(json["media_id"].as_str().unwrap(), named_media_id);

    let download_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/media/v3/download/{}/{}", server_name, named_media_id))
        .body(Body::empty())
        .unwrap();
    let download_response = ServiceExt::<Request<Body>>::oneshot(app, download_request).await.unwrap();
    assert_eq!(download_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(download_response.into_body(), 2048).await.unwrap();
...
async fn test_named_media_upload_rejects_duplicate_media_id() {
    ...
    let token = register_user(&app, &format!("media_routes_duplicate_{}", rand::random::<u32>())).await;
    ...
    let first_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/media/v3/upload/{}/{}?filename=first.txt", server_name, media_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "text/plain")
    ...
    let duplicate_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/media/v3/upload/{}/{}?filename=second.txt", server_name, media_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "text/plain")
        .body(Body::from("second"))
        .unwrap();
    let duplicate_response = ServiceExt::<Request<Body>>::oneshot(app, duplicate_request).await.unwrap();
...
async fn test_named_media_upload_rejects_non_local_server_name() {
    ...
    let token = register_user(&app, &format!("media_routes_server_name_{}", rand::random::<u32>())).await;
    let media_id = format!("fixed_media_{}", rand::random::<u32>());

    let request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/media/v3/upload/{}/{}?filename=bad.txt", "remote.example.com", media_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "text/plain")
...
async fn test_media_download_rejects_foreign_server_namespace() {
    ...
    let token = register_user(&app, &format!("media_routes_foreign_download_{}", rand::random::<u32>())).await;
    ...
    let download_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/media/v3/download/{}/{}", "remote.example.com", media_id))
        .body(Body::empty())
        .unwrap();
    let download_response = ServiceExt::<Request<Body>>::oneshot(app, download_request).await.unwrap();
...
async fn test_media_delete_rejects_foreign_server_namespace() {
    ...
    let token = register_user(&app, &format!("media_routes_foreign_delete_{}", rand::random::<u32>())).await;
    ...
    let delete_request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/media/v3/delete/{}/{}", "remote.example.com", media_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let delete_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), delete_request).await.unwrap();
    ...
    let local_download_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let local_download_response = ServiceExt::<Request<Body>>::oneshot(app, local_download_request).await.unwrap();
...
async fn test_media_thumbnail_rejects_foreign_server_namespace() {
    ...
    let token = register_user(&app, &format!("media_routes_foreign_thumbnail_{}", rand::random::<u32>())).await;
    ...
    let thumbnail_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/media/v3/thumbnail/{}/{}?width=64&height=64", "remote.example.com", media_id))
        .body(Body::empty())
        .unwrap();
    let thumbnail_response = ServiceExt::<Request<Body>>::oneshot(app, thumbnail_request).await.unwrap();
...
async fn test_legacy_media_download_missing_returns_not_found_status() {
    ...
    let v1_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let v1_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), v1_request).await.unwrap();
    assert_eq!(v1_response.status(), StatusCode::NOT_FOUND);

    let body = axum::body::to_bytes(v1_response.into_body(), 2048).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
