...
async fn create_room_with_initial_state(
    ...
    let request = Request::builder()
        ...
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "name": name, "initial_state": initial_state }).to_string()))
        .unwrap();
...
async fn test_power_levels_contract_rejects_non_members() {
    ...
    assert_matrix_error(
        ...
        Request::builder()
            .method("GET")
            .uri(format!("/_matrix/client/v1/rooms/{}/state/m.room.power_levels/", encoded_room_id))
...
async fn test_search_rooms_contract_hides_private_rooms_from_outsiders() {
    ...
    assert!(
        !results.iter().any(|room| room["room_id"].as_str() == Some(room_id.as_str())),
...
async fn test_get_state_event_empty_key_returns_raw_content_only() {
    ...
    put_state_event_empty_key(&app, &alice_token, &room_id, "m.room.topic", json!({ "topic": "raw topic payload" }))
        .await;
...
async fn test_scanner_info_contract_is_not_empty_success() {
    ...
    assert!(!json["message"].as_str().unwrap_or("").trim().is_empty(), "message should not be empty");
}
...
async fn test_room_account_data_write_ack_persists_value() {
    ...
    let response = ServiceExt::<Request<Body>>::oneshot(
        ...
        Request::builder()
            .method("PUT")
            .uri(format!("/_matrix/client/v3/rooms/{}/account_data/{}", encoded_room_id, data_type))
            .header("Authorization", format!("Bearer {}", token))
            .header("Content-Type", "application/json")
...
#[tokio::test]
async fn test_burn_after_read_contract_rejects_non_members() {
    let Some(app) = super::setup_fresh_test_app().await else {
        return;
    };

    let owner = format!("burn_owner_{}", rand::random::<u32>());
    let outsider = format!("burn_outsider_{}", rand::random::<u32>());
    let (owner_token, _) = register_user(&app, &owner).await;
    let (outsider_token, _) = register_user(&app, &outsider).await;

    let room_id = create_room(&app, &owner_token, "Burn Membership Contract").await;
    let encoded_room_id = encode_room_id(&room_id);
