...
async fn setup_federation_ingress_app(server_name: &str, key_id: &str, signing_key_b64: &str) -> Option<axum::Router> {
    let (app, _) = setup_federation_ingress_app_with(server_name, key_id, signing_key_b64, |_| {}).await?;
    Some(app)
}
...
fn signed_request(
    ...
    let mut builder = Request::builder()
        .method(method)
        .uri(uri)
        .header("Authorization", format!("X-Matrix origin=\"{}\",key=\"{}\",sig=\"{}\"", server_name, key_id, sig_b64));
...
async fn create_shared_room_for_users(state: &AppState, room_id: &str, creator_user_id: &str, other_user_id: &str) {
    state.services.room_storage.create_room(room_id, creator_user_id, "public", "10", false).await.unwrap();
...
fn tiny_png() -> Vec<u8> {
    vec![
        ...
        0x0C, 0x49, 0x44, 0x41, 0x54, 0x08, 0xD7, 0x63, 0xF8, 0xCF, 0xC0, 0x00, 0x00, 0x03, 0x01, 0x01, 0x00, 0xC9,
...
async fn test_federation_destination_accepts_configured_server_name_alias() {
    ...
    let uri = format!("/_matrix/federation/v1/exchange_third_party_invite/!testroom:{}", public_server_name);
...
async fn test_federation_protected_route_rejects_missing_signature() {
    ...
    let Some(app) = setup_federation_ingress_app(server_name, key_id, &signing_key_b64).await else {
        return;
    };

    let uri = format!("/_matrix/federation/v1/exchange_third_party_invite/!testroom:{}", server_name);
...
async fn test_federation_protected_route_rejects_invalid_signature() {
    ...
    let request = Request::builder()
        ...
        .header(
            "Authorization",
            format!("X-Matrix origin=\"{}\",key=\"{}\",sig=\"{}\"", server_name, key_id, "invalid"),
        )
        .header("Content-Type", "application/json")
        .body(Body::from(body))
        .unwrap();
...
async fn test_federation_protected_route_allows_valid_signature_and_reaches_handler() {
    ...
    let uri = format!("/_matrix/federation/v1/exchange_third_party_invite/!testroom:{}", server_name);
    let content = json!({ "invite": { "display_name": "test" } });
    let request = signed_request("PUT", &uri, server_name, key_id, &signing_key, Some(&content));
...
async fn test_federation_exchange_third_party_invite_rejects_sender_domain_mismatch() {
    ...
    state.services.user_storage.create_user(&creator, "creator", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &creator, "public", "1", true).await.unwrap();
    ...
    let response =
        app.oneshot(signed_request("PUT", &uri, server_name, key_id, &signing_key, Some(&body))).await.unwrap();
...
async fn test_federation_thirdparty_invite_rejects_sender_domain_mismatch() {
    ...
    let response =
        app.oneshot(signed_request("POST", uri, server_name, key_id, &signing_key, Some(&body))).await.unwrap();
...
async fn test_federation_make_join_rejects_user_domain_mismatch() {
    ...
    let uri = format!("/_matrix/federation/v1/make_join/!joincheck:{}/@mallory:evil.example", server_name);
...
async fn test_federation_make_leave_rejects_user_domain_mismatch() {
    ...
    let uri = format!("/_matrix/federation/v1/make_leave/!leavecheck:{}/@mallory:evil.example", server_name);
    let response = app.oneshot(signed_request("GET", &uri, server_name, key_id, &signing_key, None)).await.unwrap();
...
async fn test_federation_get_state_rejects_server_without_joined_member() {
    ...
    state.services.user_storage.create_user(&creator, "creator", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &creator, "public", "1", true).await.unwrap();
    state.services.member_storage.add_member(&room_id, &creator, "join", None, None, None, None).await.unwrap();
...
async fn test_federation_state_and_backfill_endpoints_return_spec_shaped_minimal_payloads() {
    ...
    state.services.user_storage.create_user(&creator, "creator", None, false).await.unwrap();
    state.services.user_storage.create_user(&remote_member, "member", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &creator, "invite", "1", false).await.unwrap();
    state.services.member_storage.add_member(&room_id, &creator, "join", None, None, None, None).await.unwrap();
    state.services.member_storage.add_member(&room_id, &remote_member, "join", None, None, None, None).await.unwrap();

    for (event_id, event_type, content, ts) in [
        ("$power-shape", "m.room.power_levels", json!({ "users_default": 0 }), 200_i64),
        ("$a-name-shape", "m.room.name", json!({ "name": "Alpha" }), 100_i64),
        ("$z-topic-shape", "m.room.topic", json!({ "topic": "Zulu" }), 100_i64),
        ("$avatar-latest-shape", "m.room.avatar", json!({ "url": "mxc://test.example.com/latest-avatar" }), 400_i64),
    ] {
        state
            .services
            .event_storage
    ...
    let get_state_body = axum::body::to_bytes(get_state_response.into_body(), usize::MAX).await.unwrap();
    ...
    let get_state_at_event_body =
        axum::body::to_bytes(get_state_at_event_response.into_body(), usize::MAX).await.unwrap();
    ...
    let get_state_ids_body = axum::body::to_bytes(get_state_ids_response.into_body(), usize::MAX).await.unwrap();
    ...
    let get_state_ids_at_event_response = app
        ...
        .oneshot(signed_request(
            "GET",
            &format!("/_matrix/federation/v1/state_ids/{}?event_id=$power-shape", room_id),
            server_name,
            key_id,
            &signing_key,
            None,
    ...
    let get_state_ids_at_event_body =
        axum::body::to_bytes(get_state_ids_at_event_response.into_body(), usize::MAX).await.unwrap();
    let get_state_ids_at_event_json: Value = serde_json::from_slice(&get_state_ids_at_event_body).unwrap();
    ...
    let get_event_auth_response = app
        ...
        .oneshot(signed_request(
            "GET",
            &format!("/_matrix/federation/v1/get_event_auth/{}/{}", room_id, "$power-shape"),
            server_name,
            key_id,
            &signing_key,
            None,
    ...
    let get_event_auth_body = axum::body::to_bytes(get_event_auth_response.into_body(), usize::MAX).await.unwrap();
    ...
    let backfill_response = app
        .oneshot(signed_request(
            "GET",
            &format!("/_matrix/federation/v1/backfill/{}?v=$seed-shape&limit=2", room_id),
            server_name,
            key_id,
            &signing_key,
            None,
    ...
    let backfill_body = axum::body::to_bytes(backfill_response.into_body(), usize::MAX).await.unwrap();
    ...
    assert!(backfill_pdus.iter().all(|event| event["origin_server_ts"].as_i64().unwrap_or(i64::MAX) < 250));
    assert!(backfill_pdus.iter().all(|event| event["event_id"] != "$seed-shape"));
    assert!(backfill_pdus.iter().all(|event| event["event_id"] != "$a-message-shape"));
    assert!(backfill_pdus.iter().all(|event| event["event_id"] != "$b-message-shape"));
    assert!(backfill_pdus.iter().all(|event| event["event_id"] != "$avatar-latest-shape"));
...
async fn test_federation_state_endpoints_reject_event_ids_from_other_rooms() {
    ...
    for user_id in [&creator, &remote_member] {
        state.services.user_storage.create_user(user_id, user_id, None, false).await.unwrap();
    }

    state.services.room_storage.create_room(&target_room_id, &creator, "invite", "1", false).await.unwrap();
    state.services.member_storage.add_member(&target_room_id, &creator, "join", None, None, None, None).await.unwrap();
    state
        .services
        .member_storage
    ...
    state.services.room_storage.create_room(&other_room_id, &creator, "invite", "1", false).await.unwrap();
    state.services.member_storage.add_member(&other_room_id, &creator, "join", None, None, None, None).await.unwrap();
    ...
    for uri in [
        format!("/_matrix/federation/v1/state/{}?event_id=$other-room-event", target_room_id),
        format!("/_matrix/federation/v1/state_ids/{}?event_id=$other-room-event", target_room_id),
    ] {
        let response =
            app.clone().oneshot(signed_request("GET", &uri, server_name, key_id, &signing_key, None)).await.unwrap();

        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
        let body = axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let json: Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(json["errcode"], "M_BAD_JSON");
        assert!(json["error"].as_str().unwrap_or_default().contains("Event does not belong to this room"));
    }

    let backfill_response = app
        .oneshot(signed_request(
            "GET",
            &format!("/_matrix/federation/v1/backfill/{}?v=$other-room-event&limit=10", target_room_id),
            server_name,
            key_id,
            &signing_key,
            None,
...
async fn test_federation_get_user_devices_omits_sensitive_metadata() {
    ...
    state.services.user_storage.create_user(&user_id, "alice", None, false).await.unwrap();
    state.services.device_storage.create_device("ALICEDEVICE", &user_id, Some("Alice device")).await.unwrap();
    state.services.room_storage.create_room(&room_id, &user_id, "private", "1", false).await.unwrap();
    state.services.member_storage.add_member(&room_id, &user_id, "join", None, None, None, None).await.unwrap();
...
async fn test_federation_get_user_devices_rejects_server_without_shared_room() {
    ...
    state.services.user_storage.create_user(&user_id, "device-target", None, false).await.unwrap();
...
async fn test_federation_keys_query_filters_local_users_without_shared_rooms() {
    ...
    state.services.user_storage.create_user(&shared_user_id, "query-shared", None, false).await.unwrap();
    state.services.user_storage.create_user(&isolated_user_id, "query-isolated", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &shared_user_id, "private", "1", false).await.unwrap();
    state.services.member_storage.add_member(&room_id, &shared_user_id, "join", None, None, None, None).await.unwrap();
...
async fn test_federation_keys_claim_filters_local_users_without_shared_rooms() {
    ...
    state.services.user_storage.create_user(&shared_user_id, "claim-shared", None, false).await.unwrap();
    state.services.user_storage.create_user(&isolated_user_id, "claim-isolated", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &shared_user_id, "private", "1", false).await.unwrap();
    state.services.member_storage.add_member(&room_id, &shared_user_id, "join", None, None, None, None).await.unwrap();
...
async fn test_federation_event_endpoints_return_spec_shaped_pdu_response() {
    ...
    state.services.user_storage.create_user(&creator, "owner", None, false).await.unwrap();
    state.services.user_storage.create_user(&remote_member, "member", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &creator, "invite", "1", false).await.unwrap();
    state.services.member_storage.add_member(&room_id, &creator, "join", None, None, None, None).await.unwrap();
    state.services.member_storage.add_member(&room_id, &remote_member, "join", None, None, None, None).await.unwrap();
    ...
    let get_event_body = axum::body::to_bytes(get_event_response.into_body(), usize::MAX).await.unwrap();
    ...
    let get_room_event_body = axum::body::to_bytes(get_room_event_response.into_body(), usize::MAX).await.unwrap();
...
async fn test_federation_timestamp_to_event_rejects_server_without_joined_member() {
    ...
    let uri = format!("/_matrix/federation/v1/timestamp_to_event/{}?ts=1717171717000", room_id);
    let response = app.oneshot(signed_request("GET", &uri, server_name, key_id, &signing_key, None)).await.unwrap();
...
async fn test_federation_get_joining_rules_returns_effective_state_event_rule() {
    ...
    assert_eq!(json["allow"][0]["room_id"], Value::String("!space:example.com".to_string()));
}
...
async fn test_federation_hierarchy_returns_space_hierarchy_shape_instead_of_placeholder_summary() {
    ...
    state.services.user_storage.create_user(&creator, "spaceadmin", None, false).await.unwrap();
    state.services.room_storage.create_room(&root_room_id, &creator, "public", "1", true).await.unwrap();
    state.services.room_storage.create_room(&child_room_id, &creator, "public", "1", true).await.unwrap();
    state
        .services
        .space_storage
        ...
        .await
        .unwrap();
    let root_space = state.services.space_service.get_space_by_room(&root_room_id).await.unwrap().unwrap();
    state
        .services
        .space_storage
    ...
    let uri = format!("/_matrix/federation/v1/hierarchy/{}?max_depth=3&limit=10", root_room_id);
    let response = app.oneshot(signed_request("GET", &uri, server_name, key_id, &signing_key, None)).await.unwrap();
...
async fn test_federation_backfill_rejects_unjoined_server_for_private_room() {
    ...
    let uri = format!("/_matrix/federation/v1/backfill/{}?v=$seed:test.example.com&limit=10", room_id);
    let response = app.oneshot(signed_request("GET", &uri, server_name, key_id, &signing_key, None)).await.unwrap();
...
async fn test_federation_query_directory_rejects_unjoined_server_for_private_alias() {
    ...
    state.services.room_storage.create_room(&room_id, &creator, "invite", "1", false).await.unwrap();
    state.services.member_storage.add_member(&room_id, &creator, "join", None, None, None, None).await.unwrap();
    state.services.room_service.set_room_alias(&room_id, &room_alias, &creator).await.unwrap();

    let uri = format!("/_matrix/federation/v1/query/directory?room_alias={}", urlencoding::encode(&room_alias));
    let response = app.oneshot(signed_request("GET", &uri, server_name, key_id, &signing_key, None)).await.unwrap();
...
async fn test_federation_query_directory_rejects_non_local_alias() {
    ...
    state.services.room_storage.create_room(&room_id, &creator, "public", "1", true).await.unwrap();
    state.services.member_storage.add_member(&room_id, &creator, "join", None, None, None, None).await.unwrap();
    state.services.room_service.set_room_alias(&room_id, &remote_alias, &creator).await.unwrap();

    let uri = format!("/_matrix/federation/v1/query/directory?room_alias={}", urlencoding::encode(&remote_alias));
    let response = app.oneshot(signed_request("GET", &uri, server_name, key_id, &signing_key, None)).await.unwrap();
...
async fn test_federation_media_thumbnail_rejects_oversized_dimensions() {
    ...
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert!(
        json["error"].as_str().is_some_and(|message| message.contains("Thumbnail dimensions must be between 1")),
...
async fn test_federation_profile_query_rejects_non_local_user() {
    ...
    state.services.user_storage.create_user(&remote_user, "alice", None, false).await.unwrap();
...
async fn test_federation_profile_query_rejects_missing_local_user() {
    ...
    let Some((app, _state)) = setup_federation_ingress_app_with(server_name, key_id, &signing_key_b64, |_| {}).await
    else {
        return;
    };
...
async fn test_federation_profile_query_honors_field_filter_without_leaking_user_id() {
    ...
    state.services.user_storage.create_user(remote_user, "mallory", None, false).await.unwrap();
...
async fn test_federation_openid_userinfo_rejects_deactivated_user_token() {
    ...
    state.services.user_storage.create_user(&user_id, "openid-deactivated", None, false).await.unwrap();
    ...
    state.services.user_storage.deactivate_user(&user_id).await.unwrap();

    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/_matrix/federation/v1/openid/userinfo?access_token={}", token_value))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn test_federation_query_auth_returns_minimal_success_payload() {
    ...
    let response = app
        .oneshot(signed_request("GET", "/_matrix/federation/v1/query/auth", server_name, key_id, &signing_key, None))
        .await
        .unwrap();
...
async fn test_federation_keys_upload_returns_unrecognized_with_migration_hint() {
    ...
    assert!(json["error"].as_str().unwrap_or_default().contains("user/keys endpoints"));
}
...
async fn test_federation_legacy_keys_claim_returns_unrecognized_with_supported_path_hint() {
    ...
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["errcode"], "M_UNRECOGNIZED");
    assert!(json["error"].as_str().unwrap_or_default().contains("/_matrix/federation/v1/user/keys/claim"));
}
...
async fn test_federation_legacy_keys_query_returns_unrecognized_with_supported_path_hint() {
    ...
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["errcode"], "M_UNRECOGNIZED");
    assert!(json["error"].as_str().unwrap_or_default().contains("/_matrix/federation/v1/user/keys/query"));
}
...
async fn test_federation_send_join_v2_persists_join_event() {
    ...
    state.services.user_storage.create_user(&creator, "creator", None, false).await.unwrap();
    state.services.user_storage.create_user(&existing_member, "member", None, false).await.unwrap();
    state.services.user_storage.create_user(&joiner, "joiner", None, false).await.unwrap();
    ...
    let stored_event =
        state.services.event_storage.get_event(event_id).await.unwrap().expect("join event should be persisted");
    ...
    assert_eq!(stored_member.display_name.as_deref(), Some("Federated Joiner"));
}
...
async fn test_federation_send_join_rejects_uninvited_user_for_invite_only_room() {
    ...
    state.services.user_storage.create_user(&creator, "creator", None, false).await.unwrap();
    state.services.user_storage.create_user(&invitee, "remotejoiner", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &creator, "invite", "1", false).await.unwrap();
    state.services.member_storage.add_member(&room_id, &creator, "join", None, None, None, None).await.unwrap();
    ...
    let stored_v1 = state.services.event_storage.get_event(event_id_v1).await.unwrap();
    let stored_v2 = state.services.event_storage.get_event(event_id_v2).await.unwrap();
...
async fn test_federation_send_transaction_processes_presence_edu_when_enabled() {
    ...
    state
        .services
        .user_storage
        .create_user(&user_id, &format!("presence_{}", rand::random::<u32>()), None, false)
        .await
        .unwrap();
...
async fn test_federation_send_transaction_rejects_member_state_event_without_power() {
    ...
    state.services.user_storage.create_user(&creator, "creator", None, false).await.unwrap();
    state.services.user_storage.create_user(&member, "member", None, false).await.unwrap();
    state.services.room_storage.create_room(&room_id, &creator, "public", "1", true).await.unwrap();
    state.services.member_storage.add_member(&room_id, &creator, "join", None, None, None, None).await.unwrap();
    state.services.member_storage.add_member(&room_id, &member, "join", None, None, None, None).await.unwrap();
    ...
    let stored = state.services.event_storage.get_event(event_id).await.unwrap();
...
async fn test_federation_make_join_returns_429_when_join_lane_is_saturated() {
    ...
    let permit = state.federation_join_semaphore.clone().acquire_owned().await.unwrap();

    let uri =
        format!("/_matrix/federation/v1/make_join/!room:{} /@alice:{}", server_name, server_name).replace(" ", "");
