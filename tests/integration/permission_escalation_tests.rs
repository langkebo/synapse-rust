    ...
    match action {
        "invite" => {
            ...
            if user_power < required {
                return Err(format!(
                    "Insufficient power level for invite: {} < {}",
                    user_power, required
                ));
            }
        }
        "ban" => {
            ...
            if user_power < required {
                return Err(format!(
                    "Insufficient power level for ban: {} < {}",
                    user_power, required
                ));
            }
        }
        "kick" => {
            ...
            if user_power < required {
                return Err(format!(
                    "Insufficient power level for kick: {} < {}",
                    user_power, required
                ));
            }
        }
        "redact" => {
            ...
            if user_power < required {
                return Err(format!(
                    "Insufficient power level for redact: {} < {}",
                    user_power, required
                ));
            }
        }
        "modify_power_levels" => {
            ...
            if user_power < required {
                return Err(format!(
                    "Insufficient power level for modify_power_levels: {} < {}",
                    user_power, required
                ));
            }
        }
...
fn simulate_room_key_access(is_member: bool, is_admin: bool) -> Result<(), String> {
...
fn test_horizontal_escalation_invite_with_default_threshold() {
    ...
    assert!(
        result.is_ok(),
        "Regular member should be able to invite with default threshold"
    );
}
...
fn test_horizontal_escalation_invite_with_custom_threshold() {
    ...
    assert!(
        result.is_err(),
        "Regular member should NOT be able to invite when threshold is 50"
    );
}
...
fn test_horizontal_escalation_ban_by_moderator() {
    ...
    assert!(
        result.is_ok(),
        "Moderator should be able to ban with default threshold"
    );
}
...
fn test_horizontal_escalation_ban_with_custom_threshold() {
    ...
    assert!(
        result.is_err(),
        "Moderator should NOT be able to ban when threshold is 75"
    );
}
...
fn test_vertical_escalation_server_admin_overrides_room_permissions() {
    ...
    assert!(
        result.is_ok(),
        "Server admin should always be able to ban regardless of room power levels"
    );
}
...
fn test_vertical_escalation_modify_power_levels_requires_high_level() {
    ...
    assert!(
        result.is_err(),
        "Moderator should NOT be able to modify power levels when state_default is 100"
    );
}
...
fn test_vertical_escalation_modify_power_levels_by_admin() {
    ...
    assert!(
        result.is_ok(),
        "Room admin should be able to modify power levels"
    );
}
...
fn test_join_room_public_allowed() {
    ...
    assert!(
        result.is_ok(),
        "Public room should allow joining without invite"
    );
}
...
fn test_join_room_invite_only_without_invite() {
    ...
    assert!(
        result.is_err(),
        "Invite-only room should reject joining without invite"
    );
}
...
fn test_join_room_invite_only_with_invite() {
    ...
    assert!(
        result.is_ok(),
        "Invite-only room should allow joining with valid invite"
    );
}
...
fn test_join_room_banned_user() {
    ...
    assert!(
        result.is_err(),
        "Banned user should not be able to join even public rooms"
    );
}
...
fn test_room_key_access_member_allowed() {
    ...
    assert!(
        result.is_ok(),
        "Room member should be able to access room keys"
    );
}
...
fn test_room_key_access_non_member_blocked() {
    ...
    assert!(
        result.is_err(),
        "Non-member should NOT be able to access room keys"
    );
}
...
fn test_room_key_access_admin_allowed() {
    ...
    assert!(
        result.is_ok(),
        "Server admin should be able to access room keys"
    );
}
...
fn test_redact_own_message_allowed() {
    ...
    assert!(
        result.is_err(),
        "Regular user should NOT be able to redact others' messages (sender check is separate)"
    );
}
...
fn test_redact_by_moderator() {
    ...
    assert!(
        result.is_ok(),
        "Moderator should be able to redact messages"
    );
}
...
fn test_custom_kick_threshold() {
    ...
    assert!(
        result.is_err(),
        "Moderator with power 50 should NOT kick when threshold is 75"
    );
}
...
fn test_missing_power_levels_uses_defaults() {
    ...
    assert!(
        ban_result.is_err(),
        "Default ban threshold (50) should block user with power 49"
    );
    assert!(
        invite_result.is_ok(),
        "Default invite threshold (0) should allow user with power 0"
    );
}
...
fn test_rbac_super_admin_can_modify_admin_status() {
    ...
    assert_eq!(
        role, "super_admin",
        "Only super_admin should modify admin status"
    );
}
...
fn test_rbac_non_super_admin_cannot_modify_admin_status() {
    ...
    for role in roles {
        assert_ne!(
            role, "super_admin",
            "Non-super_admin roles should not modify admin status"
        );
    }
}
...
fn test_deactivated_user_token_refresh_blocked() {
    ...
    assert!(
        user_deactivated,
        "Deactivated user should be blocked from token refresh"
    );
}
...
fn test_device_id_ownership_enforcement() {
    ...
    assert_ne!(
        device_user_id, requesting_user,
        "Different user should not use another's device_id"
    );
}
