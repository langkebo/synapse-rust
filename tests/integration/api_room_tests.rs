...
fn create_test_config() -> Config {
    Config {
        server: ServerConfig {
            ...
            remote_media_lifetime: 86400 * 30,
        ...
        sms: synapse_rust::common::config::SmsConfig::default(),
        ...
        telemetry: synapse_rust::common::OpenTelemetryConfig::default(),
        prometheus: synapse_rust::common::PrometheusConfig::default(),
...
async fn send_message_event(
    ...
    let request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/send/m.room.message/{}", room_id, txn_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "application/json")
    ...
    let body = axum::body::to_bytes(response.into_body(), 1024).await.ok()?;
    let json: Value = serde_json::from_slice(&body).ok()?;
    json.get("event_id")?.as_str().map(|value| value.to_string())
}
...
async fn test_user_rooms_rejects_admin_access_to_other_users_scope() {
    ...
    let room_id = create_room(&app, &owner_token, "owner room").await.expect("room should be created");
...
async fn test_room_lifecycle() {
    ...
    let response = ServiceExt::<Request<Body>>::oneshot(app.clone(), request).await.expect("Request failed");
    ...
    let request_whoami = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let response_whoami = ServiceExt::<Request<Body>>::oneshot(app.clone(), request_whoami).await.unwrap();
    let body_whoami = axum::body::to_bytes(response_whoami.into_body(), 1024).await.unwrap();
...
async fn test_encrypted_create_room_preserves_creator_membership_and_invite() {
    ...
    let Some(alice_token) = register_user(&app, &format!("enc_owner_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };
    let Some(bob_token) = register_user(&app, &format!("enc_invitee_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register bob");
        return;
    };

    let bob_user_id = get_user_id(&app, &bob_token).await.expect("bob user id should be available");
    ...
    let create_body =
        axum::body::to_bytes(create_response.into_body(), 4096).await.expect("create room body should be readable");
    let create_json: Value = serde_json::from_slice(&create_body).expect("create room json");
    let room_id = create_json["room_id"].as_str().expect("room id should be present").to_string();
    ...
    let joined_rooms_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let joined_rooms_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), joined_rooms_request)
        .await
    ...
    let joined_rooms_json: Value = serde_json::from_slice(&joined_rooms_body).expect("joined rooms json");
    let joined_rooms = joined_rooms_json["joined_rooms"].as_array().expect("joined_rooms should be an array");
    assert!(
    ...
    let joined_members_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let joined_members_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), joined_members_request)
        .await
    ...
    let alice_sync_body =
        axum::body::to_bytes(alice_sync_response.into_body(), 32768).await.expect("alice sync body should be readable");
    let alice_sync_json: Value = serde_json::from_slice(&alice_sync_body).expect("alice sync json");
    assert!(
    ...
    let bob_sync_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let bob_sync_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), bob_sync_request).await.expect("bob sync should complete");
    assert_eq!(bob_sync_response.status(), StatusCode::OK);
    let bob_sync_body =
        axum::body::to_bytes(bob_sync_response.into_body(), 32768).await.expect("bob sync body should be readable");
...
async fn test_private_room_member_endpoints_reject_invited_user() {
    ...
    let Some(alice_token) = register_user(&app, &format!("members_owner_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };

    let Some(bob_token) = register_user(&app, &format!("members_invited_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register bob");
        return;
    };
    ...
    let joined_members_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let joined_members_response = ServiceExt::<Request<Body>>::oneshot(app, joined_members_request).await.unwrap();
...
async fn test_private_room_aliases_reject_non_member() {
    ...
    let Some(owner_token) = register_user(&app, &format!("aliases_owner_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register owner");
        return;
    };

    let Some(outsider_token) = register_user(&app, &format!("aliases_outsider_{}", rand::random::<u32>())).await else {
    ...
    let outsider_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let outsider_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), outsider_request).await.unwrap();
...
async fn test_room_invite_route_succeeds_for_existing_user() {
    ...
    let Some(alice_token) = register_user(&app, &format!("invite_alice_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };

    let Some(bob_token) = register_user(&app, &format!("invite_bob_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register bob");
        return;
    };
...
async fn test_room_invite_route_returns_not_found_for_missing_user() {
    ...
    let Some(alice_token) = register_user(&app, &format!("invite_missing_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };
    ...
    let request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "user_id": missing_user_id }).to_string()))
        .unwrap();
...
async fn test_room_invite_route_rejects_non_member_inviter() {
    ...
    let Some(alice_token) = register_user(&app, &format!("invite_owner_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };

    let Some(eve_token) = register_user(&app, &format!("invite_intruder_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register eve");
        return;
    };

    let Some(bob_token) = register_user(&app, &format!("invite_target_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register bob");
        return;
    };
...
async fn test_directory_routes_work_across_r0_and_v3() {
    ...
    let create_room_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"name": "Directory Room", "visibility": "public"}).to_string()))
        .unwrap();
    ...
    let r0_public_rooms_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_public_rooms_request).await.unwrap();
    assert_eq!(r0_public_rooms_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(r0_public_rooms_response.into_body(), 10240).await.unwrap();
    ...
    let v3_public_rooms_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), v3_public_rooms_request).await.unwrap();
    assert_eq!(v3_public_rooms_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(v3_public_rooms_response.into_body(), 10240).await.unwrap();
    let v3_public_rooms_json: Value = serde_json::from_slice(&body).unwrap();

    assert!(r0_public_rooms_json["chunk"].as_array().unwrap().iter().any(|room| room["room_id"] == room_id));
    ...
    let r0_aliases_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let r0_aliases_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_aliases_request).await.unwrap();
    assert_eq!(r0_aliases_response.status(), StatusCode::OK);

    let v3_aliases_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v3/directory/room/{}/alias", room_id))
        .header("Authorization", format!("Bearer {}", alice_token))
        .body(Body::empty())
        .unwrap();
    let v3_aliases_response = ServiceExt::<Request<Body>>::oneshot(app, v3_aliases_request).await.unwrap();
...
async fn test_room_state_and_redaction() {
    ...
    let request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/redact/{}/txn_redact_1", room_id, event_id))
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
...
async fn test_room_write_routes_reject_admin_non_member() {
    ...
    let owner_token = register_user(&app, &owner_name).await.expect("owner should register");
    let member_token = register_user(&app, &member_name).await.expect("member should register");
    let admin_token = register_user(&app, &admin_name).await.expect("admin should register");
    promote_to_admin(&pool, &cache, &admin_user_id).await;

    let room_id =
        create_room(&app, &owner_token, "Admin Non-Member Write Guard Room").await.expect("room should be created");
    let member_user_id = get_user_id(&app, &member_token).await.expect("member user id should resolve");
    ...
    let join_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let join_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), join_request).await.unwrap();
    assert_eq!(join_response.status(), StatusCode::OK);

    let seed_message_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/send/m.room.message/owner_seed_txn", room_id))
        .header("Authorization", format!("Bearer {}", owner_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "msgtype": "m.text", "body": "seed event" }).to_string()))
        .unwrap();
    let seed_message_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), seed_message_request).await.unwrap();
    assert_eq!(seed_message_response.status(), StatusCode::OK);
    let seed_body = axum::body::to_bytes(seed_message_response.into_body(), 1024).await.unwrap();
    let seed_json: Value = serde_json::from_slice(&seed_body).unwrap();
    let event_id = seed_json["event_id"].as_str().expect("seed message should return event id");

    let send_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/send/m.room.message/admin_write_txn", room_id))
        .header("Authorization", format!("Bearer {}", admin_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "msgtype": "m.text", "body": "admin outsider write" }).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    ...
    let state_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", admin_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "topic": "admin outsider topic" }).to_string()))
        .unwrap();
    ...
    let power_levels_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/send/m.room.power_levels/admin_power_levels_txn", room_id))
        .header("Authorization", format!("Bearer {}", admin_token))
        .header("Content-Type", "application/json")
        .body(Body::from(
            json!({
        ...
        .unwrap();
    let power_levels_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), power_levels_request).await.unwrap();
    assert_eq!(power_levels_response.status(), StatusCode::FORBIDDEN);

    let redact_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/redact/{}/admin_redact_txn", room_id, event_id))
        .header("Authorization", format!("Bearer {}", admin_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "reason": "admin outsider redact" }).to_string()))
        .unwrap();
    let redact_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), redact_request).await.unwrap();
    ...
    let kick_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", admin_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "user_id": member_user_id, "reason": "admin outsider kick" }).to_string()))
        .unwrap();
    ...
    let ban_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", admin_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "user_id": member_user_id, "reason": "admin outsider ban" }).to_string()))
        .unwrap();
    ...
    let unban_request = Request::builder()
        ...
        .body(Body::from(json!({ "user_id": member_user_id }).to_string()))
        .unwrap();
    let unban_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), unban_request).await.unwrap();
...
async fn test_unban_requires_ban_power_level() {
    ...
    let owner_token = register_user(&app, &owner_name).await.expect("owner should register");
    let moderator_token = register_user(&app, &moderator_name).await.expect("moderator should register");
    let target_token = register_user(&app, &target_name).await.expect("target should register");

    let room_id = create_room(&app, &owner_token, "Unban Ban Threshold Guard").await.expect("room should be created");
    ...
    for invited_user_id in [&moderator_user_id, &target_user_id] {
        let invite_request = Request::builder()
            ...
            .header("Authorization", format!("Bearer {}", owner_token))
            .header("Content-Type", "application/json")
            .body(Body::from(json!({ "user_id": invited_user_id }).to_string()))
            .unwrap();
        let invite_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), invite_request).await.unwrap();
        assert_eq!(invite_response.status(), StatusCode::OK);
    }
    ...
    let ban_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", owner_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "user_id": target_user_id, "reason": "ban threshold regression guard" }).to_string()))
        .unwrap();
    let ban_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), ban_request).await.unwrap();
...
async fn test_upgrade_room_requires_tombstone_permission() {
    ...
    let room_id =
        create_room(&app, &owner_token, "Upgrade Permission Guard Room").await.expect("room should be created");
    ...
    let upgrade_request = Request::builder()
        ...
        .unwrap();
    let upgrade_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), upgrade_request).await.unwrap();
...
async fn test_unban_rejects_target_with_higher_power_level() {
    ...
    let room_id = create_room(&app, &owner_token, "Unban Power Guard Room").await.expect("room should be created");
    ...
    let ban_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", owner_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "user_id": target_user_id, "reason": "ban target before unban guard" }).to_string()))
        .unwrap();
    let ban_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), ban_request).await.unwrap();
    assert_eq!(ban_response.status(), StatusCode::OK);
...
async fn test_redact_requires_current_room_membership_even_for_sender() {
    ...
    let token = register_user(&app, &user_name).await.expect("user should register");
    ...
    let create_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "name": "Redact Membership Guard Room" }).to_string()))
        .unwrap();
    ...
    let send_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/send/m.room.message/redact_membership_txn", room_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "msgtype": "m.text", "body": "redact after leave" }).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    let send_body = axum::body::to_bytes(send_response.into_body(), 1024).await.unwrap();
    ...
    let leave_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let leave_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), leave_request).await.unwrap();
    assert_eq!(leave_response.status(), StatusCode::OK);

    let redact_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/redact/{}/redact_after_leave_txn", room_id, event_id))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "reason": "should fail after leaving room" }).to_string()))
        .unwrap();
    let redact_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), redact_request).await.unwrap();
    assert_eq!(redact_response.status(), StatusCode::FORBIDDEN);
}
...
async fn test_room_visibility_route_rejects_admin_non_creator() {
    ...
    let room_id = create_room(&app, &owner_token, "Visibility Guard Room").await.expect("room should be created");
    ...
    let owner_request = Request::builder()
        ...
        .body(Body::from(json!({ "visibility": "public" }).to_string()))
        .unwrap();
    let owner_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), owner_request).await.unwrap();
    assert_eq!(owner_response.status(), StatusCode::OK);
}
...
async fn test_knock_rejects_invite_only_room() {
    ...
    let owner_token = register_user(&app, &owner_name).await.expect("owner should register");
    let outsider_token = register_user(&app, &outsider_name).await.expect("outsider should register");

    let room_id = create_room(&app, &owner_token, "Invite Only Knock Guard").await.expect("room should be created");
    ...
    let knock_request = Request::builder()
        ...
        .unwrap();
    let knock_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), knock_request).await.unwrap();
...
async fn test_room_visibility_route_rejects_outsider_private_update() {
    ...
    let room_id =
        create_room(&app, &owner_token, "Visibility Private Guard Room").await.expect("room should be created");
...
async fn test_send_receipt_rejects_non_member() {
    ...
    let owner_token =
        register_user(&app, &format!("receipt_owner_{}", rand::random::<u32>())).await.expect("owner should register");
    let outsider_token = register_user(&app, &format!("receipt_outsider_{}", rand::random::<u32>()))
        .await
        .expect("outsider should register");

    let room_id = create_room(&app, &owner_token, "Receipt Membership Guard").await.expect("room should be created");
    ...
    let request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/client/r0/rooms/{}/receipt/m.read/{}", room_id, event_id.replace('$', "%24")))
        .header("Authorization", format!("Bearer {}", outsider_token))
        .header("Content-Type", "application/json")
...
async fn test_get_receipts_requires_room_view_access() {
    ...
    let owner_token =
        register_user(&app, &format!("receipts_owner_{}", rand::random::<u32>())).await.expect("owner should register");
    let outsider_token = register_user(&app, &format!("receipts_outsider_{}", rand::random::<u32>()))
        .await
        .expect("outsider should register");

    let room_id = create_room(&app, &owner_token, "Receipts View Guard").await.expect("room should be created");
    let event_id = send_message_event(
        &app,
        &owner_token,
        &room_id,
    ...
    let anonymous_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/r0/rooms/{}/receipts/m.read/{}", room_id, encoded_event_id))
        .body(Body::empty())
        .unwrap();
    ...
    let outsider_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/r0/rooms/{}/receipts/m.read/{}", room_id, event_id.replace('$', "%24")))
        .header("Authorization", format!("Bearer {}", outsider_token))
        .body(Body::empty())
        .unwrap();
    let outsider_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), outsider_request).await.unwrap();
...
async fn test_send_receipt_rejects_cross_room_event_id() {
    ...
    let source_room_id = create_room(&app, &token, "Receipt Source Room").await.expect("source room should be created");
    let target_room_id = create_room(&app, &token, "Receipt Target Room").await.expect("target room should be created");
...
#[tokio::test]
async fn test_send_receipt_accepts_unknown_event_id_as_noop() {
    let Some(app) = setup_test_app().await else {
        eprintln!("Skipping test: database not available");
        return;
    };
...
async fn test_room_visibility_route_rejects_creator_after_leaving_room() {
    ...
    let room_id =
        create_room(&app, &owner_token, "Visibility Departed Owner Guard").await.expect("room should be created");
    ...
    let update_request = Request::builder()
        ...
        .body(Body::from(json!({ "visibility": "public" }).to_string()))
        .unwrap();
    let update_response = ServiceExt::<Request<Body>>::oneshot(app, update_request).await.unwrap();
...
async fn test_room_info_and_event_routes_work_across_r0_and_v3() {
    ...
    let Some(alice_token) = register_user(&app, &format!("room_compat_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };
    ...
    let r0_info_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let r0_info_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_info_request).await.unwrap();
    assert_eq!(r0_info_response.status(), StatusCode::OK);
    let body = axum::body::to_bytes(r0_info_response.into_body(), 2048).await.unwrap();
    ...
    let v3_info_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let v3_info_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), v3_info_request).await.unwrap();
    assert_eq!(v3_info_response.status(), StatusCode::OK);
    let body = axum::body::to_bytes(v3_info_response.into_body(), 2048).await.unwrap();
    let v3_info_json: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(r0_info_json, v3_info_json);

    let send_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/v3/rooms/{}/send/m.room.message/compat_txn_{}", room_id, rand::random::<u32>()))
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "cross-version event"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(send_response.into_body(), 1024).await.unwrap();
    ...
    let get_event_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let get_event_response = ServiceExt::<Request<Body>>::oneshot(app, get_event_request).await.unwrap();
...
async fn test_room_report_route_boundaries_are_preserved() {
    ...
    let Some(alice_token) = register_user(&app, &format!("room_report_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };
    ...
    let send_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/send/m.room.message/report_txn_{}", room_id, rand::random::<u32>()))
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "report target"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    ...
    for path in [
        ...
        let request = Request::builder()
            ...
            .header("Authorization", format!("Bearer {}", alice_token))
            .header("Content-Type", "application/json")
            .body(Body::from(json!({"reason": "compatibility check", "score": -50}).to_string()))
            .unwrap();
        let response = ServiceExt::<Request<Body>>::oneshot(app.clone(), request).await.unwrap();
    ...
    let r0_report_room_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"reason": "should not exist"}).to_string()))
        .unwrap();
    let r0_report_room_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_report_room_request).await.unwrap();
    assert_eq!(r0_report_room_response.status(), StatusCode::NOT_FOUND);

    let scanner_request = Request::builder()
        .uri(format!("/_matrix/client/v1/rooms/{}/report/{}/scanner_info", room_id, event_id))
        .header("Authorization", format!("Bearer {}", alice_token))
        .body(Body::empty())
        .unwrap();
    let scanner_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), scanner_request).await.unwrap();
    assert_ne!(scanner_response.status(), StatusCode::NOT_FOUND);

    let v3_scanner_request = Request::builder()
        .uri(format!("/_matrix/client/v3/rooms/{}/report/{}/scanner_info", room_id, event_id))
        .header("Authorization", format!("Bearer {}", alice_token))
        .body(Body::empty())
        .unwrap();
    let v3_scanner_response = ServiceExt::<Request<Body>>::oneshot(app, v3_scanner_request).await.unwrap();
...
async fn test_relations_routes_work_across_v1_and_r0() {
    ...
    let Some(alice_token) = register_user(&app, &format!("relations_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };
    ...
    let send_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "relation target"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    ...
    let v1_put_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/v1/rooms/{}/relations/{}/m.annotation/{}", room_id, event_id, event_id))
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"key": "🔥"}).to_string()))
        .unwrap();
    let v1_put_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), v1_put_request).await.unwrap();
    assert_eq!(v1_put_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(v1_put_response.into_body(), 1024).await.unwrap();
    ...
    let r0_relations_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/r0/rooms/{}/relations/{}/m.annotation", room_id, event_id))
        .body(Body::empty())
        .unwrap();
    let r0_relations_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_relations_request).await.unwrap();
    assert_eq!(r0_relations_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(r0_relations_response.into_body(), 2048).await.unwrap();
    ...
    let r0_aggregations_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/r0/rooms/{}/aggregations/{}/m.annotation", room_id, event_id))
        .body(Body::empty())
        .unwrap();
    let r0_aggregations_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_aggregations_request).await.unwrap();
    assert_eq!(r0_aggregations_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(r0_aggregations_response.into_body(), 2048).await.unwrap();
    ...
    let v3_relations_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v3/rooms/{}/relations/{}/m.annotation", room_id, event_id))
        .body(Body::empty())
        .unwrap();
    let v3_relations_response = ServiceExt::<Request<Body>>::oneshot(app, v3_relations_request).await.unwrap();
...
async fn test_report_room_v3_uses_report_rate_limits_contract_and_returns_expected_payload() {
    ...
    let Some(access_token) = register_user(&app, &format!("report_room_v3_{}", rand::random::<u32>())).await else {
    ...
    assert!(!blocked_until_ts_exists, "Legacy blocked_until_ts column should not be referenced by the current schema");
    ...
    storage
        .block_user_reports(&reporter, blocked_until_at, "rate limited in integration test")
        .await
    ...
    let rate_limit = storage.check_rate_limit(&reporter).await.expect("Failed to evaluate report rate limit");
    assert!(!rate_limit.is_allowed);
    assert_eq!(rate_limit.block_reason.as_deref(), Some("rate limited in integration test"));

    storage.unblock_user_reports(&reporter).await.expect("Failed to unblock reporter");

    let unblocked_rate_limit =
        storage.check_rate_limit(&reporter).await.expect("Failed to evaluate report rate limit after unblock");
...
async fn test_report_event_forbidden_for_non_member() {
    ...
    let Some(owner_token) = register_user(&app, &format!("report_event_owner_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register owner");
        return;
    };
    let Some(outsider_token) = register_user(&app, &format!("report_event_outsider_{}", rand::random::<u32>())).await
    else {
        eprintln!("Skipping test: failed to register outsider");
        return;
    };
    ...
    let send_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", owner_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "event to report"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    ...
    let request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/client/v3/rooms/{}/report/{}", room_id, event_id))
        .header("Authorization", format!("Bearer {}", outsider_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"reason": "outsider should not report", "score": -20}).to_string()))
        .unwrap();
    let response = ServiceExt::<Request<Body>>::oneshot(app, request).await.unwrap();
    assert_eq!(response.status(), StatusCode::FORBIDDEN);
...
async fn test_report_room_forbidden_for_non_member() {
    ...
    let Some(owner_token) = register_user(&app, &format!("report_room_owner_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register owner");
        return;
    };
    let Some(outsider_token) = register_user(&app, &format!("report_room_outsider_{}", rand::random::<u32>())).await
    else {
        eprintln!("Skipping test: failed to register outsider");
        return;
    };
    ...
    let request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", outsider_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"reason": "outsider should not report", "description": "not a member"}).to_string()))
        .unwrap();
    let response = ServiceExt::<Request<Body>>::oneshot(app, request).await.unwrap();
    assert_eq!(response.status(), StatusCode::FORBIDDEN);
...
async fn test_report_event_rejects_event_from_other_room() {
    ...
    let Some(access_token) = register_user(&app, &format!("report_event_mismatch_{}", rand::random::<u32>())).await
    else {
        eprintln!("Skipping test: failed to register user");
        return;
    };
    ...
    let send_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", access_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "wrong room event"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    ...
    let request = Request::builder()
        .method("POST")
        .uri(format!("/_matrix/client/v3/rooms/{}/report/{}", target_room_id, event_id))
        .header("Authorization", format!("Bearer {}", access_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"reason": "room mismatch", "score": -50}).to_string()))
        .unwrap();
    let response = ServiceExt::<Request<Body>>::oneshot(app, request).await.unwrap();
    assert_eq!(response.status(), StatusCode::NOT_FOUND);
...
async fn test_scanner_info_rejects_event_from_other_room() {
    ...
    let Some(access_token) = register_user(&app, &format!("scanner_info_mismatch_{}", rand::random::<u32>())).await
    else {
        eprintln!("Skipping test: failed to register user");
        return;
    };
    ...
    let send_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", access_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "wrong room scanner event"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    ...
    let request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v1/rooms/{}/report/{}/scanner_info", target_room_id, event_id))
        .header("Authorization", format!("Bearer {}", access_token))
        .body(Body::empty())
        .unwrap();
    let response = ServiceExt::<Request<Body>>::oneshot(app, request).await.unwrap();
...
async fn test_reaction_send_routes_work_across_r0_and_v3() {
    ...
    let Some(alice_token) = register_user(&app, &format!("reactions_{}", rand::random::<u32>())).await else {
        eprintln!("Skipping test: failed to register alice");
        return;
    };
    ...
    let send_request = Request::builder()
        ...
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "reaction target"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    ...
    let r0_reaction_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/r0/rooms/{}/send/m.reaction/r0_reaction_{}", room_id, rand::random::<u32>()))
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(
            json!({
        ...
        .unwrap();
    let r0_reaction_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), r0_reaction_request).await.unwrap();
    assert_eq!(r0_reaction_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(r0_reaction_response.into_body(), 1024).await.unwrap();
    ...
    let v3_reaction_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/v3/rooms/{}/send/m.reaction/v3_reaction_{}", room_id, rand::random::<u32>()))
        .header("Authorization", format!("Bearer {}", alice_token))
        .header("Content-Type", "application/json")
        .body(Body::from(
            json!({
        ...
        .unwrap();
    let v3_reaction_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), v3_reaction_request).await.unwrap();
    assert_eq!(v3_reaction_response.status(), StatusCode::OK);

    let v3_relations_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v3/rooms/{}/relations/{}", room_id, event_id))
        .body(Body::empty())
        .unwrap();
    let v3_relations_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), v3_relations_request).await.unwrap();
    assert_eq!(v3_relations_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(v3_relations_response.into_body(), 2048).await.unwrap();
    ...
    let r0_relations_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/r0/rooms/{}/relations/{}", room_id, event_id))
        .body(Body::empty())
        .unwrap();
    let r0_relations_response = ServiceExt::<Request<Body>>::oneshot(app, r0_relations_request).await.unwrap();
...
async fn test_room_version_is_available_after_create_room() {
    ...
    let Some(token) = register_user(&app, &format!("room_version_{}", rand::random::<u32>())).await else {
        return;
    };
    ...
    let version_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let version_response = ServiceExt::<Request<Body>>::oneshot(app, version_request).await.unwrap();
    assert_eq!(version_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(version_response.into_body(), 1024).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["room_version"], DEFAULT_ROOM_VERSION);
}

#[tokio::test]
async fn test_create_room_rejects_unsupported_room_version() {
    let Some(app) = setup_test_app().await else {
        return;
    };

    let Some(token) = register_user(&app, &format!("unsupported_room_version_{}", rand::random::<u32>())).await else {
        return;
    };
    ...
    let create_request = Request::builder()
        ...
        .body(Body::from(
            json!({
                "name": "Unsupported Version Test Room",
                "room_version": "12"
            })
            .to_string(),
        ))
        .unwrap();
    let create_response = ServiceExt::<Request<Body>>::oneshot(app, create_request).await.unwrap();
    assert_eq!(create_response.status(), StatusCode::BAD_REQUEST);

    let body = axum::body::to_bytes(create_response.into_body(), 1024).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["errcode"], "M_UNSUPPORTED_ROOM_VERSION");
}

#[tokio::test]
async fn test_creation_content_cannot_override_create_fields() {
    let Some(app) = setup_test_app().await else {
        return;
    };

    let username = format!("creation_content_create_fields_{}", rand::random::<u32>());
        ...
        .body(Body::from(
            json!({
                ...
                "creation_content": {
                    ...
                    "predecessor": {
                        "room_id": "!spoofed:localhost",
                        "event_id": "$spoofed:localhost"
                    },
                    "m.federate": false
                }
            })
    ...
    let create_state_request = Request::builder()
        .uri(format!("/_matrix/client/v3/rooms/{}/state/m.room.create", room_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let create_state_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), create_state_request).await.unwrap();
    assert_eq!(create_state_response.status(), StatusCode::OK);
...
#[tokio::test]
async fn test_create_room_rejects_protected_initial_state_events() {
    let Some(app) = setup_test_app().await else {
        return;
    };

    let Some(token) = register_user(&app, &format!("protected_initial_state_{}", rand::random::<u32>())).await else {
        return;
    };

    for protected_event_type in ["m.room.create", "m.room.member", "m.room.tombstone"] {
        let create_request = Request::builder()
            .method("POST")
            .uri("/_matrix/client/v3/createRoom")
            .header("Authorization", format!("Bearer {}", token))
            ...
            .body(Body::from(
                json!({
                    "name": "Protected Initial State Test Room",
                    "initial_state": [{
                        "type": protected_event_type,
                        "state_key": "",
                        "content": {
                            "creator": "@attacker:localhost",
                            "room_version": "1",
                            "replacement_room": "!spoofed:localhost"
                        }
                    }]
                })
                .to_string(),
...
async fn test_room_hierarchy_returns_room_summary_for_regular_room() {
    ...
    let Some(token) = register_user(&app, &format!("room_hierarchy_{}", rand::random::<u32>())).await else {
        return;
    };
...
async fn test_room_timeline_route_returns_real_messages() {
    ...
    let Some(token) = register_user(&app, &format!("room_timeline_{}", rand::random::<u32>())).await else {
        return;
    };
    ...
    let send_request = Request::builder()
        .method("PUT")
        .uri(format!("/_matrix/client/v3/rooms/{}/send/m.room.message/timeline_txn_{}", room_id, rand::random::<u32>()))
        .header("Authorization", format!("Bearer {}", token))
        .header("Content-Type", "application/json")
        .body(Body::from(json!({"msgtype": "m.text", "body": "timeline route message"}).to_string()))
        .unwrap();
    let send_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), send_request).await.unwrap();
    assert_eq!(send_response.status(), StatusCode::OK);
    ...
    let timeline_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v3/rooms/{}/timeline?limit=10", room_id))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let timeline_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), timeline_request).await.unwrap();
    assert_eq!(timeline_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(timeline_response.into_body(), 4096).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
    let chunk = json["chunk"].as_array().unwrap();
    assert!(!chunk.is_empty());
    assert_eq!(chunk[0]["content"]["body"].as_str(), Some("timeline route message"));
...
async fn test_room_messages_route_accepts_sync_prev_batch_token() {
    ...
    let body_whoami = axum::body::to_bytes(response_whoami.into_body(), 2048).await.unwrap();
    ...
    for body in ["msg-1", "msg-2", "msg-3"] {
        let send_request = Request::builder()
            .method("PUT")
            .uri(format!("/_matrix/client/v3/rooms/{}/send/m.room.message/{}_{}", room_id, body, rand::random::<u32>()))
            .header("Authorization", format!("Bearer {}", token))
            .header("Content-Type", "application/json")
    ...
    let create_filter_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), create_filter_request).await.unwrap();
    assert_eq!(create_filter_response.status(), StatusCode::OK);
    let create_filter_body = axum::body::to_bytes(create_filter_response.into_body(), 2048).await.unwrap();
    ...
    let sync_body = axum::body::to_bytes(sync_response.into_body(), 128 * 1024).await.unwrap();
    let sync_json: Value = serde_json::from_slice(&sync_body).unwrap();
    let prev_batch =
        sync_json["rooms"]["join"][room_id.as_str()]["timeline"]["prev_batch"].as_str().unwrap().to_string();
    assert!(prev_batch.starts_with('t'));

    let messages_request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v3/rooms/{}/messages?from={}&dir=b&limit=5", room_id, prev_batch))
        .header("Authorization", format!("Bearer {}", token))
        .body(Body::empty())
        .unwrap();
    let messages_response = ServiceExt::<Request<Body>>::oneshot(app, messages_request).await.unwrap();
    assert_eq!(messages_response.status(), StatusCode::OK);

    let messages_body = axum::body::to_bytes(messages_response.into_body(), 128 * 1024).await.unwrap();
...
async fn test_room_timeline_route_rejects_non_member() {
    ...
    let Some(alice_token) = register_user(&app, &format!("timeline_owner_{}", rand::random::<u32>())).await else {
        return;
    };
    let Some(bob_token) = register_user(&app, &format!("timeline_guest_{}", rand::random::<u32>())).await else {
        return;
    };
    ...
    let admin_timeline_response =
        ServiceExt::<Request<Body>>::oneshot(app.clone(), admin_timeline_request).await.unwrap();
...
async fn test_room_messages_route_rejects_non_member() {
    ...
    let Some(alice_token) = register_user(&app, &format!("messages_owner_{}", rand::random::<u32>())).await else {
        return;
    };
    let Some(bob_token) = register_user(&app, &format!("messages_guest_{}", rand::random::<u32>())).await else {
        return;
    };
    ...
    let request = Request::builder()
        .method("GET")
        .uri(format!("/_matrix/client/v3/rooms/{}/messages?from=0&dir=b&limit=10", room_id))
        .header("Authorization", format!("Bearer {}", bob_token))
        .body(Body::empty())
        .unwrap();
    let response = ServiceExt::<Request<Body>>::oneshot(app.clone(), request).await.unwrap();
...
async fn test_room_state_route_rejects_non_member() {
    ...
    let Some(alice_token) = register_user(&app, &format!("state_owner_{}", rand::random::<u32>())).await else {
        return;
    };
    let Some(bob_token) = register_user(&app, &format!("state_guest_{}", rand::random::<u32>())).await else {
        return;
    };
...
async fn test_room_unread_count_route_returns_counts_from_summary() {
    ...
    let Some(token) = register_user(&app, &format!("room_unread_{}", rand::random::<u32>())).await else {
        return;
    };
    ...
    let unread_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let unread_response = ServiceExt::<Request<Body>>::oneshot(app, unread_request).await.unwrap();
    assert_eq!(unread_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(unread_response.into_body(), 2048).await.unwrap();
    let json: Value = serde_json::from_slice(&body).unwrap();
...
async fn test_room_unread_count_route_rejects_non_member() {
    ...
    let Some(alice_token) = register_user(&app, &format!("unread_owner_{}", rand::random::<u32>())).await else {
        return;
    };
    let Some(bob_token) = register_user(&app, &format!("unread_guest_{}", rand::random::<u32>())).await else {
        return;
    };
...
async fn test_room_spaces_route_returns_child_rooms_and_parent_spaces() {
    ...
    let Some(token) = register_user(&app, &format!("room_spaces_{}", rand::random::<u32>())).await else {
        return;
    };
    ...
    let parent_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let parent_response = ServiceExt::<Request<Body>>::oneshot(app.clone(), parent_request).await.unwrap();
    assert_eq!(parent_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(parent_response.into_body(), 4096).await.unwrap();
    ...
    let child_request = Request::builder()
        ...
        .body(Body::empty())
        .unwrap();
    let child_response = ServiceExt::<Request<Body>>::oneshot(app, child_request).await.unwrap();
    assert_eq!(child_response.status(), StatusCode::OK);

    let body = axum::body::to_bytes(child_response.into_body(), 4096).await.unwrap();
