...
async fn setup_test_app(openclaw_enabled: bool) -> Option<axum::Router> {
    ...
    let cache = container.core.cache.clone();
...
async fn register_guest_user(app: &axum::Router) -> (String, String) {
    let request =
        Request::builder().method("POST").uri("/_matrix/client/v3/register/guest").body(Body::empty()).unwrap();
...
async fn get_json_response(app: &axum::Router, uri: &str, access_token: &str) -> (StatusCode, Value) {
...
async fn json_response(
    ...
    let json = serde_json::from_slice(&body).unwrap_or_else(|_| {
        panic!("expected JSON response from {} with status {}, body: {}", uri, status, String::from_utf8_lossy(&body))
    });
...
async fn test_openclaw_routes_are_not_mounted_by_default() {
    ...
    let (status, _) = get_json_response(&app, &format!("{}/connections", OPENCLAW_UNSTABLE_PREFIX), &user_token).await;
...
async fn test_openclaw_routes_mount_only_under_unstable_prefix_when_enabled() {
    ...
    let (stable_status, _) = get_json_response(&app, "/_matrix/client/v3/openclaw/connections", &user_token).await;
    assert_eq!(stable_status, StatusCode::NOT_FOUND);

    let (unstable_status, unstable_json) =
        get_json_response(&app, &format!("{}/connections", OPENCLAW_UNSTABLE_PREFIX), &user_token).await;
...
async fn test_openclaw_routes_reject_guest_users() {
    ...
    let (status, json) =
        get_json_response(&app, &format!("{}/connections", OPENCLAW_UNSTABLE_PREFIX), &guest_token).await;
...
async fn test_openclaw_update_connection_rejects_private_ip_base_url() {
    ...
    assert_eq!(update_body["error"], "OpenClaw base_url cannot target local or private IP ranges");
}
...
async fn test_openclaw_private_connection_returns_not_found_to_other_users() {
    ...
    let (owner_token, _) = register_user(&app, &format!("openclaw_owner_{}", rand::random::<u32>())).await.unwrap();
    let (other_token, _) = register_user(&app, &format!("openclaw_other_{}", rand::random::<u32>())).await.unwrap();
...
async fn test_openclaw_private_role_is_hidden_but_public_role_remains_readable() {
    ...
    let (owner_token, _) =
        register_user(&app, &format!("openclaw_role_owner_{}", rand::random::<u32>())).await.unwrap();
    let (other_token, _) =
        register_user(&app, &format!("openclaw_role_other_{}", rand::random::<u32>())).await.unwrap();
