...
CREATE_TABLE_RE = re.compile(r"CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([a-zA-Z_][a-zA-Z0-9_]*)")
# 新增索引：CREATE [UNIQUE] INDEX [IF NOT EXISTS] idx_xxx
CREATE_INDEX_RE = re.compile(r"CREATE\s+(?:UNIQUE\s+)?INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?([a-zA-Z_][a-zA-Z0-9_]*)")
# 新增列：ALTER TABLE xxx ADD COLUMN [IF NOT EXISTS] yyy
ADD_COLUMN_RE = re.compile(r"ALTER\s+TABLE\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+ADD\s+COLUMN\s+(?:IF\s+NOT\s+EXISTS\s+)?([a-zA-Z_][a-zA-Z0-9_]*)")


def latest_baseline() -> pathlib.Path:
    baselines = sorted(
        p for p in MIGRATIONS_DIR.glob("00000000_unified_schema_v*.sql")
...
def main() -> int:
    ...
    for f in ts_files:
        ...
        for obj in CREATE_TABLE_RE.findall(text):
            if f"CREATE TABLE IF NOT EXISTS {obj}" not in baseline_text and obj not in baseline_text:
    ...
    if missing:
        print(f"❌ {baseline.name} 漏吸收以下增量迁移对象（会导致 forward-only source 缺失）:")
        for m in missing:
            print(f"   - {m}")
