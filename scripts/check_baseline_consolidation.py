...
背景：`migrations/` 只有一条正向链（consolidated baseline + extension + `V*`），
所有 schema 变更都必须折入 baseline。一旦某个时间戳增量迁移没有被折进去，它既不
会被任何 applier 执行（`docker/db_migrate.sh` 只跑 baseline 与其后的正向文件），也
不会反映在 CI 建的库里 —— 生产库与 CI 库就此分叉。baseline 是手动 consolidate，
历史上曾漏吸收 8 个迁移（federation_dead_letter_queue 等），导致 CI 的 DB 与生产
不一致。
...
COLUMN_DEF_RE = re.compile(
    r"(?:^|\n)\s*(?P<col>[a-zA-Z_][a-zA-Z0-9_]*)\s+(?:[a-zA-Z][a-zA-Z0-9_]*)\b",
    re.MULTILINE,
)
    ...
    for m in TABLE_COLUMN_RE.finditer(text):
        ...
        for fk in _FK_IN_TABLE_RE.finditer(m.group("body")):
            ...
            groups.setdefault(key, []).append(
                fk.group("name") or f"{table}.{fk.group('col')}(inline)"
            )
    for m in _ALTER_FK_RE.finditer(text):
        key = (
            m.group("table").lower(),
            m.group("col").lower(),
            m.group("parent").lower(),
        )
        groups.setdefault(key, []).append(m.group("name"))
...
def main() -> int:
    ...
    if dup_idx or dup_fk:
        ...
        for (tbl, uniq, method, cols, where), names in sorted(dup_idx.items()):
            ...
            print(
                f"   - 索引 [{tbl}] {kind}{method} ({cols}){extra}: {', '.join(sorted(names))}"
            )
            print(
                "     同一 (表, 列集, 唯一性, 方法, 谓词) 只需一个；保留约束索引，删除普通索引。"
            )
    ...
    if missing:
        print(
            f"❌ {baseline.name} 漏吸收以下增量迁移对象（会让 migrations/ 正向链不完整）:"
        )
        for m in missing:
            print(f"   - {m}")
        print(
    ...
    print(
        f"✅ {baseline.name} 已吸收全部 {len(ts_files)} 个增量迁移的对象；无重复索引/外键。"
    )
    return 0
