...
class MatrixClient:
    ...
    def login(self, username, password, retries=3, backoff_base=2.0):
        ...
        for attempt in range(retries):
            ...
            if status == 429:
                ...
                print(
                    f"  [login] rate-limited (attempt {attempt + 1}/{retries}), "
    ...
    def join_room(self, token, room_id_or_alias):
        body = json.dumps({})
        path = (
            f"/_matrix/client/v3/join/{urllib.parse.quote(room_id_or_alias, safe='')}"
        )
        return self._req("POST", path, token=token, body=body)
    ...
    def set_displayname(self, token, user_id, displayname):
        ...
        path = f"/_matrix/client/v3/profile/{urllib.parse.quote(user_id, safe='')}/displayname"
        return self._req("PUT", path, token=token, body=body)
...
def run_admin_phase(client):
    ...
    ledger = load_ledger()
    admin_eps = [
        e for e in ledger if isinstance(e, dict) and "/admin/" in str(e.get("path", ""))
    ]
...
def run_permission_phase(client, cfg=None):
    ...
    def rec(
        cid, desc, method, path, token_type, expected, status, body, ok, finding_kw=None
    ):
    ...
    rec(
        "C-PER-001",
        f"{user_a_name} creates private room",
        "POST",
        "/createRoom",
        user_a_name,
        "200",
    ...
    rec(
        "C-PER-002",
        f"{user_a_name} sends text to own room",
        "PUT",
        f"/rooms/{room_id}/send/m.room.message/...",
        user_a_name,
        "200",
        s,
        b,
    ...
    rec(
        "C-PER-003",
        f"{user_b_name} reads {user_a_name}'s room without invite",
        "GET",
        ...
        finding_kw=(
            ...
            if ok_t3
                ...
                expected="403 or 404",
                actual=str(s),
                endpoint=f"/rooms/{room_id}/messages",
    ...
    rec(
        "C-PER-004",
        f"{user_b_name} self-invites (joins) {user_a_name}'s private room",
        "POST",
        f"/join/{room_id}",
        user_b_name,
        "403",
    ...
    rec(
        "C-PER-005",
        f"{user_a_name} invites {user_b_name}",
        "POST",
        ...
        finding_kw=(
            ...
            if s == 200
                ...
                expected="200",
                actual=str(s),
                endpoint=f"/rooms/{room_id}/invite",
                auth_context=user_a_name,
            )
        ),
    )
    ...
    rec(
        "C-PER-006",
        f"{user_b_name} joins after invite",
        "POST",
        f"/join/{room_id}",
        user_b_name,
        "200",
    ...
    rec(
        "C-PER-007",
        f"{user_b_name} reads after joining",
        "GET",
        f"/rooms/{room_id}/messages",
        user_b_name,
        "200",
    ...
    rec(
        "C-PER-008",
        f"{user_b_name} (member) tries to kick {user_a_name} (owner)",
        "POST",
    ...
    s, _, b = client._req(
        "GET",
        f"/_synapse/admin/v1/whois/{urllib.parse.quote(target_who, safe='')}",
        token=t1_tok,
    )
    ok_t9 = s == 403
    rec(
        "C-PER-009",
        f"{user_a_name} (non-admin) calls admin whois",
        "GET",
        f"/_synapse/admin/v1/whois/{target_who}",
        user_a_name,
        "403",
        s,
        b,
        ...
        finding_kw=(
            ...
            if ok_t9
                ...
                expected="403",
                actual=str(s),
                endpoint=f"/_synapse/admin/v1/whois/{target_who}",
                auth_context=user_a_name,
    ...
    s, _, b = client._req(
        "GET",
        f"/_synapse/admin/v1/whois/{urllib.parse.quote(target_who_t1, safe='')}",
    )
    ok_t10 = s == 401
    rec(
        "C-PER-010",
        "anonymous calls admin whois",
        "GET",
        f"/_synapse/admin/v1/whois/{target_who_t1}",
        "anonymous",
        "401",
        s,
        b,
        ...
        finding_kw=(
            ...
            if ok_t10
                ...
                expected="401",
                actual=str(s),
                endpoint=f"/_synapse/admin/v1/whois/{target_who_t1}",
    ...
    rec(
        "C-PER-011",
        f"{user_a_name} reads {user_b_name}'s profile",
        "GET",
        f"/profile/{target_t2}",
        user_a_name,
        "200",
    ...
    rec(
        "C-PER-012",
        f"{user_b_name} sets {user_a_name}'s displayname",
        "PUT",
        f"/profile/{target_t1}/displayname",
        user_b_name,
        "403",
        s,
        b,
        ...
        finding_kw=(
            ...
            if ok_t12
                ...
                expected="403",
                actual=str(s),
                endpoint=f"/profile/{target_t1}/displayname",
                auth_context=user_b_name,
    ...
    rec(
        "C-PER-013",
        f"{user_b_name} (member) bans {user_a_name} (owner)",
        "POST",
        f"/rooms/{room_id}/ban",
        user_b_name,
        "403",
...
def run_boundary_phase(client, cfg=None):
    ...
    s, _, b = client._req(
        "GET",
        "/_matrix/client/v3/sync?since=not-a-valid-token!!!@@",
        token=t1_tok,
    )
    rec(
        "C-BND-005",
        "GET /sync invalid since",
        "GET",
        "/sync",
        user_a_name,
        "400",
    ...
    rec(
        "C-BND-006",
        "createRoom with name>255 chars",
        "POST",
        "/createRoom",
        user_a_name,
        "200 or 400",
    ...
    rec(
        "C-BND-012",
        "room_id with null byte",
        "GET",
        "/rooms/!%00inject:matrix.test/messages",
        user_a_name,
        "400 or 404 (sanitized)",
        s,
        b,
        ok_b12,
        finding_kw=(
...
def run_functional_phase(client, cfg=None):
    ...
    s, _, b = client.create_room(
        t1_tok, "public_chat", f"func-room-{room_suffix}", f"func topic {room_suffix}"
    )
    room_id = ""
    if s == 200:
        try:
    ...
    rec(
        "C-FUN-001a",
        f"F1: {user_a_name} creates public room",
        "POST",
        "/createRoom",
        user_a_name,
        "200",
    ...
    rec(
        "C-FUN-001b",
        "F1: full invite->join->send->read flow",
        "MULTI",
        f"/rooms/{room_id}/...",
    ...
    rec(
        ...
        finding_kw=(
            ...
            if f3_pass
                ...
                expected="200 with user_id/devices",
                actual=str(s),
                endpoint=f"/_synapse/admin/v1/whois/{target_who}",
                auth_context="admin",
            )
    ...
    if admin_room:
        ...
        rec(
            "C-FUN-004",
            f"F4: admin kicks {user_a_name}",
            "POST",
            f"/rooms/{admin_room}/kick",
            "admin",
            "200",
            s_kick,
            bkick,
            ...
            finding_kw=(
                ...
                if f4_pass
                    ...
                    expected="200",
                    actual=str(s_kick),
                    endpoint=f"/rooms/{admin_room}/kick",
                    auth_context="admin",
                )
            ),
        )
        ...
        rec(
            "C-FUN-004",
            f"F4: admin kicks {user_a_name} (admin room create failed)",
            "POST",
            "/rooms/.../kick",
            "admin",
            "200",
    ...
    rec(
        "C-FUN-005",
        f"F5: {user_a_name} uploads PNG",
        "POST",
        "/_matrix/media/v3/upload",
        user_a_name,
        "200 with content_uri",
        s,
        b,
        f5_pass,
        finding_kw=(
    ...
    rec(
        "C-FUN-006",
        f"F6: {user_a_name} deletes other device",
        "DELETE",
        "/devices/ANOTHERUSERDEVICE000",
        user_a_name,
        "403 or 401 (UIA)",
        s,
        b,
        ...
        finding_kw=(
            ...
            if f6_pass
                ...
                expected="403/401(UIA)",
                actual=str(s),
                endpoint="/_matrix/client/v3/devices/{device_id}",
                auth_context=user_a_name,
...
def main():
    ...
    report = {
        ...
        "phases": {
            "admin": {
                "cases": sum(1 for c in all_cases if c.phase == "admin"),
                "findings": sum(1 for f in all_findings if f.phase == "admin"),
            },
            "permission": {
                "cases": sum(1 for c in all_cases if c.phase == "permission"),
                "findings": sum(1 for f in all_findings if f.phase == "permission"),
            },
            "boundary": {
                "cases": sum(1 for c in all_cases if c.phase == "boundary"),
                "findings": sum(1 for f in all_findings if f.phase == "boundary"),
            },
            "functional": {
                "cases": sum(1 for c in all_cases if c.phase == "functional"),
