...
run_migrations() {
    ...
    retry 3 5 compose run -T --rm --no-deps -e "ENABLED_EXTENSIONS=${ENABLED_EXTENSIONS}" migrator migrate
    log_success "数据库迁移完成"
    log_info "验证数据库架构完整性..."
    retry 3 5 compose run -T --rm --no-deps -e "ENABLED_EXTENSIONS=${ENABLED_EXTENSIONS}" migrator validate
