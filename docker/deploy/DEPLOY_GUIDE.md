...
> **当前镜像版本**: `vmuser232922/mysynapse:0.1.6-amd64` (linux/amd64)
> **生产 Server Name**: `matrix.test`
> **目标编排**: docker-compose (synapse-rust + postgres-16 + redis-7 + nginx + element-web)

本指南覆盖三种使用场景：

1. **快速部署** —— 已有镜像，只想跑起来：见 [快速部署](#快速部署)。
2. **构建 → 推送 → 部署 一体化** —— 修改了源码或依赖，需要重新出镜像并发到 Docker Hub：见 [构建/推送/部署 完整流程](#构建推送部署-完整流程)。
3. **运维进阶** —— 升级、迁移、故障排查：见 [进阶操作手册](#进阶操作手册实战要点)。
...
SYNAPSE_IMAGE=vmuser232922/mysynapse:0.1.6-amd64
EOF
...
```bash
# 拉取最新镜像
docker pull vmuser232922/mysynapse:0.1.6-amd64
...
适用于「修改了源码 / 依赖 / 迁移脚本，需要出新镜像并发布到 Docker Hub」的场景。
脚本入口为仓库根目录 `build-and-push.sh`。

### 1. 清理本地缓存与旧镜像 (可选但推荐)

避免旧的 buildx 缓存带来「源码改了但镜像没变」的诡异情况。

```bash
# Cargo target/ 与 docker buildx 缓存
cargo clean
docker builder prune -af

# 删除本地旧的 mysynapse tag (不影响正在运行的容器，因容器引用的是镜像 ID)
docker images vmuser232922/mysynapse --format '{{.Repository}}:{{.Tag}}' \
  | xargs -r docker rmi
```

### 2. 构建生产级 AMD64 镜像

构建脚本 (`build-and-push.sh`) 会：

- 通过 `buildx` 在 Apple Silicon / x86 上一致地产出 `linux/amd64`；
- 使用 `target=tools` (debian-slim + libssl3 + postgresql-client + tini)，体积约 200MB，**容器内自治完成 等待 DB → 跑迁移 → schema 校验 → 启动**；
- 把 `migrations/` **全量** SQL（含 `*.undo.sql` 回滚脚本）复制进 `/app/migrations/`，并把 `db_migrate.sh` / `container-migrate.sh` 安装到 `/app/scripts/`；
- 默认打两个 tag：`vmuser232922/mysynapse:<VERSION>-amd64` 与 `vmuser232922/mysynapse:latest`。

```bash
cd synapse-rust

# 默认: VERSION=0.1.6, IMAGE=vmuser232922/mysynapse, target=tools, platform=linux/amd64
./build-and-push.sh

# 仅本地构建，不推送 (用于先冒烟测试)
SKIP_PUSH=1 ./build-and-push.sh

# 自定义版本号
VERSION=0.1.7 ./build-and-push.sh
```

> 构建结束后用 `docker image inspect vmuser232922/mysynapse:0.1.6-amd64 --format '{{.Architecture}}/{{.Os}}'` 应输出 `amd64/linux`。

### 3. 登录并推送到 Docker Hub

`build-and-push.sh` 会自动调 `docker push`，前提是当前 Docker 已登录 vmuser232922 账号。
首次执行（或登出过）须先：
...
> **凭据安全**：不要在脚本里硬编码密码；CI 用 secret，本机用 `docker login` 一次后凭据
> 落入 `~/.docker/config.json`。完成推送后建议 `docker logout`。
...
```bash
# A. /etc/hosts 把域名指到本机 (首次必须)
sudo sh -c 'echo "127.0.0.1 matrix.test element.test" >> /etc/hosts'

# B. 拉新镜像 + 滚动重启 synapse-rust 服务 (其它依赖不重启)
...
`docker-compose.image.yml` 的作用是把 `build:` 重置为 `null`、强制 `pull_policy: always`，
保证完全使用 Docker Hub 上的镜像而不是再触发本地构建。
...
1. **不要覆盖旧 tag** —— 把 `docker/.env` 里的 `SYNAPSE_IMAGE_TAG` 递增（如
   `0.1.5-amd64 → 0.1.6-amd64`），保留回滚通道。
2. **先打包后切换** —— 在主机为 `arm64` 的 macOS 上构建 `linux/amd64` 必须显式
   ...
   docker buildx build \
     ...
     --target tools \
     --tag vmuser232922/mysynapse:0.1.6-amd64 \
...
# 2. 推送两个 tag
docker push vmuser232922/mysynapse:0.1.6-amd64
...
| 设备/SAML 配置改完重启丢失                   | 旧镜像没有 `saml_config_overrides` 表                        | 升级到 `≥0.1.6` 并应用对应迁移                  |
