...
mod db_tests {
    ...
    async fn cleanup_worker(pool: &Arc<PgPool>, worker_id: &str) {
        let _ = sqlx::query(r"DELETE FROM worker_task_assignments WHERE assigned_worker_id = $1")
            ...
            .execute(&**pool)
            .await;
        let _ = sqlx::query(r"DELETE FROM worker_commands WHERE target_worker_id = $1")
            .bind(worker_id)
            .execute(&**pool)
            .await;
        let _ = sqlx::query(r"DELETE FROM replication_positions WHERE worker_id = $1")
            .bind(worker_id)
            .execute(&**pool)
            .await;
    ...
    async fn cleanup_command(pool: &Arc<PgPool>, command_id: &str) {
        let _ =
            sqlx::query(r"DELETE FROM worker_commands WHERE command_id = $1").bind(command_id).execute(&**pool).await;
    }

    async fn cleanup_task(pool: &Arc<PgPool>, task_id: &str) {
        let _ =
            sqlx::query(r"DELETE FROM worker_task_assignments WHERE task_id = $1").bind(task_id).execute(&**pool).await;
    }
    ...
    async fn test_get_workers_by_type() {
        ...
        let workers =
            storage.get_workers_by_type("media_repository").await.expect("get_workers_by_type should succeed");
    ...
    async fn test_claim_next_pending_task_assigns_to_worker() {
        ...
        let claimed =
            storage.claim_next_pending_task(&worker_id).await.expect("claim_next_pending_task should succeed");
    ...
    async fn test_assign_task_to_worker_succeeds_for_pending() {
        ...
        let ok = storage
            .assign_task_to_worker(&task.task_id, &worker_id)
            .await
