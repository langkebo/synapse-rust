...
impl ApplicationServiceStorage {
    ...
    async fn clear_namespaces(&self, as_id: &str) -> Result<(), sqlx::Error> {
        sqlx::query!(r"DELETE FROM application_service_user_namespaces WHERE as_id = $1", as_id)
            .execute(&*self.pool)
            .await?;
        sqlx::query!(r"DELETE FROM application_service_room_alias_namespaces WHERE as_id = $1", as_id)
            .execute(&*self.pool)
            .await?;
        sqlx::query!(r"DELETE FROM application_service_room_namespaces WHERE as_id = $1", as_id)
            .execute(&*self.pool)
            .await?;
    ...
    pub async fn get_by_id(&self, as_id: &str) -> Result<Option<ApplicationService>, sqlx::Error> {
        sqlx::query_as!(
            ApplicationService,
            r#"SELECT id, as_id, url, as_token, hs_token, sender_localpart,
                is_enabled AS "is_enabled!", is_rate_limited AS "is_rate_limited!",
                protocols AS "protocols!", namespaces AS "namespaces!",
                created_ts, updated_ts, description, api_key, config FROM application_services WHERE as_id = $1"#,
            as_id,
        )
    ...
    pub async fn get_by_token(&self, as_token: &str) -> Result<Option<ApplicationService>, sqlx::Error> {
        sqlx::query_as!(
            ...
            r#"SELECT id, as_id, url, as_token, hs_token, sender_localpart,
                is_enabled AS "is_enabled!", is_rate_limited AS "is_rate_limited!",
                protocols AS "protocols!", namespaces AS "namespaces!",
                created_ts, updated_ts, description, api_key, config FROM application_services WHERE as_token = $1 AND is_enabled = TRUE"#,
            as_token,
        )
        .fetch_optional(&*self.pool)
        .await
    ...
    pub async fn get_by_hs_token(&self, hs_token: &str) -> Result<Option<ApplicationService>, sqlx::Error> {
        sqlx::query_as!(
            ...
            r#"SELECT id, as_id, url, as_token, hs_token, sender_localpart,
                is_enabled AS "is_enabled!", is_rate_limited AS "is_rate_limited!",
                protocols AS "protocols!", namespaces AS "namespaces!",
                created_ts, updated_ts, description, api_key, config FROM application_services WHERE hs_token = $1 AND is_enabled = TRUE"#,
            hs_token,
        )
        .fetch_optional(&*self.pool)
        .await
    ...
    pub async fn get_all_active(&self) -> Result<Vec<ApplicationService>, sqlx::Error> {
        sqlx::query_as!(
            ...
            r#"SELECT id, as_id, url, as_token, hs_token, sender_localpart,
                is_enabled AS "is_enabled!", is_rate_limited AS "is_rate_limited!",
                protocols AS "protocols!", namespaces AS "namespaces!",
                created_ts, updated_ts, description, api_key, config FROM application_services WHERE is_enabled = TRUE ORDER BY created_ts DESC, id DESC"#,
        )
        .fetch_all(&*self.pool)
        .await
    }
    ...
    pub async fn get_pending_transactions(
        ...
        sqlx::query_as!(
            ApplicationServiceTransaction,
            r#"SELECT id, as_id, txn_id, transaction_id, events AS "events!", sent_ts, completed_ts, retry_count, last_error
    ...
    pub async fn register_virtual_user(
        ...
        sqlx::query_as!(
            ...
            RETURNING as_id, user_id, displayname, avatar_url, created_ts
            "#,
            as_id,
            user_id,
            displayname,
    ...
    pub async fn get_virtual_users(&self, as_id: &str) -> Result<Vec<ApplicationServiceUser>, sqlx::Error> {
        sqlx::query_as!(
            ApplicationServiceUser,
            r"SELECT as_id, user_id, displayname, avatar_url, created_ts FROM application_service_users WHERE as_id = $1",
            as_id,
        )
        .fetch_all(&*self.pool)
        .await
    ...
    pub async fn get_statistics(&self) -> Result<Vec<serde_json::Value>, sqlx::Error> {
        sqlx::query!(
            ...
            SELECT
                ...
                svc.is_enabled AS "is_enabled!",
                svc.is_rate_limited AS "is_rate_limited!",
