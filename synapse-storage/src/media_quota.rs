...
mod tests {
    ...
    // -------------------------------------------------------------------------
    // Appended tests (v2 plan A10): QuotaCheckResult transitions, MediaQuotaAlert
    // timestamp logic, MediaUsageLog construction, serde roundtrips.
    // -------------------------------------------------------------------------

    #[test]
    fn test_quota_check_result_serde_roundtrip() {
        let result = QuotaCheckResult {
            is_allowed: true,
            reason: None,
            current_usage: 0,
        ...
        // Verify the `#[serde(rename = "allowed")]` field actually serializes
        // under the JSON key `allowed`, not `is_allowed`.
        let json = serde_json::to_string(&result).expect("serialize QuotaCheckResult");
        let value: serde_json::Value = serde_json::from_str(&json).expect("parse json");
        assert!(value.get("allowed").is_some(), "field should serialize as `allowed`");
        assert!(value.get("is_allowed").is_none(), "field should NOT serialize as `is_allowed`");

        let restored: QuotaCheckResult = serde_json::from_str(&json).expect("deserialize");
        assert!(restored.is_allowed);
        assert_eq!(restored.quota_limit, 1_073_741_824);
    }

    #[test]
    fn test_quota_check_result_blocked_above_100_percent() {
        let result = QuotaCheckResult {
            is_allowed: false,
            reason: Some("Storage quota exceeded".to_string()),
            current_usage: 2_000_000_000,
            quota_limit: 1_000_000_000,
            usage_percent: 200.0,
        };
        assert!(!result.is_allowed);
        assert!(result.usage_percent > 100.0);
        assert!(result.current_usage > result.quota_limit);
    }

    #[test]
    fn test_quota_check_result_boundary_at_zero_usage() {
        let result = QuotaCheckResult {
            is_allowed: true,
            reason: None,
            current_usage: 0,
    ...
    #[test]
    fn test_media_quota_alert_is_read_transition() {
        let mut alert = create_test_media_alert();
        assert!(!alert.is_read, "fresh alert should be unread");
        // Mark as read.
        alert.is_read = true;
        assert!(alert.is_read, "alert should be marked as read");
    }

    #[test]
    fn test_media_quota_alert_threshold_logic() {
        let alert = create_test_media_alert();
        // Compute the actual usage ratio represented by the alert record.
        let usage_ratio = alert.current_usage_bytes as f64 / alert.quota_limit_bytes as f64 * 100.0;
        // The fixture is calibrated to ~80% (0.8 GiB / 1.0 GiB). Verify the
        // ratio is at the threshold boundary (within a small tolerance).
        assert!(
            (usage_ratio - alert.threshold_percent as f64).abs() < 1.0,
            "fixture usage ratio should be at the configured threshold, got {usage_ratio}%"
        );
        assert_eq!(alert.threshold_percent, 80);
        assert!(alert.current_usage_bytes < alert.quota_limit_bytes, "alert below hard limit");
    }

    #[test]
    fn test_media_usage_log_construction() {
        let log = MediaUsageLog {
            id: 1,
            user_id: "@user:example.com".to_string(),
            media_id: "media-abc123".to_string(),
            file_size_bytes: 1_048_576,
            mime_type: Some("image/png".to_string()),
            operation: "upload".to_string(),
            timestamp: Utc::now().timestamp_millis(),
        };
        assert_eq!(log.media_id, "media-abc123");
        assert_eq!(log.operation, "upload");
        assert_eq!(log.mime_type.as_deref(), Some("image/png"));
    }

    #[test]
    fn test_media_usage_log_serde_roundtrip() {
        let log = MediaUsageLog {
            id: 42,
        ...
        let json = serde_json::to_string(&log).expect("serialize MediaUsageLog");
        let restored: MediaUsageLog = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(restored.id, 42);
        assert_eq!(restored.operation, "delete");
        assert!(restored.mime_type.is_none());
    }

    #[test]
    fn test_media_quota_config_serde_roundtrip() {
        let config = create_test_quota_config();
        let json = serde_json::to_string(&config).expect("serialize MediaQuotaConfig");
        let restored: MediaQuotaConfig = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(restored.name, config.name);
        assert_eq!(restored.max_storage_bytes, config.max_storage_bytes);
        assert_eq!(restored.allowed_mime_types, config.allowed_mime_types);
        assert!(restored.is_default);
    }

    #[test]
    fn test_server_media_quota_with_no_limits() {
        // A server with no explicit limits (uses config defaults).
        let quota = ServerMediaQuota {
            id: 1,
        ...
        assert!(quota.max_file_size_bytes.is_none());
        assert!(quota.max_files_count.is_none());
        assert_eq!(quota.current_storage_bytes, 0);
    }

    #[test]
    fn test_user_media_quota_with_custom_overrides() {
        let quota = UserMediaQuota {
            id: 1,
            user_id: "@vip:example.com".to_string(),
            quota_config_id: Some(2),
            custom_max_storage_bytes: Some(10_737_418_240), // 10 GiB override
            custom_max_file_size_bytes: Some(524_288_000),
            custom_max_files_count: Some(5000),
            current_storage_bytes: 1_073_741_824,
            current_files_count: 100,
            created_ts: 1_700_000_000_000,
            updated_ts: Some(1_700_000_000_500),
        };
        assert!(quota.custom_max_storage_bytes.is_some());
        assert!(quota.custom_max_storage_bytes.unwrap() > quota.current_storage_bytes);
        assert_eq!(quota.updated_ts, Some(1_700_000_000_500));
    }
}
