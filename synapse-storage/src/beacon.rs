...
/// The `CreateBeaconInfoParams` struct.
...
/// The `CreateBeaconLocationParams` struct.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl BeaconStorage {
    ...
    pub async fn create_beacon_info(&self, params: CreateBeaconInfoParams) -> Result<BeaconInfo, sqlx::Error> {
        ...
        let row = sqlx::query_as!(
            ...
            RETURNING id, room_id, event_id, state_key, sender, description,
                      timeout, is_live, asset_type, created_ts, updated_ts, expires_at
            "#,
            params.room_id,
            params.event_id,
    ...
    /// See [`get_beacon_info_by_state_key`].
    ...
    pub async fn create_beacon_location(
        ...
        let row = sqlx::query_as!(
            ...
            RETURNING id, room_id, event_id, beacon_info_id, sender, uri,
                      description, timestamp, accuracy, created_ts
            "#,
            params.room_id,
            params.event_id,
    ...
    async fn update_beacon_expiry(&self, beacon_info_id: &str, location_ts: i64) -> Result<(), sqlx::Error> {
        let beacon_info = sqlx::query_as!(
            BeaconInfo,
            r#"
            SELECT id, room_id, event_id, state_key, sender, description, timeout, is_live, asset_type, created_ts, updated_ts, expires_at
            FROM beacon_info
            WHERE event_id = $1
            "#,
            beacon_info_id,
    ...
    /// See [`count_locations_in_room_by_sender_since`].
