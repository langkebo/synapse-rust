...
pub async fn register_guest(State(state): State<AppState>) -> Result<Json<Value>, ApiError> {
    ...
    let user = UserStorage::create_user(&state.services.account.user_storage, &user_id, &username, None, false)
        .await
...
pub async fn get_guest_info(
    ...
    let user = UserStorage::get_user_by_id(&state.services.account.user_storage, &auth_user.user_id)
        .await
...
pub async fn upgrade_guest(
    ...
    match user {
        Some(u) if u.is_guest => {
            let existing = UserStorage::get_user_by_username(&state.services.account.user_storage, username)
                .await
...
pub fn guest_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
