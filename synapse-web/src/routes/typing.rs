...
use crate::routes::{ensure_room_member_strict, ApiError, AppState, AuthenticatedUser};
use axum::{
    extract::{Path, State},
    ...
    if is_typing {
        ...
        let _ = state
            ...
            .broadcast_edu_to_room(
                &room_id,
                &edu,
                state.services.core.config.server.server_name.as_deref().unwrap_or("localhost"),
            )
            .await;
...
pub fn typing_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
