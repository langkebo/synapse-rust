...
use synapse_services::{AdminUserCursor, AdminUserService};
...
fn admin_user_service(state: &AppState) -> AdminUserService {
    AdminUserService::new(
        state.services.account.user_storage.pool.clone(),
        state.services.account.user_storage.clone(),
        state.services.account.device_storage.clone(),
        state.services.rooms.room_storage.clone(),
        state.services.rooms.member_storage.clone(),
        state.services.core.config.server.name.clone(),
    )
}
...
pub async fn get_users_v2(
    ...
    let admin_user_service = admin_user_service(&state);

    let page = admin_user_service
        .list_users_v2(
            limit,
            cursor.map(|(created_ts, user_id)| AdminUserCursor { created_ts, user_id: user_id.to_owned() }),
...
pub async fn get_user_v2(
    ...
    let admin_user_service = admin_user_service(&state);
    let user = admin_user_service.get_user_v2(&user_id).await?;
...
pub async fn get_user_stats(_admin: AdminUser, State(state): State<AppState>) -> Result<Json<Value>, ApiError> {
    let admin_user_service = admin_user_service(&state);
    let stats = admin_user_service.get_user_stats().await?;
...
pub async fn get_single_user_stats(
    ...
    let admin_user_service = admin_user_service(&state);
    let stats = admin_user_service.get_single_user_stats(&user_id).await?;
...
pub async fn batch_create_users(
    ...
    let users: Vec<(String, String, Option<String>, bool)> = body
        ...
        .collect();
    let admin_user_service = admin_user_service(&state);
    let result = admin_user_service.batch_create_users(&users).await?;
...
pub async fn batch_deactivate_users(
    ...
    let admin_user_service = admin_user_service(&state);
    let result = admin_user_service.batch_deactivate_users(&body.users).await?;
