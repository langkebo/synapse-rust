...
use crate::routes::extractors::ServerName;
...
pub fn create_federation_router() -> Router<crate::routes::AppState> {
    Router::new()
...
mod cursor_tests {
    ...
    fn test_pending_federation_cursor_round_trip() {
        let cursor = encode_pending_federation_cursor(&synapse_services::admin::PendingFederationCursor {
            updated_ts: 1_700_000_000_000,
            server_name: "matrix.example.com".to_string(),
        });
        assert_eq!(
            decode_pending_federation_cursor(Some(&cursor)),
            Some(synapse_services::admin::PendingFederationCursor {
                updated_ts: 1_700_000_000_000,
                server_name: "matrix.example.com".to_string(),
            })
        );
...
) -> Result<(i32, Option<synapse_services::admin::DestinationCursor>), ApiError> {
