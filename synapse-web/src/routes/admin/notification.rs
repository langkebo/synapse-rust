...
pub fn admin_notification_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
...
pub async fn send_server_notice(
    ...
    let room_id = format!("!server_notice_{}:{}", uuid::Uuid::new_v4(), state.services.core.config.server.name);
    let now = chrono::Utc::now().timestamp_millis();
    let server_user = format!("@server:{}", state.services.core.config.server.name);
    let message_event_id = format!("${}:{}", uuid::Uuid::new_v4(), state.services.core.config.server.name);
    let create_event_id = format!("${}:{}", uuid::Uuid::new_v4(), state.services.core.config.server.name);
    let membership_event_id = format!("${}:{}", uuid::Uuid::new_v4(), state.services.core.config.server.name);
