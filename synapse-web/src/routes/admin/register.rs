...
pub fn admin_register_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
...
fn verify_mac(
    ...
    let Ok(provided) = synapse_common::crypto::decode_hex(mac_hex) else {
        return false;
    };
...
async fn register(
    ...
    let auth_service = AuthService::new_with_lifetime(
        ...
        state.cache.clone(),
        state.services.core.metrics.clone(),
        &config.security,
        &config.server.name,
