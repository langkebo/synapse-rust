...
pub fn admin_server_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
...
pub async fn get_admin_whoami(admin: AdminUser, State(state): State<AppState>) -> Result<Json<Value>, ApiError> {
    Ok(Json(json!({
        ...
        "name": format!("@{}:{}", admin.user_id, state.services.core.config.server.name),
...
pub async fn get_status(_admin: AdminUser, State(state): State<AppState>) -> Result<Json<Value>, ApiError> {
    let db_ok = sqlx::query("SELECT 1").fetch_one(&*state.services.account.user_storage.pool).await.is_ok();
