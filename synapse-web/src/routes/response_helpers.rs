...
pub(crate) async fn filter_users_with_shared_rooms(
    ...
    for user_id in requested_users {
        ...
        let shared = state.services.rooms.member_storage.share_common_room(current_user_id, user_id).await.unwrap_or(false);
...
mod tests {
    ...
    fn test_require_found_returns_not_found_error() {
        ...
        match error {
            e if e.is_not_found() => assert!(e.message().contains("missing")),
