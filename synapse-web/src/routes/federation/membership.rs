...
pub(super) async fn knock_room(
    ...
    let event_id = format!("${}", synapse_common::crypto::generate_event_id(&state.services.core.server_name));
...
pub(super) async fn get_user_devices(
    ...
    let cross_signing_storage =
        synapse_e2ee::cross_signing::CrossSigningStorage::new(&state.services.account.device_storage.pool);
    let master_key = cross_signing_storage
        .get_cross_signing_key(&user_id, "master")
        .await
        .map_err(|e| ApiError::internal_with_log("Failed to get master key", &e))?
        .and_then(|key| key.key_json);
    let self_signing_key = cross_signing_storage
