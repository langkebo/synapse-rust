...
async fn openid_userinfo(State(state): State<AppState>, Query(params): Query<Value>) -> Result<Json<Value>, ApiError> {
    ...
    let openid_storage = synapse_storage::OpenIdTokenStorage::new(&state.services.account.user_storage.pool);
...
pub fn create_federation_router(state: AppState) -> Router<AppState> {
    ...
    let protected =
        protected.layer(middleware::from_fn_with_state(state, crate::middleware::federation_auth_middleware));
...
pub fn federation_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
