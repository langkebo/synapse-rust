...
pub(super) async fn key_query(
    ...
    if server_name == state.services.core.server_name || server_name == state.services.core.config.federation.server_name {
...
pub(super) async fn keys_claim(
    ...
    let mut request: synapse_e2ee::device_keys::KeyClaimRequest =
...
pub(super) async fn keys_query(
    ...
    let mut request: synapse_e2ee::device_keys::KeyQueryRequest =
...
async fn resolve_server_keys(state: &AppState) -> Result<Value, ApiError> {
        ...
        .map_err(|e| ApiError::internal_with_log("Failed to load federation signing key", &e))?
    {
        return state
            ...
            .or_else(|_| {
                ...
                if let Err(e) = synapse_federation::signing::sign_json(
                    &config.server_name,
                    &key_id_for_sign,
                    &secret_key_for_sign,
                    &mut response,
    ...
    if let Some(secret_key) = config.signing_key.as_deref() {
        if let Err(e) =
            synapse_federation::signing::sign_json(&config.server_name, &key_id_for_sign, secret_key, &mut response)
        {
