...
async fn sso_redirect(
    ...
    if state.services.sso.saml_service.is_enabled() {
        let auth_request = state.services.sso.saml_service.get_auth_redirect(Some(&redirect_uri)).await?;
...
pub fn oidc_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
...
pub fn oidc_fallback_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
    use crate::routes::route_ledger::RouteEntry;
    use axum::http::Method;
...
pub fn oidc_route_manifest_for(state: &AppState) -> Vec<crate::routes::route_ledger::RouteEntry> {
...
async fn oidc_userinfo(
    ...
    let picture = profile.get("avatar_url").and_then(|v| v.as_str()).map(|s| {
        if let Ok(loc) = synapse_common::media_locator::MediaLocator::parse(s) {
            loc.to_mxc_url()
    ...
    match grant_type.as_str() {
        "authorization_code" => {
            ...
            let matrix_user_id = if let Some(existing) = bound_user_id {
                ...
                if state.services.account.user_storage.get_user_by_id(&matrix_user_id).await.unwrap_or(None).is_some() {
                    ::tracing::warn!(
...
async fn oidc_callback(
    ...
    let user_id = format!("@{}:{}", oidc_user.localpart, state.services.core.server_name);
