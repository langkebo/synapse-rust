...
pub async fn configure_key_rotation(
    ...
    Ok(Json(json!({
        "enabled": status.get("rotation_enabled"),
        "interval_ms": persisted_interval_ms.unwrap_or(state.services.core.config.federation.key_rotation_grace_period_ms as i64),
...
pub fn key_rotation_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
