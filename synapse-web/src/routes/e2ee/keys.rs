...
async fn query_keys(
    ...
    let device_keys = if requested_users.is_empty() {
        let mut shared =
            ctx.room_service.membership().get_shared_room_users(&auth_user.user_id).await.map_err(|e| {
                synapse_common::error::ApiError::internal_with_cause("Failed to load shared room users", e)
            })?;
    ...
    if let Some(device_keys_obj) = merged_device_keys.as_object() {
        ...
        if !user_ids.is_empty() {
            let batch_result = ctx.cross_signing_service.get_verified_devices_batch(&user_ids).await.map_err(|e| {
                synapse_common::error::ApiError::internal_with_cause("Failed to load verified devices batch", e)
            })?;
...
async fn claim_keys(
    ...
    let request: synapse_e2ee::device_keys::KeyClaimRequest = serde_json::from_value(body)
        .map_err(|e| synapse_common::error::ApiError::bad_request(format!("Invalid request: {e}")))?;
