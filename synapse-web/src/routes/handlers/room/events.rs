...
use crate::routes::{validate_event_id, validate_room_id, AppState, AuthenticatedUser};
...
pub(crate) async fn get_room_message_queue(
    ...
    let processing_count =
        state.services.rooms.event_storage.count_room_events_by_status(&room_id, "processing").await.unwrap_or(0);

    let failed_count = state.services.rooms.event_storage.count_room_events_by_status(&room_id, "failed").await.unwrap_or(0);
...
pub(crate) async fn translate_room_event(
    ...
    let event = get_room_event(&state.services.rooms.event_storage, &room_id, &event_id).await?;
    ...
    let translation_result =
        state.services.extensions.translation_service.translate(text_to_translate, target_lang, source_lang).await.map_err(
            |e| {
...
pub(crate) async fn redact_event(
    ...
    let new_event_id = synapse_common::crypto::generate_event_id(&state.services.core.server_name);
