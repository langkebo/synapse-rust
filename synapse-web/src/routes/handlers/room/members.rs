...
pub(crate) async fn kick_user(
    ...
    state.services.core.auth_service.can_kick_user(&room_id, &auth_user.user_id, target).await?;

    let _kicked_by = auth_user.user_id.clone();
    let event_id = synapse_common::crypto::generate_event_id(&state.services.core.server_name);
    let content = json!({
        "membership": "leave",
...
pub(crate) async fn ban_user(
    ...
    state.services.core.auth_service.can_ban_user(&room_id, &auth_user.user_id, target).await?;
...
pub(crate) async fn unban_user(
    ...
    state.services.core.auth_service.can_unban_user(&room_id, &auth_user.user_id, target).await?;
