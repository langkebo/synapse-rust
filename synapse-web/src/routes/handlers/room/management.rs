...
pub(crate) async fn room_initial_sync(
    ...
    let from = params
        .get("from")
        .and_then(|value| synapse_common::parse_stream_token(value).or_else(|| value.parse::<i64>().ok()))
        .unwrap_or(0);
...
pub(crate) async fn get_retention_policy(
    ...
    let server_policy = state.services.admin.retention_storage.get_server_policy().await.ok();
    ...
    match room_policy {
        ...
        None => {
            let default = server_policy.unwrap_or(synapse_storage::retention::ServerRetentionPolicy {
                id: 0,
                max_lifetime: None,
                min_lifetime: 0,
