...
pub fn search_route_manifest() -> Vec<crate::routes::route_ledger::RouteEntry> {
...
async fn search_room_events(
    ...
    let filter = search.filter.as_ref().map(|filter| synapse_services::RoomEventsSearchFilter {
...
async fn timestamp_to_event(
    ...
    let direction = if dir == "b" {
        synapse_services::TimestampDirection::Backward
    } else {
        synapse_services::TimestampDirection::Forward
    };
...
async fn get_event_context(
    ...
    crate::routes::validate_room_id(&room_id)?;
    crate::routes::validate_event_id(&event_id)?;
