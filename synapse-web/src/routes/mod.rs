...
pub(crate) use auth_compat::{
    auth_fallback_web, check_username_availability, get_login_flows, get_register_flows, login, logout, logout_all,
...
pub(crate) use handlers::room::{
    ...
    get_joined_members, get_joined_rooms, get_membership_events, get_messages, get_my_rooms, get_power_levels,
    get_receipts, get_retention_policy, get_room_account_data, get_room_capabilities, get_room_encrypted_events,
    ...
    kick_user, knock_room, leave_room, put_power_levels, put_state_event, put_state_event_empty_key,
    ...
    sign_room_event, translate_room_event, translate_text, unban_user, upgrade_room, verify_room_event,
};
pub use handlers::{
    get_capabilities, get_client_versions, get_server_version, get_well_known_client, get_well_known_server,
    get_well_known_support, health_check,
};
...
mod top_level_router_tests {
    ...
    fn test_top_level_router_keeps_version_boundaries() {
        ...
        assert!(capabilities_paths.iter().all(|path| !path.contains("/versions")));
        assert!(media_config_paths.iter().all(|path| path.starts_with("/media/")));
        assert!(direct_only_paths.iter().all(|path| !path.ends_with("/capabilities")));
    }
}
...
mod room_router_tests {
    ...
    fn test_room_routes_structure() {
        ...
        assert!(r0_v3_compat_routes.iter().all(|route| route.starts_with("/_matrix/client/")));
...
mod presence_router_tests {
    ...
    fn test_presence_compat_router_contains_shared_paths() {
        ...
        assert!(shared_paths.iter().all(|path| path.starts_with("/presence/")));
    }
    ...
    fn test_presence_router_keeps_presence_list_outside_compat_scope() {
        ...
        assert!(compat_paths.iter().all(|path| !path.ends_with("/presence/list")));
        assert!(v3_only_paths.iter().all(|path| path.ends_with("/presence/list")));
    }
}
