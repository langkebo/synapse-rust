...
pub async fn rate_limit_middleware(State(state): State<AppState>, request: Request<Body>, next: Next) -> Response {
    ...
    let (endpoint_id, per_second, burst_size) = match &file_config {
        Some(fc) => {
            let (id, r) = synapse_common::rate_limit_config::select_endpoint_rule(fc, path);
            (id, r.per_second, r.burst_size)
        }
        None => {
            let (id, r) = synapse_common::rate_limit_config::select_endpoint_rule_runtime(&config, path);
            (id, r.per_second, r.burst_size)
        }
    };
...
mod tests {
    ...
    fn test_select_endpoint_rule() {
        ...
        let (id, rule) =
            synapse_common::rate_limit_config::select_endpoint_rule_runtime(&config, "/_matrix/client/r0/login");
        assert_eq!(id, "/_matrix/client/r0/login");
        assert_eq!(rule.per_second, 5);

        let (id, rule) = synapse_common::rate_limit_config::select_endpoint_rule_runtime(
            &config,
        ...
        let (id, rule) =
            synapse_common::rate_limit_config::select_endpoint_rule_runtime(&config, "/_matrix/client/versions");
        assert_eq!(id, "/_matrix/client");
        assert_eq!(rule.per_second, 50);

        let (id, rule) = synapse_common::rate_limit_config::select_endpoint_rule_runtime(&config, "/other/path");
