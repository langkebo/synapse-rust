...
fn canonical_federation_request_bytes(
    ...
    let result =
        synapse_federation::signing::canonical_federation_request_bytes(method, uri, origin, destination, content);
...
fn verify_server_keys_signature(body: &Value, origin: &str, key_id: &str, verify_key: &str) -> bool {
    ...
    let canonical = synapse_federation::signing::canonical_json_string(&unsigned);
