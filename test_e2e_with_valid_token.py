#!/usr/bin/env python3
"""
Synapse Rust 端到端加密API测试脚本（使用有效token）
"""
...
def test_e2e_api(user_id, token, room_id):
    """测试端到端加密API"""
    headers = {"Authorization": f"Bearer {token}"}
    ...
    data = {
        "device_keys": {
            ...
            "keys": {
                "curve25519:test_device": "test_key",
                "ed25519:test_device": "test_key"
            }
        }
    }
    response = requests.post(url, json=data, headers=headers)
...
def main():
    ...
    # 运行测试
    test_e2e_api(user_id, token, room_id)
