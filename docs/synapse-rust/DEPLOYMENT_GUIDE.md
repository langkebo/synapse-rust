...
| 组件 | 状态 | 说明 |
|------|------|------|
| OIDC Service | ⚠️ 可选能力 | `src/services/oidc_service.rs`，需配置启用 |
| OIDC 路由 | ⚠️ 可选能力 | 需在配置中启用后暴露 |
| OIDC 配置 | ✅ 已实现 | `src/common/config.rs` |

> **注意**：OIDC/SAML/SSO 属于可选企业认证能力，不计入核心主承诺。需要外部身份提供商配置，默认不启用。
