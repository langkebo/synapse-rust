...
- CI 脚本 `scripts/run_ci_tests.sh` 已配置 `TEST_THREADS=2` 作为稳定配置
- 验证结果：`--test-threads=2` 时 337 个测试全部通过（432 秒），`--test-threads=4` 时出现 4 个间歇性失败（296 秒）

**根本原因**：
- 测试共享全局数据库连接池
- 高并发时连接池配置不足以支撑所有并行测试
- 部分测试持有连接时间较长（如 presence 测试）
- 每个测试创建独立 schema 并初始化迁移，在 4 线程并发时导致数据库初始化超时（30 秒）
- 实测数据：2 线程时所有测试通过（432 秒），4 线程时 4 个测试失败（296 秒，但不可靠）
