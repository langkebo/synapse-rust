...
| B4-1c 🆕 | **剩 4 个转换**（`InviteBlocklist`/`Module`/`RendezvousMessage`/`EmailVerification`）：唯一阻塞点是它们唯一的 `dyn` 消费者在 `src/web/routes/context.rs` | 等 `src/web/routes/**` codemod 落地后一次性完成 |
