...
| B4-4-迁移 🔶 | **两批共消除 30/49 个文件**（49 → 38 → **19**）。手法固定为**依赖方向反转**：服务层 `pub use` 重导出 HTTP 层需要的 DTO / 游标编解码函数，`src/web` 里的 `synapse_storage::<mod>::X` 改写为 `synapse_services::<host>::X`；**不**重导出 storage 的 impl struct 或 store trait。第 1 批（`6ab3d1be`）消掉 event / audit DTO（11 个文件）；第 2 批消掉 `user` / `event_report` / `widget` / `thread` / `saml` / `application_service` / `space` / `beacon` / `federation_blacklist`(游标) / `registration_token`(游标) / `push_notification`(DTO) / `sticky_event` / `room_tag`（19 个文件，含 `space.rs` 系列 4 个、`widget.rs`、`tags.rs`、`saml.rs` 等） | `grep -rl synapse_storage src/web --include='*.rs' \| wc -l` = **19**，与 gate 白名单条目数**逐一相等**；注入探针文件后 gate EXIT=1（本轮复验）；`cargo test --test unit --features test-utils` = 1821 passed / 0 failed |
| B4-4-剩余 ⚠️ | **19 个文件仍未下沉**，剩下的全是**真正的 storage 句柄 / store trait**（不是 DTO），需要单独设计而非重导出：`rendezvous`(`RendezvousMessageStorage`/`RendezvousStoreApi`)、`delayed_events`(`DelayedEventStorageApi`)、`oidc_session_storage`、`login_token`、`module`(`ModuleStorage`)、`sliding_sync`、`server_notification`、`email_verification`、`device`、`presence`、`retention`、`feature_flags`、`background_update`、`invite_blocklist`、`oidc_user_mapping`、`token`、`room`(搜索游标 `RoomSearchCursor`)、以及 DTO 与 trait 混用的 `event_report.rs` / `app_service.rs` / `push_notification.rs` | 同 B4-4 门禁；每下沉一个文件从白名单删一行 |
