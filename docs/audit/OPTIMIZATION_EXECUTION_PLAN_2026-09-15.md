...
| B4-4-迁移 🔶 | **三批共消除 42/49 个文件**（49 → 38 → 19 → **7**）。手法固定为**依赖方向反转**：服务层 `pub use` 重导出 HTTP 层需要的 DTO / 游标编解码函数，`src/web` 里的 `synapse_storage::<mod>::X` 改写为 `synapse_services::<host>::X`；**不**重导出 storage 的 impl struct 或 store trait。第 1 批（`6ab3d1be`）消掉 event / audit DTO（11 个）；第 2 批（`91cfd3f0`）消掉 `user`/`event_report`/`widget`/`thread`/`saml`/`application_service`/`space`/`beacon`/`federation_blacklist`/`registration_token`/`push_notification`(DTO)/`sticky_event`/`room_tag`（19 个）；第 3 批消掉 `server_notification`/`retention`/`room`(搜索游标)/`sliding_sync`/`feature_flags`/`application_service`(剩余)/`background_update`/`event_report`(剩余)/`module`/`push_notification`(剩余)/audit(根路径)（12 个） | `grep -rl synapse_storage src/web --include='*.rs' \| wc -l` = **7**，与 gate 白名单条目数**逐一相等**；注入探针文件后 gate EXIT=1（本轮复验）；`cargo test --test unit --features test-utils` = 1821 passed / 0 failed |
| B4-4-剩余 ⚠️ | **7 个文件仍未下沉**，全部是**真正的 storage 句柄 / store trait**（DTO 层面的搬运已经做完了），必须靠新建服务或改 DI 面，不能靠重导出：`context.rs`（路由 state 直接持有 15 个 storage 句柄，见 A1/B4-5）、`qr_login_token.rs`(`LoginTokenStoreApi`)、`oidc/mod.rs`(`OidcSessionStoreApi`+`OidcAuthSession`)、`delayed_events.rs` + `handlers/room/events.rs`(`DelayedEventStorageApi`/`DelayedEventAction`/`CreateDelayedEventRequest`，领域内**没有** service，只有 `wiring/admin.rs` 里注入)、`rendezvous.rs` + `msc4108_rendezvous.rs`(`RendezvousStoreApi`/`RendezvousMessageStorage`，同上无 service) | 同 B4-4 门禁；每下沉一个文件从白名单删一行 |
