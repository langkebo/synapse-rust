...
| B4-4-剩余 ⚠️ | **5 个文件仍未下沉**，全部是**真正的 storage 句柄 / store trait**：`context.rs`（路由 state 直接持有 storage 句柄，见 A1/B4-5）、`qr_login_token.rs`(`LoginTokenStoreApi`)、`oidc/mod.rs`(`OidcSessionStoreApi`+`OidcAuthSession`)、`rendezvous.rs` + `msc4108_rendezvous.rs`(`RendezvousStoreApi`/`RendezvousMessageStorage`，领域内**没有** service) | 同 B4-4 门禁；每下沉一个文件从白名单删一行 |
