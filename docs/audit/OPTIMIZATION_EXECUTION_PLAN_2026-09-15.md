...
| B4-5b ⏳(待开) | **`src/web` 独立成 crate —— 已完成可行性测绘，未动手**。实测：根 crate `src/` 共 **67,457** 行，其中 `src/web` **59,544 行 / 157 文件**（拆出后根仍 ~8k 行，故须连带搬 `federation`(1297)/`e2ee`(866)/`common`(1067) 才能 <5k）。**关键测绘结论**：① `crate::web` 在 web 之外只有 **3 个文件**引用（`server/mod.rs`、`server/router.rs`、`federation/edu.rs`）；② `crate::federation` 在 web 之外 **0 引用**，且 `federation/edu.rs`(1271 行) 反向依赖 `web::routes::context::FederationContext` → **web 与 federation 胶水必须同批搬**；③ `src/e2ee` 866 行里 **753 行是 interop 测试**，其余是 2 行/个的薄壳，web 对它的 46 处引用可直接改指 `synapse_e2ee::*`；④ 真正的硬点是 **feature 透传**（saml-sso/cas-sso/builtin-oidc/server-notifications/friends/… 需要在新 crate 重新声明并在根 crate 转发），以及 `src/common` 薄壳里 3 处真实代码（`error` 的两个映射函数、`health::CacheHealthCheck`、`logging::init_logging`——后者依赖 web 的 `RequestIdPropagationLayer`，必须落在新 crate）。**建议作为独立一轮执行**：单批 157 文件搬移 + 全量 import 重写 + Cargo feature 矩阵，中途失败会留下无法编译的树 | 完成判据：`cargo check --workspace --all-features` = 0；根 crate <5k 行；`scripts/ci/check_web_layering.py` 白名单 **0 条目并删除该文件**；docker 构建矩阵（本机无 Docker，需在 CI 侧确认） |
