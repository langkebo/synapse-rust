...
| B5-3 | **S-12（D3）**：MSC4108 `DELETE` 204 补 `Last-Modified`/`Cache-Control: no-store`/`Pragma: no-cache` | **重写 S-15 的自证测试**：改为真实调用 handler 断言响应头 |
| B5-4 | **S-9（D3）**：threepid 孤儿路由 —— 要么补装配点，要么从 `ROUTE_CONTRACT.md` 删除 | ledger 与契约文档一致 |
| B5-5 | **S-13 附带**：`extract_registered.py` 链式方法只取第一个 method 的缺陷（`get().put().delete()` 只提取 GET） | 对 MSC4108 重新提取 → 4 条全出 |
