...
| B4-4-剩余 ⚠️ | **3 个文件仍未下沉**：`context.rs`（路由 state 直接持 storage 句柄，见 A1/B4-5）、`rendezvous.rs` + `msc4108_rendezvous.rs`(`RendezvousStoreApi`/`RendezvousMessageStorage`，领域内没有 service；注意 `RendezvousMessageStorage` 是**具体类型而非 trait**，下沉时服务只能持 `Arc<RendezvousMessageStorage>`) | 同 B4-4 门禁；每下沉一个文件从白名单删一行 |
