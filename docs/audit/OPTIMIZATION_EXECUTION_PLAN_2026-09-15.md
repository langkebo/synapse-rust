...
| B4-4-迁移 🔶 | **七批共消除 48/49 个文件**（49 → 38 → 19 → 7 → 5 → 4 → 3 → **1**）。手法固定为**依赖方向反转**：服务层 `pub use` 重导出 HTTP 层需要的 DTO / 游标编解码函数，`src/web` 里的 `synapse_storage::<mod>::X` 改写为 `synapse_services::<host>::X`；**不**重导出 storage 的 impl struct 或 store trait。第 1 批（`6ab3d1be`）消掉 event / audit DTO（11 个）；第 2 批（`91cfd3f0`）消掉 `user`/`event_report`/`widget`/`thread`/`saml`/`application_service`/`space`/`beacon`/`federation_blacklist`/`registration_token`/`push_notification`(DTO)/`sticky_event`/`room_tag`（19 个）；第 3 批消掉 `server_notification`/`retention`/`room`(搜索游标)/`sliding_sync`/`feature_flags`/`application_service`(剩余)/`background_update`/`event_report`(剩余)/`module`/`push_notification`(剩余)/audit(根路径)（12 个） | `grep -rl synapse_storage src/web --include='*.rs' \| wc -l` = **1**，与 gate 白名单条目数**逐一相等**；注入探针文件后 gate EXIT=1（本轮复验）；`cargo test --test unit --features test-utils` = 1821 passed / 0 failed |
...
| B4-4-剩余 ⚠️ | **只剩 `context.rs`**：路由 state 结构体直接持有 storage 句柄（DI 面）。DTO 与「有策略的领域」都已下沉完毕；这一处与 B4-5「`src/web` 独立成 crate」是同一件事（新 crate 的 state 类型该怎么拿依赖），按约定并入 B4-5 一起解决 | `scripts/ci/check_web_layering.py` 届时白名单应为 **0 条目**（可整份删除） |
...
| B4-5b ✅ | **`src/web` 已独立为 `synapse-web` crate，根 crate 收到 <5k 行。** 实测根 crate `src/`：**67,457 → 4,324 行**（目标 <5k），`synapse-web` = 60,484 行。落地内容：①`git mv src/web/{mod.rs→lib.rs,middleware,routes,utils}` + `src/federation/**`（federation 胶水与 web 互相依赖，必须同批搬）；②143 个文件的路径重写（`crate::web::`→`crate::`、`crate::common::`→`synapse_common::`、`crate::cache::`→`synapse_cache::`、`crate::e2ee::`→`synapse_e2ee::`、`crate::test_utils::`→`synapse_test_utils::`）；③`lib.rs` 去 glob（`pub use middleware::*` / `routes::*` 改为显式列表）；④根 crate 的 `server/*`、`tests/*`、`benches/*`、`src/bin/*` 共 ~130 处 `synapse_rust::web::` / `crate::web::` 改指 `synapse_web::`；⑤feature 矩阵：`synapse-web` 复制 14 个扩展开关并在根 crate 逐条转发；新建 `[workspace.lints.clippy]` 供新 crate 继承；⑥**4 个门禁的扫描面同步扩容**（否则代码搬出 `src/` 会静默逃逸）：trait 棘轮 ROOTS、sqlx 棘轮 SCAN_DIRS、web 分层门禁 WEB 目录、route-contract 提取器/生成器输出路径——sqlx 计数因此从「假降到 1453」回到真实的 **1484**，trait 从假降 61 回到 **65** |根 `find src -name '*.rs' \| xargs cat \| wc -l` = **4,324**；`cargo check --workspace --all-targets --all-features` = 0；`cargo test -p synapse-web --lib` = 632 passed；unit = 1808 passed / 1 failed（唯一失败是并发会话的 v12 迁移）；federation 胶水随迁） |
| B4-5c ⏳ | **`synapse-web/src/routes/context.rs` 的 DI 面仍未改**：路由 state 结构体直接持有 15+ 个 storage 句柄与 `Arc<PgPool>`（第 217 行）。分层门禁的白名单只剩这一条；此外 `scripts/quality/check_route_layering.sh`（**未接入 CI** 的历史脚本，已同步指向新目录）会因为同一条 `PgPool` 报告违规——该违规在 HEAD 就存在，非本次引入。建议下一步：让 `ServiceContainer` 按域暴露窄接口（或让 state 只持服务），然后删除白名单文件并把该脚本接入 CI | 分层门禁白名单 **0 条目并删除文件**；`check_route_layering.sh` exit=0 |
