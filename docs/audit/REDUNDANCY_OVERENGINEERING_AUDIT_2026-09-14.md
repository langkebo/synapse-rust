...
| 指标 | 数值 |
|---|---|
| 生产源码（7 个 crate 的 `src/`） | **355,111 行** |
...
| `#[allow(dead_code)]` | **151–160** 处（126 处集中在 `api_doc/schemas.rs`） |
...
| 门禁 | 位置 | 状态 | 证据 |
|---|---|---|---|
| **fmt ratchet** | `scripts/check_fmt_ratchet.sh`，`ci.yml:214` | **当前为红（提交态）** | 实测 `fmt debt: current=2 baseline=0` → `::error::fmt debt increased`，脚本 **exit=1**；offending 为 `tests/unit/test_isolation_unification_tests.rs:619`（2 处）。`git diff HEAD -- tests/unit/test_isolation_unification_tests.rs` 为空 ⇒ **未格式化代码是提交进去的**（该文件最后提交 `9f68b4b3`，2026-09-14）。而 `AGENTS.md` 声称 "Format debt is at zero and the CI ratchet is strict" —— **该声明与主干状态不符**。**已独立核验** |
