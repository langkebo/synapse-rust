> **这是一份历史日志（已归档），不是现存问题清单。**
>
> 2026-09-22 起，`docs/audit/GATE_INTEGRITY_FOLLOWUP_2026-09-19.md` 被重写为**只保留未解决问题
> 与优化建议**的清单；本文件保存重写前的完整逐轮叙事（每条修复的红/绿证明、run id、命令输出、
> 测量数字）。下面所有 `§x.y` 编号**保持原样**，因此代码与文档里引用旧编号的注释仍然可解析。
>
> 已解决项不要再从这里抄回清单；新增的未解决项请写入
> `docs/audit/GATE_INTEGRITY_FOLLOWUP_2026-09-19.md`。
