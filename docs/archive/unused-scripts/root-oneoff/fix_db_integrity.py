...
for table, col, ref_table, ref_col in orphan_cleanups:
    try:
        cur.execute(
            f"DELETE FROM {table} WHERE {col} NOT IN (SELECT {ref_col} FROM {ref_table})"
        )
...
try:
    cur.execute(
        "ALTER TABLE room_children ADD CONSTRAINT uq_room_children_parent_child UNIQUE (parent_room_id, child_room_id)"
    )
...
try:
    cur.execute(
        "ALTER TABLE retention_cleanup_queue ADD CONSTRAINT uq_retention_cleanup_queue_room_event UNIQUE (room_id, event_id)"
    )
...
try:
    cur.execute(
        "ALTER TABLE room_summary_state ADD CONSTRAINT uq_room_summary_state_room_type_state UNIQUE (room_id, state_type, state_key)"
    )
