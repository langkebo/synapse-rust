...
mod tests {
    ...
    #[test]
    fn test_server_acl_content_default() {
        let acl = ServerAclContent::default();
        assert!(acl.allow.is_empty());
        assert!(acl.deny.is_empty());
        // Default derive gives false for bool; serde default gives true (tested separately).
        assert!(!acl.allow_ip_literals);
    }
    ...
    fn test_server_acl_content_serde_roundtrip() {
        ...
        let restored: ServerAclContent = serde_json::from_str(&json).unwrap();
    ...
    #[test]
    fn test_from_value_invalid_json_returns_none() {
        let invalid = json!("not_an_object");
        assert!(ServerAclContent::from_value(&invalid).is_none());
    }

    #[test]
    fn test_from_value_with_null_fields_uses_defaults() {
        // Missing allow/deny should default to empty vecs, allow_ip_literals to true.
        let content = json!({"allow": ["*"]});
        let acl = ServerAclContent::from_value(&content).expect("should parse");
        assert_eq!(acl.allow, vec!["*"]);
        assert!(acl.deny.is_empty());
        assert!(acl.allow_ip_literals);
    }
    ...
    #[test]
    fn test_is_ip_literal_edge_cases() {
        // IPv4 with port
        assert!(is_ip_literal("192.168.1.1:8080"));
        // Valid octet at boundary
        assert!(is_ip_literal("255.255.255.255"));
        // Not an IP (5 octets)
        assert!(!is_ip_literal("1.2.3.4.5"));
        // Not an IP (octet > 255)
        assert!(!is_ip_literal("256.1.1.1"));
    }

    #[test]
    fn test_acl_deny_overrides_ip_literal_allow() {
        // Even with allow_ip_literals=true, deny takes precedence.
        let acl = ServerAclContent {
            allow: vec!["*".to_string()],
            deny: vec!["192.168.1.1".to_string()],
            allow_ip_literals: true,
        };
        assert!(!acl.is_server_allowed("192.168.1.1"));
    }
    ...
    #[test]
    fn test_acl_ip_literal_check_before_allow() {
        // allow_ip_literals=false denies IP even if it matches an allow pattern.
        let acl = ServerAclContent {
            allow: vec!["*".to_string()],
            deny: vec![],
            allow_ip_literals: false,
        };
        assert!(!acl.is_server_allowed("10.0.0.1"));
    }
}
